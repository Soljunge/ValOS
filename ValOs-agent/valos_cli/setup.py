"""
Interactive setup wizard for ValOs.
...
Modular wizard with independently-runnable sections:
  ...
  5. Tools — configure text-to-speech (TTS), web search, image generation, and more.

Config files are stored in ~/.valos-agent/ for easy access.
"""
...
from tools.tool_backend_helpers import managed_valos_tools_enabled
from valos_constants import get_optional_skills_dir
...
def _setup_copilot_reasoning_selection(
    ...
    *,
    ...
    from valos_cli.models import github_model_reasoning_efforts, normalize_copilot_model_id
...
def _setup_provider_model_selection(config, provider_id, current_model, prompt_choice, prompt_fn):
    ...
    """
    from valos_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    from valos_cli.config import get_env_value
...
# display_valos_home imported lazily at call sites (stale-module safety during valos-agent update)

from valos_cli.colors import Colors, color


def _setup_gray(text: str) -> str:
...
def mark_onboarding_complete() -> None:
    """Persist that the user completed onboarding."""
    ensure_valos_home()
    _onboarding_marker_path().write_text("done\n", encoding="utf-8")


def print_header(title: str):
    """Print a section header."""
    print()
    print(_setup_gray(f"🪙 {title}"))


def print_info(text: str):
    """Print info text."""
    print(_setup_gray(f"  {text}"))
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(_setup_gray("🤖 ValOs setup"))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure ValOs using environment variables or config commands:")
    print_info("  valos-agent config set model.provider custom")
    print_info("  valos-agent config set model.base_url http://localhost:8080/v1")
    print_info("  valos-agent config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'valos-agent setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt(question: str, default: str = None, password: bool = False) -> str:
    ...
    try:
        if password:
            ...
            value = getpass.getpass(_setup_gray(display))
        else:
            value = input(_setup_gray(display))
...
def prompt_choice(question: str, choices: list, default: int = 0) -> int:
    ...
    print(_setup_gray(question))
    for i, choice in enumerate(choices):
        marker = "[x]" if i == default else "[ ]"
        line = f"  {marker} {choice}"
        if i == default:
            print(_setup_gray(line))
        else:
            print(line)
    ...
    while True:
        try:
            value = input(
                _setup_gray(f"  Select [1-{len(choices)}] ({default + 1}): ")
            )
            if not value:
                return default
            idx = int(value) - 1
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    while True:
        try:
            value = (
                input(_setup_gray(f"{question} [{default_str}]: "))
                .strip()
                .lower()
            )
        except (KeyboardInterrupt, EOFError):
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from valos_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    print()
    print(_setup_gray(f"  ─── {var.get('description', var['name'])} ───"))
    print()
    if tools_str:
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'valos-agent setup')")


def _print_setup_summary(config: dict, valos_home):
    ...
    tool_status = []
    subscription_features = get_valos_subscription_features(config)
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'valos-agent setup' to configure"))
    ...
    if subscription_features.web.managed_by_valos:
        tool_status.append(("Web Search & Extract (ValOs subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_valos:
        tool_status.append(("Browser Automation (ValOs Browserbase)", True, None))
    ...
    if subscription_features.image_gen.managed_by_valos:
        tool_status.append(("Image Generation (ValOs subscription)", True, None))
    ...
    if subscription_features.tts.managed_by_valos:
        tool_status.append(("Text-to-Speech (OpenAI via ValOs subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'valos-agent setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_valos:
        tool_status.append(("Modal Execution (ValOs subscription)", True, None))
    elif config.get("terminal", {}).get("backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'valos-agent setup terminal'"))
    elif managed_valos_tools_enabled() and subscription_features.valos_auth_present:
        tool_status.append(("Modal Execution (optional via ValOs subscription)", True, None))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'valos-agent setup tools' to configure them,"
        )
        from valos_constants import display_valos_home as _dhh
    ...
    from valos_constants import display_valos_home as _dhh
    print(_setup_gray(f"Files are in {_dhh()}/"))
    print()
    print(f"   {_setup_gray('Settings:')}  {get_config_path()}")
    print(f"   {_setup_gray('API Keys:')}  {get_env_path()}")
    print(
        f"   {_setup_gray('Data:')}      {valos_home}/cron/, sessions/, logs/"
    )
    print()

    print(_setup_gray("─" * 60))
    print()
    print(_setup_gray("Edit your configuration:"))
    print()
    print(f"   {_setup_gray('valos-agent setup')}          Re-run the full wizard")
    print(f"   {_setup_gray('valos-agent setup model')}    Change model/provider")
    print(f"   {_setup_gray('valos-agent setup terminal')} Change terminal backend")
    print(f"   {_setup_gray('valos-agent setup gateway')}  Configure messaging")
    print(f"   {_setup_gray('valos-agent setup tools')}    Configure tool providers")
    print()
    print(f"   {_setup_gray('valos-agent config')}         View current settings")
    print(
        f"   {_setup_gray('valos-agent config edit')}    Open config in your editor"
    )
    print(f"   {_setup_gray('valos-agent config set <key> <value>')}")
    print("                          Set a specific value")
    print()
    print("   Or edit the files directly:")
    print(f"   {_setup_gray(f'nano {get_config_path()}')}")
    print(f"   {_setup_gray(f'nano {get_env_path()}')}")
    print()

    print(_setup_gray("─" * 60))
    print()
    print(_setup_gray("Ready to go."))
    print()
    print(f"   {_setup_gray('valos-agent')}              Start chatting")
    print(f"   {_setup_gray('valos-agent gateway')}      Start messaging gateway")
    print(f"   {_setup_gray('valos-agent doctor')}       Check for issues")
    print()
...
# between `valos-agent tools` and `valos-agent setup tools`).
...
def setup_model_provider(config: dict):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``valos-agent model``)
    ...
    provider added to ``valos-agent model`` is automatically available here.
    """
    ...
    # Delegate to the shared valos-agent model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from valos_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: valos-agent model")
    ...
    valos_subscription_selected = selected_provider == "valos"
    ...
    if _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from valos_cli.auth_commands import auth_add_command
            ...
            print_info(
                "ValOs can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if selected_provider in _vision_backends:
        # If the user just selected a backend ValOs can already use for
        # vision, treat it as covered. Auth/setup failure returns earlier.
        _vision_needs_setup = False

    if _vision_needs_setup:
        _prov_names = {
            "valos-api": "ValOs Portal API key",
        ...
        print_info("doesn't currently provide one ValOs can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'valos-agent setup' or configure AUXILIARY_VISION_* settings")


    if selected_provider == "valos" and valos_subscription_selected:
        ...
        if "tts" in changed_defaults:
            print_success("Text-to-speech (TTS) provider set to: OpenAI TTS via your ValOs subscription")
        else:
            print_info(f"Keeping your existing text-to-speech (TTS) provider: {current_tts}")

    save_config(config)

    # Offer TTS provider selection at the end of model setup, except when
    # ValOs subscription defaults are already being applied.
    if selected_provider != "valos":
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_valos_tools_enabled() and subscription_features.valos_auth_present:
        choices.append("ValOs Subscription (managed OpenAI TTS, billed to your subscription)")
        providers.append("valos-openai")
    choices.extend(
        [
    ...
    selected_via_valos = selected == "valos-openai"
    if selected == "valos-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed ValOs gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.valos-agent/.env."
            )
    ...
    elif selected == "openai" and not selected_via_valos:
    ...
    save_config(config)
    print_success(
        f"Text-to-speech (TTS) provider set to: {provider_labels.get(selected, selected)}"
    )


def setup_tts(config: dict):
    """Standalone TTS setup (for 'valos-agent setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where ValOs runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using ValOs via Telegram/Discord, this is where")
        print_info(
        ...
        managed_modal_available = bool(
            managed_valos_tools_enabled()
            and
            get_valos_subscription_features(config).valos_auth_present
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed ValOs gateway and bill to your subscription.")
...
def setup_agent_settings(config: dict):
    ...
    current_max = get_env_value("VALOS_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("VALOS_MAX_ITERATIONS", str(max_iter))
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with ValOs from anywhere.")
    print()
    ...
    if not existing_telegram and prompt_yes_no("Set up Telegram bot?", False):
        ...
        if token:
            ...
            print()
            print_info("📬 Home Channel: where ValOs delivers cron job results,")
    ...
    if not existing_slack and prompt_yes_no("Set up Slack bot?", False):
        ...
        print()
        print_info(
            "   Full guide: https://valos-agent.valos-agent.com/docs/user-guide/messaging/slack/"
        )
        print()
    ...
    if not existing_matrix and prompt_yes_no("Set up Matrix?", False):
        ...
        if token or get_env_value("MATRIX_PASSWORD"):
            ...
            # Home room
            print()
            print_info("📬 Home Room: where ValOs delivers cron job results and notifications.")
    ...
    if not existing_mattermost and prompt_yes_no("Set up Mattermost?", False):
        ...
        if token:
            ...
            # Home channel
            print()
            print_info("📬 Home Channel: where ValOs delivers cron job results and notifications.")
    ...
    if not existing_whatsapp and prompt_yes_no("Set up WhatsApp?", False):
        ...
        print_info("Requires Node.js. Run 'valos-agent whatsapp' for guided setup.")
        print()
        if prompt_yes_no("Enable WhatsApp now?", True):
            save_env_value("WHATSAPP_ENABLED", "true")
            print_success("WhatsApp enabled")
            print_info("Run 'valos-agent whatsapp' to choose your mode (separate bot number")
    ...
    if not existing_webhook and prompt_yes_no("Set up webhooks? (GitHub, GitLab, etc.)", False):
        ...
        print()
        print_info(
            "   Full guide: https://valos-agent.valos-agent.com/docs/user-guide/messaging/webhooks/"
        )
        print()
        ...
        print_info(
            "   https://valos-agent.valos-agent.com/docs/user-guide/messaging/webhooks/#configuring-routes"
        )
        print()
        print_info("   Open config in your editor:  valos-agent config edit")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     valos-agent config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: valos-agent gateway install")
            else:
                print_info("  You can install later: valos-agent gateway install")
                if _is_linux:
                    print_info("  Or as a boot-time service: sudo valos-agent gateway install --system")
                print_info("  Or run in foreground:  valos-agent gateway")
        else:
            print_info("Start the gateway to bring your bots online:")
            print_info("   valos-agent gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `valos-agent setup tools` and `valos-agent tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from valos_cli.config
    ...
    if section_key == "model":
        ...
        if not has_key:
            ...
            try:
                from valos_cli.auth import get_active_provider
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_valos.py"
)


def _offer_openclaw_migration(valos_home: Path) -> bool:
    ...
    print_info("ValOs can import your settings, memories, skills, and API keys.")
    print()
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "openclaw_to_valos", _OPENCLAW_SCRIPT
        )
        ...
        migrator = mod.Migrator(
            ...
            target_root=valos_home.resolve(),
            execute=True,
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in ValOs.")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      valos-agent setup           — full or quick (auto-detected)
      valos-agent setup model     — just model/provider
      valos-agent setup terminal  — just terminal backend
      valos-agent setup gateway   — just messaging platforms
      valos-agent setup tools     — just tool configuration
      valos-agent setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(_setup_gray(f"│     ValOs setup — {label:<37s} │"))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.DIM,
    ...
    print(
        _setup_gray("│                 ValOs setup wizard               │")
    )
    print(
        color("├─────────────────────────────────────────────────────────┤", Colors.DIM)
    )
    print(
        _setup_gray("│  Let's configure your ValOs installation.        │")
    )
    print(
        _setup_gray("│  Press Ctrl+C at any time to exit.               │")
    )
    print(
        color("└─────────────────────────────────────────────────────────┘", Colors.DIM)
    )
    ...
    if is_existing:
        ...
        print_success("You already have ValOs configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, valos_home)
            ...
            print_info("Exiting. Run 'valos-agent setup' again when ready.")
            return
        elif choice == 9:
            print_info("Exiting. Run 'valos-agent setup' again when ready.")
            return
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, valos_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        print_info("  5. Tools — configure text-to-speech (TTS), web search, image generation, and more.")
        print()
        print_info("Press Enter to begin, or Ctrl+C to exit.")
        try:
            input(_setup_gray("  Press Enter to start... "))
        except (KeyboardInterrupt, EOFError):
            print()
            return

        # Offer OpenClaw migration before configuration begins
        migration_ran = _offer_openclaw_migration(valos_home)
        if migration_ran:
    ...
    print_info(f"Data folder:  {valos_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'valos-agent config edit'")
...
def _run_quick_setup(config: dict, valos_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'valos-agent setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect ValOs to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'valos-agent setup gateway'.")
        ...
        for idx in selected_indices:
            ...
            print()
            print(_setup_gray(f"  ─── {emoji} {plat} ───"))
            print()
