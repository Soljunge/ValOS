"""
Unified tool configuration for ValOs.

`valos-agent tools` and `valos-agent setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.valos-agent/config.yaml under
...
# They're still in _VALOS_CORE_TOOLS (available at runtime if enabled),
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from valos_cli.plugins import discover_plugins, get_plugin_toolsets
...
PLATFORMS = {
    "cli":      {"label": "🖥️  CLI",       "default_toolset": "valos-agent-cli"},
    "telegram": {"label": "📱 Telegram",   "default_toolset": "valos-agent-telegram"},
    "discord":  {"label": "💬 Discord",    "default_toolset": "valos-agent-discord"},
    "slack":    {"label": "💼 Slack",      "default_toolset": "valos-agent-slack"},
    "whatsapp": {"label": "📱 WhatsApp",   "default_toolset": "valos-agent-whatsapp"},
    "signal":   {"label": "📡 Signal",     "default_toolset": "valos-agent-signal"},
    "homeassistant": {"label": "🏠 Home Assistant", "default_toolset": "valos-agent-homeassistant"},
    "email":    {"label": "📧 Email",      "default_toolset": "valos-agent-email"},
    "matrix":   {"label": "💬 Matrix",     "default_toolset": "valos-agent-matrix"},
 "dingtalk": {"label": "💬 DingTalk", "default_toolset": "valos-agent-dingtalk"},
    "feishu": {"label": "🪽 Feishu", "default_toolset": "valos-agent-feishu"},
    "wecom": {"label": "💬 WeCom", "default_toolset": "valos-agent-wecom"},
    "api_server": {"label": "🌐 API Server", "default_toolset": "valos-agent-api-server"},
    "mattermost": {"label": "💬 Mattermost", "default_toolset": "valos-agent-mattermost"},
    "webhook": {"label": "🔗 Webhook", "default_toolset": "valos-agent-webhook"},
}
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            {
                "name": "ValOs Subscription",
                "tag": "Managed OpenAI TTS billed to your subscription",
                "env_vars": [],
                "tts_provider": "openai",
                "requires_valos_auth": True,
                "managed_valos_feature": "tts",
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_valos_auth": True,
                "managed_valos_feature": "image_gen",
    ...
    "browser": {
        ...
        "providers": [
            {
                "name": "ValOs Subscription (Browserbase cloud)",
                "tag": "Managed Browserbase billed to your subscription",
                "env_vars": [],
                "browser_provider": "browserbase",
                "requires_valos_auth": True,
                "managed_valos_feature": "browser",
    ...
    if post_setup_key == "browserbase":
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_valos_home()}/valos-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in valos-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "valos-agent-cli" (which include all _VALOS_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "valos-agent-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `valos-agent tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by valos-agent tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (valos-agent-cli, valos-agent-telegram, etc.)
...
def _toolset_has_keys(ts_key: str, config: dict = None) -> bool:
    ...
    if ts_key in {"web", "image_gen", "tts", "browser"}:
        features = get_valos_subscription_features(config)
        feature = features.features.get(ts_key)
        if feature and (feature.available or feature.managed_by_valos):
            return True
...
def _visible_providers(cat: dict, config: dict) -> list[dict]:
    ...
    for provider in cat.get("providers", []):
        if provider.get("managed_valos_feature") and not managed_valos_tools_enabled():
            continue
        if provider.get("requires_valos_auth") and not features.valos_auth_present:
            continue
...
def _is_provider_active(provider: dict, config: dict) -> bool:
    ...
    managed_feature = provider.get("managed_valos_feature")
    if managed_feature:
        features = get_valos_subscription_features(config)
        ...
        if managed_feature == "image_gen":
            return feature.managed_by_valos
        if provider.get("tts_provider"):
            return (
                feature.managed_by_valos
                and config.get("tts", {}).get("provider") == provider["tts_provider"]
            )
        if "browser_provider" in provider:
            current = config.get("browser", {}).get("cloud_provider")
            return feature.managed_by_valos and provider["browser_provider"] == current
        if provider.get("web_backend"):
            current = config.get("web", {}).get("backend")
            return feature.managed_by_valos and current == provider["web_backend"]
        return feature.managed_by_valos
...
def _configure_provider(provider: dict, config: dict):
    ...
    if provider.get("requires_valos_auth"):
        features = get_valos_subscription_features(config)
        if not features.valos_auth_present:
            _print_warning("  ValOs Subscription is only available after logging into ValOs Portal.")
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your ValOs subscription.")
            override_envs = provider.get("override_env_vars", [])
            if any(get_env_value(env_var) for env_var in override_envs):
                _print_warning(
                    "  Direct credentials are still configured and may take precedence until you remove them from ~/.valos-agent/.env."
                )
        return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `valos-agent tools` and `valos-agent setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ ValOs Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            auto_configured = apply_valos_managed_defaults(
                config,
                enabled_toolsets=new_enabled,
            )
            if managed_valos_tools_enabled():
                for ts_key in sorted(auto_configured):
                    label = next((l for k, l, _ in CONFIGURABLE_TOOLSETS if k == ts_key), ts_key)
                    print(color(f"  ✓ {label}: using your ValOs subscription defaults", Colors.GREEN))
    ...
    print()
    from valos_constants import display_valos_home
    print(color(f"  Tool configuration saved to {display_valos_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'valos-agent' or gateway restart.", Colors.DIM))
    print()
