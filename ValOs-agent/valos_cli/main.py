#!/usr/bin/env python3
"""
ValOs CLI - Main entry point.

Usage:
    valos-agent                     # Interactive chat (default)
    valos-agent chat                # Interactive chat
    valos-agent web                 # Open local web chat
    valos-agent gateway             # Run gateway in foreground
    valos-agent gateway start       # Start gateway as service
    valos-agent gateway stop        # Stop gateway service
    valos-agent gateway status      # Show gateway status
    valos-agent gateway install     # Install gateway service
    valos-agent gateway uninstall   # Uninstall gateway service
    valos-agent setup               # Interactive setup wizard
    valos-agent logout              # Clear stored authentication
    valos-agent status              # Show status of all components
    valos-agent cron                # Manage cron jobs
    valos-agent cron list           # List cron jobs
    valos-agent cron status         # Check if cron scheduler is running
    valos-agent doctor              # Check configuration and dependencies
    valos-agent honcho setup                    # Configure Honcho AI memory integration
    valos-agent honcho status                   # Show Honcho config and connection status
    valos-agent honcho sessions                 # List directory → session name mappings
    valos-agent honcho map <name>               # Map current directory to a session name
    valos-agent honcho peer                     # Show peer names and dialectic settings
    valos-agent honcho peer --user NAME         # Set user peer name
    valos-agent honcho peer --ai NAME           # Set AI peer name
    valos-agent honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    valos-agent honcho mode                     # Show current memory mode
    valos-agent honcho mode [hybrid|honcho|local]  # Set memory mode
    valos-agent honcho tokens                   # Show token budget settings
    valos-agent honcho tokens --context N       # Set session.context() token cap
    valos-agent honcho tokens --dialectic N     # Set dialectic result char cap
    valos-agent honcho identity                 # Show AI peer identity representation
    valos-agent honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    valos-agent honcho migrate                  # Step-by-step migration guide: OpenClaw native → ValOs + Honcho
    valos-agent version             Show version
    valos-agent update              Update to latest version
    valos-agent uninstall           Uninstall ValOs
    valos-agent acp                 Run as an ACP server for editor integration
    valos-agent sessions browse     Interactive session picker with search

    valos-agent claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (valos-agent tools, valos-agent setup, valos-agent model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'valos-agent {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any valos-agent module import.
#
# Many modules cache VALOS_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("VALOS_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.valos-agent/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set VALOS_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check ~/.valos-agent/active_profile
    if profile_name is None:
        try:
            active_path = Path.home() / ".valos-agent" / "active_profile"
    ...
    # 3. If we found a profile, resolve and set VALOS_HOME
    if profile_name is not None:
        try:
            from valos_cli.profiles import resolve_profile_env
            valos_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent valos-agent from starting
            print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
            return
        os.environ["VALOS_HOME"] = valos_home
...
# Load .env from ~/.valos-agent/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from valos_cli.config import get_valos_home
from valos_cli.env_loader import load_valos_dotenv
load_valos_dotenv(project_env=PROJECT_ROOT / '.env')
...
from valos_cli import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    from valos_cli.config import get_env_path, get_valos_home, load_config
    from valos_cli.auth import get_auth_status

    # Determine whether ValOs itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from valos_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_valos_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Check for ValOs Portal OAuth credentials
    auth_file = get_valos_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if ValOs has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants ValOs to use their tokens.
    if _has_valos_config:
        try:
...
def _onboarding_marker_path() -> Path:
    """Return the file that marks onboarding as completed."""
    return get_valos_home() / ".onboarding_complete"


def _is_onboarding_complete() -> bool:
    """Return True when the user has completed the setup wizard."""
    return _onboarding_marker_path().exists()
...
def cmd_chat(args):
    ...
    try:
        from valos_cli.macos_menubar import maybe_launch_for_current_session
        maybe_launch_for_current_session()
    except Exception:
        pass
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'valos-agent sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    # Onboarding guard: after a reset, keep sending the user back into setup
    # until the full wizard completes.
    if not _is_onboarding_complete():
        print()
        print("ValOs onboarding is not finished yet.")
        print()

        from valos_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance

        if not is_interactive_stdin():
            print_noninteractive_setup_guidance(
                "No interactive TTY detected for onboarding."
            )
            sys.exit(1)
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like ValOs isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  valos-agent setup")
        print()
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'valos-agent setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from valos_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["VALOS_YOLO_MODE"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["VALOS_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from valos_cli.gateway import gateway_command
    gateway_command(args)


def cmd_web(args):
    """Run the local web interface."""
    from valos_cli.web import web_command
    web_command(args)
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from valos_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with ValOs?")
        print()
    ...
    session_dir = get_valos_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: valos-agent gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  valos-agent gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ ValOs'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  valos-agent gateway")
        ...
        print()
        print("  Or install as a service: valos-agent gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'valos-agent whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    _require_tty("setup")
    from valos_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``valos-agent model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from valos_cli.auth import (
        resolve_provider, AuthError, format_auth_error,
    )
    from valos_cli.config import load_config, get_env_value
    ...
    effective_provider = (
        config_provider
        or os.getenv("VALOS_INFERENCE_PROVIDER")
        or "auto"
    )
    try:
    ...
    providers = [
        ...
        ("valos", "ValOs Portal (ValOs subscription)"),
        ("openai-codex", "OpenAI Codex"),
    ...
    elif selected_provider == "valos":
        _model_flow_valos(config, current_model, args=args)
        ...
        def _curses_menu(stdscr):
            ...
            def _render():
                ...
                for i, choice in enumerate(choices):
                    ...
                    box = "[x]" if i == selected else "[ ]"
                    line = f" {pointer}  {box} {choice}"
                    attr = curses.A_NORMAL
                    if i == cursor:
                        attr = curses.A_BOLD
                        if curses.has_colors():
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from valos_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from valos_cli.config import get_env_value, save_env_value
    ...
    from valos_cli.models import model_ids
    ...
    if selected:
        ...
        from valos_cli.config import load_config, save_config
        cfg = load_config()
        model = cfg.get("model")
...
def _model_flow_valos(config, current_model="", args=None):
    """ValOs Portal provider: ensure logged in, then pick model."""
    from valos_cli.auth import (
        get_provider_auth_state, _prompt_model_selection, _save_model_choice,
        _update_config_for_provider, resolve_valos_runtime_credentials,
        fetch_valos_models, AuthError, format_auth_error,
        _login_valos, PROVIDER_REGISTRY,
    )
    from valos_cli.config import get_env_value, save_config, save_env_value
    from valos_cli.valos_subscription import (
        apply_valos_provider_defaults,
        get_valos_subscription_explainer_lines,
    )
    import argparse

    state = get_provider_auth_state("valos")
    if not state or not state.get("access_token"):
        print("Not logged into ValOs Portal. Starting login...")
        print()
        try:
            ...
            _login_valos(mock_args, PROVIDER_REGISTRY["valos"])
            print()
            for line in get_valos_subscription_explainer_lines():
                print(line)
        except SystemExit:
            ...
            return
        # login_valos already handles model selection + config update
        return

    # Already logged in — use curated model list (same as OpenRouter defaults).
    # The live /models endpoint returns hundreds of models; the curated list
    # shows only agentic models users recognize from OpenRouter.
    from valos_cli.models import _PROVIDER_MODELS
    model_ids = _PROVIDER_MODELS.get("valos", [])
    if not model_ids:
        print("No curated models available for ValOs Portal.")
        return
    ...
    try:
        creds = resolve_valos_runtime_credentials(min_key_ttl_seconds=5 * 60)
    except Exception as exc:
        relogin = isinstance(exc, AuthError) and exc.relogin_required
        msg = format_auth_error(exc) if isinstance(exc, AuthError) else str(exc)
        if relogin:
            print(f"Session expired: {msg}")
            print("Re-authenticating with ValOs Portal...\n")
            try:
                mock_args = argparse.Namespace(
    ...
    if selected:
        _save_model_choice(selected)
        # Reactivate ValOs as the provider and update config
        inference_url = creds.get("base_url", "")
        _update_config_for_provider("valos", inference_url)
        ...
        changed_defaults = apply_valos_provider_defaults(config)
        save_config(config)
        print(f"Default model set to: {selected} (via ValOs Portal)")
        if "tts" in changed_defaults:
            print("Text-to-speech (TTS) provider set to: OpenAI TTS via your ValOs subscription")
        else:
            current_tts = str(config.get("tts", {}).get("provider") or "edge")
            if current_tts.lower() not in {"", "edge"}:
                print(f"Keeping your existing text-to-speech (TTS) provider: {current_tts}")
        print()
        for line in get_valos_subscription_explainer_lines():
            print(line)
    else:
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from valos_cli.codex_models import get_codex_model_ids
    from valos_cli.config import get_env_value, save_env_value
    import argparse
    ...
    _codex_token = None
    try:
        from valos_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_custom(config):
    ...
    """
    from valos_cli.auth import _save_model_choice, deactivate_provider
    from valos_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    from valos_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `valos-agent model` to set a model.")
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from valos_cli.auth import _save_model_choice, deactivate_provider
    from valos_cli.config import save_env_value, load_config, save_config
    from valos_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from valos_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from valos_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates ValOs turns to `copilot --acp`.")
    print("  ValOs currently starts its own ACP subprocess for each request.")
    print("  ValOs uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set VALOS_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id in {"opencode-zen", "opencode-go"}:
        ...
        model_list = [normalize_openai_model_id(mid) for mid in model_list]
        current_model = normalize_openai_model_id(current_model)
        model_list = list(dict.fromkeys(mid for mid in model_list if mid))
    ...
    if selected:
        if provider_id in {"opencode-zen", "opencode-go"}:
            ...
            selected = normalize_openai_model_id(selected)
        ...
        if provider_id in {"opencode-zen", "opencode-go"}:
            ...
            model["api_mode"] = openai_model_api_mode(selected)
        else:
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from valos_constants import display_valos_home as _dhh_fn
            print(f"    ValOs will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
    ...
    try:
        ...
        print("    4. Re-run:               valos-agent model")
        print()
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'valos-agent model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate ValOs CLI with a provider."""
    from valos_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from valos_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from valos_cli.auth_commands import auth_command
    auth_command(args)


def cmd_status(args):
    """Show status of all components."""
    from valos_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from valos_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from valos_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from valos_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from valos_cli.config import config_command
    config_command(args)


def cmd_version(args):
    """Show version."""
    print(f"ValOs v{__version__} ({__release_date__})")
    ...
    try:
        from valos_cli.banner import check_for_updates
        from valos_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from valos_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _update_via_zip(args):
    """Update ValOs by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/horatiu.sol/valos-agent/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="valos-agent-update-")
        zip_path = os.path.join(tmp_dir, f"valos-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to valos-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"valos-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("valos-agent-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if ValOs behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        do_reset = True
        if prompt_user:
            print("\nReset working tree to clean state so ValOs can run?")
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but ValOs couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but ValOs couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if ValOs behaves unexpectedly.")
    return True

# =========================================================================
# Fork detection and upstream management for `valos-agent update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/horatiu.sol/valos-agent.git",
    "git@github.com:horatiu.sol/valos-agent.git",
    "https://github.com/horatiu.sol/valos-agent",
    "git@github.com:horatiu.sol/valos-agent",
}
OFFICIAL_REPO_URL = "https://github.com/horatiu.sol/valos-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_valos_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from valos_constants import get_valos_home
        (get_valos_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official ValOs repository.")
        print("  This means you may miss updates from horatiu.sol/valos-agent.")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/horatiu.sol/valos-agent.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/horatiu.sol/valos-agent.git' to add later.")
...
def _invalidate_update_cache():
    ...
    ``valos-agent update``, every profile is now current.
    """
    homes = []
    # Default profile home
    default_home = Path.home() / ".valos-agent"
    homes.append(default_home)
    # Named profiles under ~/.valos-agent/profiles/
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "valos-agent[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update ValOs to the latest version."""
    import shutil
    from valos_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update ValOs")
        return
    
    print("⚕ Updating ValOs...")
    print()
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/horatiu.sol/valos-agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_valos_home added to valos_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload valos_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_valos_home() added since the last release.
        try:
            import importlib
            import valos_constants as _hc
        ...
        try:
            from valos_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if not (sys.stdin.isatty() and sys.stdout.isatty()):
                ...
                print("    Run 'valos-agent config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                print()
                print("Skipped. Run 'valos-agent config migrate' later to configure.")
        else:
        ...
        try:
            ...
            # Discover all valos-agent-gateway* units (default + profiles)
            if is_linux():
                try:
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        result = subprocess.run(
                            scope_cmd + ["list-units", "valos-agent-gateway*", "--plain", "--no-legend", "--no-pager"],
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # e.g. valos-agent-gateway.service or valos-agent-gateway-coder.service
            ...
            if is_macos():
                try:
                    from valos_cli.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: valos-agent gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print("    (or: valos-agent -p <profile> gateway run  for each profile)")
        ...
        print("  valos-agent model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``valos-agent -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from valos_cli.profiles import (
    ...
    from valos_constants import display_valos_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `valos-agent profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_valos_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → valos-agent -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.valos-agent)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  valos-agent profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     valos-agent -p {name} chat")
                else:
            ...
            if clone or clone_all:
                from valos_constants import get_valos_home
                profile_dir_display = f"~/.valos-agent/profiles/{name}"
        ...
        name = args.profile_name
        from valos_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from valos_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        if remove:
            ...
            if wrapper_path:
                ...
                if custom_name:
                    wrapper_path.write_text(f'#!/bin/sh\nexec valos-agent -p {name} "$@"\n')
                print(f"✓ Alias created: {wrapper_path}")
                if not _is_wrapper_dir_in_path():
                    print(f"⚠ {_get_wrapper_dir()} is not in your PATH.")

    elif action == "rename":
        from valos_cli.profiles import rename_profile
        try:
        ...
        from valos_cli.profiles import export_profile
        name = args.profile_name
        ...
        from valos_cli.profiles import import_profile
        try:
...
def cmd_completion(args):
    ...
    from valos_cli.profiles import generate_bash_completion, generate_zsh_completion
...
def main():
    """Main entry point for valos-agent CLI."""
    parser = argparse.ArgumentParser(
        prog="valos-agent",
        description="ValOs - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    valos-agent                        Start interactive chat
    valos-agent chat -q "Hello"        Single query mode
    valos-agent -c                     Resume the most recent session
    valos-agent -c "my project"        Resume a session by name (latest in lineage)
    valos-agent --resume <session_id>  Resume a specific session by ID
    valos-agent setup                  Run setup wizard
    valos-agent logout                 Clear stored authentication
    valos-agent auth add <provider>    Add a pooled credential
    valos-agent auth list              List pooled credentials
    valos-agent auth remove <p> <n>    Remove pooled credential by index
    valos-agent auth reset <provider>  Clear exhaustion status for a provider
    valos-agent model                  Select default model
    valos-agent config                 View configuration
    valos-agent config edit            Edit config in $EDITOR
    valos-agent config set model gpt-4 Set a config value
    valos-agent gateway                Run messaging gateway
    valos-agent -s valos-agent-dev,github-auth
    valos-agent -w                     Start in isolated git worktree
    valos-agent gateway install        Install gateway background service
    valos-agent sessions list          List past sessions
    valos-agent sessions browse        Interactive session picker
    valos-agent sessions rename ID T   Rename/title a session
    valos-agent update                 Update to latest version

For more help on a command:
    valos-agent <command> --help
"""
    )
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with ValOs"
    )
    ...
    chat_parser.add_argument(
        "--provider",
        choices=["auto", "openrouter", "openai", "valos", "openai-codex", "copilot-acp", "copilot", "anthropic", "huggingface", "zai", "kimi-coding", "minimax", "minimax-cn", "kilocode"],
        default=None,
    ...
    model_parser.add_argument(
        "--portal-url",
        help="Portal base URL for ValOs login (default: production portal)"
    )
    model_parser.add_argument(
        "--inference-url",
        help="Inference API base URL for ValOs login (default: production inference API)"
    )
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for ValOs login (default: valos-agent-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
        help="OAuth scope to request for ValOs login"
    )
    model_parser.add_argument(
        "--no-browser",
        action="store_true",
        help="Do not attempt to open the browser automatically during ValOs login"
    )
    model_parser.add_argument(
        "--timeout",
        type=float,
        default=15.0,
        help="HTTP request timeout in seconds for ValOs login (default: 15)"
    )
    model_parser.add_argument(
        "--ca-bundle",
        help="Path to CA bundle PEM file for ValOs TLS verification"
    )
    model_parser.add_argument(
        "--insecure",
        action="store_true",
        help="Disable TLS verification for ValOs login (testing only)"
    )
    ...
    web_parser = subparsers.add_parser(
        "web",
        help="Open the local browser chat interface",
        description="Serve ValOs in a local web UI and start the API backend if needed",
    )
    ...
    web_parser.add_argument(
        ...
        type=int,
        default=3000,
        help="Port for the local web UI (default: 3000)",
    )
    web_parser.add_argument(
        "--api-host",
        default="127.0.0.1",
        help="Host for the ValOs API server (default: 127.0.0.1)",
    )
    web_parser.add_argument(
        "--api-port",
        type=int,
        default=8642,
        help="Port for the ValOs API server (default: 8642)",
    )
    web_parser.add_argument(
        "--no-open",
        action="store_true",
    ...
    web_parser.add_argument(
        "--no-gateway",
        action="store_true",
        help="Serve only the web UI without starting the ValOs gateway",
    )
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure ValOs with an interactive wizard. "
                    "Run a specific section: valos-agent setup model|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for ValOs CLI"
    )
    login_parser.add_argument(
        "--provider",
        choices=["valos", "openai-codex"],
        default=None,
        help="Provider to authenticate with (default: valos)"
    )
    login_parser.add_argument(
        "--portal-url",
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: valos-agent-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    auth_add.add_argument("--portal-url", help="ValOs portal base URL")
    auth_add.add_argument("--inference-url", help="ValOs inference base URL")
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of ValOs components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with ValOs setup"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage ValOs configuration"
    )
    ...
    def cmd_pairing(args):
        from valos_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from valos_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from valos_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/valos-agent-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from valos_cli.plugins_cmd import plugins_command
    ...
    # Provider selection happens via 'valos-agent memory setup'.
    # =========================================================================
    honcho_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "plugin — initial setup is via 'valos-agent memory setup'. These commands\n"
    ...
    honcho_subparsers.add_parser("setup", help="Initial Honcho setup (redirects to valos-agent memory setup)")
    ...
    honcho_subparsers.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to ValOs Honcho",
    )
    ...
    def cmd_honcho(args):
        ...
        if sub == "setup":
            ...
            print("  Running 'valos-agent memory setup'...\n")
            from valos_cli.memory_setup import memory_command
            memory_command(args)
            return
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'valos-agent tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # valos-agent tools list [--platform cli]
    ...
    # valos-agent tools disable <name...> [--platform cli]
    ...
    # valos-agent tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from valos_cli.tools_config import tools_disable_enable_command
            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from valos_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run ValOs as an MCP server",
        description=(
            "Manage MCP server connections and run ValOs as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'valos-agent mcp add' to connect to a new server, or\n"
            "'valos-agent mcp serve' to expose ValOs conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run ValOs as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from valos_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch valos-agent --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            valos_bin = shutil.which("valos-agent")
            if valos_bin:
                os.execvp(valos_bin, ["valos-agent", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "valos_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to ValOs"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to ValOs",
    ...
    def cmd_claw(args):
        from valos_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update ValOs to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Reset ValOs data or restart onboarding",
        description="Ask whether to delete all ValOs data and restart onboarding, or keep data and start the agent."
    )
    uninstall_parser.add_argument(
        "--full",
        action="store_true",
        help="Delete all ValOs data immediately and restart onboarding"
    )
    uninstall_parser.add_argument(
        "--yes", "-y",
        action="store_true",
        help="Skip confirmation and delete all ValOs data"
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run ValOs as an ACP (Agent Client Protocol) server",
        description="Start ValOs in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch ValOs as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated ValOs instances",
    )
    ...
    # e.g. ``valos-agent -c Pokemon Agent Dev`` → ``valos-agent -c 'Pokemon Agent Dev'``
