...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "valos", "openai-codex"}
...
def auth_add_command(args) -> None:
    ...
    if not requested_type:
        if provider.startswith(CUSTOM_POOL_PREFIX):
            ...
            requested_type = AUTH_TYPE_OAUTH if provider in {"anthropic", "valos", "openai-codex"} else AUTH_TYPE_API_KEY
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_valos_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:valos_pkce",
    ...
    if provider == "valos":
        creds = auth_mod._valos_device_code_login(
    ...
    raise SystemExit(f"`valos-agent auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `valos-agent auth` is called bare."""
