"""
Doctor command for valos-agent CLI.

Diagnoses issues with ValOs setup.
"""
...
from valos_cli.config import get_project_root, get_valos_home, get_env_path
from valos_constants import display_valos_home

PROJECT_ROOT = get_project_root()
VALOS_HOME = get_valos_home()
_DHH = display_valos_home()  # user-facing display path (e.g. ~/.valos-agent or ~/.valos-agent/profiles/coder)

# Load environment variables from ~/.valos-agent/.env so API key checks work
...
from valos_constants import OPENROUTER_MODELS_URL
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.valos-agent/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `valos-agent`.
    os.environ.setdefault("VALOS_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 ValOs Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.valos-agent/.env (primary location for user config)
    env_path = VALOS_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'valos-agent setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'valos-agent setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'valos-agent setup' to create one")
                issues.append("Run 'valos-agent setup' to create .env")
    
    # Check ~/.valos-agent/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    try:
        from valos_cli.auth import get_valos_auth_status, get_codex_auth_status

        valos_status = get_valos_auth_status()
        if valos_status.get("logged_in"):
            check_ok("ValOs Portal auth", "(logged in)")
        else:
            check_warn("ValOs Portal auth", "(not logged in)")
    ...
    valos_home = VALOS_HOME
    if valos_home.exists():
        check_ok(f"{_DHH} directory exists")
    else:
        if should_fix:
            valos_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = valos_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give ValOs a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# ValOs Persona\n\n"
                "<!-- Edit this file to customize how ValOs communicates. -->\n\n"
                "You are ValOs, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = valos_home / "memories"
    ...
    state_db_path = valos_home / "state.db"
    ...
    _apikey_providers = [
        ...
        # MiniMax APIs don't support /models endpoint — https://github.com/horatiu.sol/valos-agent/issues/811
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'valos-agent setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: valos-agent skills list)")
    ...
    try:
        ...
        if not _honcho_cfg_path.exists():
            check_warn("Honcho config not found", "run: valos-agent memory setup")
        elif not hcfg.enabled:
            check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
        elif not (hcfg.api_key or hcfg.base_url):
            check_fail("Honcho API key or base URL not set", "run: valos-agent memory setup")
            issues.append("No Honcho API key — run 'valos-agent memory setup'")
        else:
    ...
    # =========================================================================
    try:
        from valos_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "valos-agent -p" in content:
                            _m = _re.search(r"valos-agent -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'valos-agent doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
