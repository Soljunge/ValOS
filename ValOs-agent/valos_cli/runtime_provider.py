...
from valos_cli import auth as auth_mod
...
from valos_cli.auth import (
    ...
    format_auth_error,
    resolve_provider,
    resolve_valos_runtime_credentials,
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from valos_cli.models import copilot_model_api_mode
...
def _openai_runtime_api_mode(model_cfg: Dict[str, Any]) -> str:
    model_name = str(model_cfg.get("default") or "").strip()
    try:
        from valos_cli.models import openai_model_api_mode

        return openai_model_api_mode(model_name)
    except Exception:
        return "chat_completions"
...
def _resolve_runtime_from_pool_entry(
    *,
    ...
    if provider == "openai-codex":
        ...
        api_mode = _openai_runtime_api_mode(model_cfg)
        cfg_provider = str(model_cfg.get("provider") or "").strip().lower()
        if cfg_provider == "openai":
            cfg_base_url = str(model_cfg.get("base_url") or "").strip().rstrip("/")
            base_url = cfg_base_url or base_url or "https://api.openai.com/v1"
        else:
        ...
        if configured_mode and _provider_supports_explicit_api_mode(provider, configured_provider):
            ...
            from valos_cli.models import opencode_model_api_mode
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "openai":
        ...
        if cfg_provider == "openai":
            cfg_base_url = str(model_cfg.get("base_url") or "").strip().rstrip("/")
        base_url = explicit_base_url or cfg_base_url or "https://api.openai.com/v1"
        api_key = explicit_api_key
        if not api_key:
    ...
    if provider == "valos":
        state = auth_mod.get_provider_auth_state("valos") or {}
        base_url = (
            explicit_base_url
            or str(state.get("inference_base_url") or auth_mod.DEFAULT_VALOS_INFERENCE_URL).strip().rstrip("/")
        )
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        return {
            ...
            "base_url": creds.get("base_url", "").rstrip("/"),
            "api_key": creds.get("api_key", ""),
            "source": creds.get("source", "valos-agent-auth-store"),
            "last_refresh": creds.get("last_refresh"),
            "requested_provider": requested_provider,
        }
