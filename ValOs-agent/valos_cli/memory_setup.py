"""valos-agent memory setup|status — configure memory provider plugins.
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as valos-agent tools)
# ---------------------------------------------------------------------------
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.valos-agent/plugins/ and try again.\n")
        return
    ...
    env_path = Path(os.environ.get("VALOS_HOME", os.path.expanduser("~/.valos-agent"))) / ".env"
    env_writes = {}
    ...
    valos_home = str(Path(os.environ.get("VALOS_HOME", os.path.expanduser("~/.valos-agent"))))
    if provider_config and hasattr(provider, "save_config"):
        try:
            provider.save_config(provider_config, valos_home)
        except Exception as e:
...
def cmd_status(args) -> None:
    ...
    if provider_name:
        ...
        if found:
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.valos-agent/plugins/")
