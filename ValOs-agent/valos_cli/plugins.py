...
1. **User plugins**   – ``~/.valos-agent/plugins/<name>/``
2. **Project plugins** – ``./.valos-agent/plugins/<name>/`` (opt-in via
   ``VALOS_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``valos_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "valos_agent.plugins"

_NS_PARENT = "valos_plugins"
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.valos-agent/plugins/)
        valos_home = os.environ.get("VALOS_HOME", os.path.expanduser("~/.valos-agent"))
        user_dir = Path(valos_home) / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.valos-agent/plugins/)
        if _env_enabled("VALOS_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".valos-agent" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``valos_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``valos-agent tools`` TUI so plugin-provided toolsets appear
