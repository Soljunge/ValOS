"""
MCP Server Management CLI — ``valos-agent mcp`` subcommand.

Implements ``valos-agent mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.valos-agent/config.yaml under the ``mcp_servers`` key.
"""
...
from valos_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_valos_home,  # noqa: F401 — used by test mocks
)
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  valos-agent mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  valos-agent mcp add github --command npx --args @modelcontextprotocol/server-github')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_valos_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: valos-agent mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_valos_home()}/config.yaml ({tool_count}/{total} tools enabled)")
    _info("Start a new session to use these tools.")


# ─── valos-agent mcp remove ───────────────────────────────────────────────────────
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  valos-agent mcp add <name> --url <endpoint>')
        _info('  valos-agent mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
# ─── valos-agent mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'valos-agent mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``valos-agent mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("valos-agent mcp serve                              Run as MCP server")
        _info("valos-agent mcp add <name> --url <endpoint>        Add an MCP server")
        _info("valos-agent mcp add <name> --command <cmd>         Add a stdio server")
        _info("valos-agent mcp remove <name>                      Remove a server")
        _info("valos-agent mcp list                               List servers")
        _info("valos-agent mcp test <name>                        Test connection")
        _info("valos-agent mcp configure <name>                   Toggle tools")
        print()
