"""Slash command definitions and autocomplete for the ValOs CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("webui", "Open the local ValOs web interface", "Tools & Skills",
               cli_only=True),
    ...
    CommandDef("paste", "Check clipboard for an image and attach it", "Info",
               cli_only=True),
    CommandDef("update", "Update ValOs to the latest version", "Info",
               gateway_only=True),
...
def _resolve_config_gates() -> set[str]:
    ...
    try:
        ...
        config_path = os.path.join(
            os.getenv("VALOS_HOME", os.path.expanduser("~/.valos-agent")),
            "config.yaml",
        )
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``valos-agent skills
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /valos-agent handler.

    Maps both canonical names and aliases so /valos-agent bg do stuff works
    the same as /valos-agent background do stuff.
    """
    overrides = _resolve_config_gates()
