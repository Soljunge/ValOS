...
def switch_model(
            ...
            if target_provider == "custom":
                return ModelSwitchResult(
                    ...
                    error_message=(
                        ...
                        "or run: valos-agent setup → Custom OpenAI-compatible endpoint"
                    ),
                )
            return ModelSwitchResult(
                success=False,
    ...
    if target_provider in {"opencode-zen", "opencode-go"}:
        ...
        api_mode = openai_model_api_mode(new_model)
    ...
    if not cust_base or "openrouter.ai" in cust_base:
        return CustomAutoResult(
            ...
            error_message=(
                ...
                "in .env, or run: valos-agent setup → Custom OpenAI-compatible endpoint"
            ),
        )
