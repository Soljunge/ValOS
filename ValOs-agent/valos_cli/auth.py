"""
Multi-provider authentication system for ValOs.

Supports OAuth device code flows (ValOs Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.valos-agent/auth.json with cross-process file locking.
...
from valos_cli.config import get_valos_home, get_config_path
...
# ValOs Portal defaults
DEFAULT_VALOS_PORTAL_URL = "https://portal.valos-agent.com"
DEFAULT_VALOS_INFERENCE_URL = "https://inference-api.valos-agent.com/v1"
DEFAULT_VALOS_CLIENT_ID = "valos-agent-cli"
DEFAULT_VALOS_SCOPE = "inference:mint_agent_key"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    "valos": ProviderConfig(
        id="valos",
        name="ValOs Portal",
        auth_type="oauth_device_code",
        portal_base_url=DEFAULT_VALOS_PORTAL_URL,
        inference_base_url=DEFAULT_VALOS_INFERENCE_URL,
        client_id=DEFAULT_VALOS_CLIENT_ID,
        scope=DEFAULT_VALOS_SCOPE,
    ),
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from valos_cli.copilot_auth import resolve_copilot_token
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `valos-agent model` to re-authenticate."

    if error.code == "subscription_required":
        return (
            "No active paid subscription found on ValOs Portal. "
    ...
    if error.code == "insufficient_credits":
        return (
            ...
            "Top up/renew credits in ValOs Portal, then retry."
        )
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("VALOS_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.valos-agent/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    return get_valos_home() / "auth.json"
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "valos_portal" in systems:
            providers["valos"] = systems["valos_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "valos" if providers else None}
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `valos-agent logout`.
...
def resolve_provider(
    ...
    *,
    ...
    raise AuthError(
        "No inference provider configured. Run 'valos-agent model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.valos-agent/.env.",
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.valos-agent/auth.json (not ~/.codex/)
#
# ValOs maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from ValOs auth store (~/.valos-agent/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `valos-agent login` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `valos-agent login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `valos-agent login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `valos-agent login` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to ValOs auth store (~/.valos-agent/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating ValOs auth state."""
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to ValOs auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from ValOs's own Codex token store."""
    try:
        ...
        if cli_tokens:
            logger.info("Migrating Codex credentials from ~/.codex/ to ValOs auth store")
            print("⚠️  Migrating Codex credentials to ValOs's own auth store.")
            print("   This avoids conflicts with Codex CLI and VS Code.")
            print("   Run `valos-agent login` to create a fully independent session.\n")
    ...
    refresh_timeout_seconds = float(os.getenv("VALOS_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other ValOs processes
    ...
    base_url = (
        os.getenv("VALOS_CODEX_BASE_URL", "").strip().rstrip("/")
...
# =============================================================================
# ValOs Portal — token refresh, agent key minting, model discovery
# =============================================================================
    ...
    *,
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="valos", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="valos", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token"}
    raise AuthError(description, provider="valos", code=code, relogin_required=relogin)
    ...
    *,
    ...
    if response.status_code == 200:
        ...
        if "api_key" not in payload:
            raise AuthError("Mint response missing api_key",
                            provider="valos", code="server_error")
        return payload
    ...
    try:
        ...
        raise AuthError("Agent key mint request failed",
                        provider="valos", code="server_error") from exc
...
def fetch_valos_models(
    *,
    ...
    """Fetch available model IDs from the ValOs inference API."""
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="valos", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip ValOs models — they're not reliable for agentic tool-calling
            if "valos-agent" in mid.lower():
                continue
...
def resolve_valos_access_token(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    """Resolve a refresh-aware ValOs Portal access token for managed tool gateways."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
        state = _load_provider_state(auth_store, "valos")

        if not state:
            raise AuthError(
                "ValOs is not logged into ValOs Portal.",
                provider="valos",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("VALOS_PORTAL_BASE_URL")
            or os.getenv("VALOS_PORTAL_BASE_URL")
            or DEFAULT_VALOS_PORTAL_URL
        ).rstrip("/")
        client_id = str(state.get("client_id") or DEFAULT_VALOS_CLIENT_ID)
        ...
        if not isinstance(access_token, str) or not access_token:
            raise AuthError(
                "No access token found for ValOs Portal login.",
                provider="valos",
                relogin_required=True,
            )
        ...
        _save_provider_state(auth_store, "valos", state)
        _save_auth_store(auth_store)
        return state["access_token"]


def refresh_valos_oauth_pure(
    access_token: str,
    refresh_token: str,
    client_id: str,
    portal_base_url: str,
    ...
    *,
    ...
    """Refresh ValOs OAuth state without mutating auth.json."""
    state: Dict[str, Any] = {
        "access_token": access_token,
        "refresh_token": refresh_token,
        "client_id": client_id or DEFAULT_VALOS_CLIENT_ID,
        "portal_base_url": (portal_base_url or DEFAULT_VALOS_PORTAL_URL).rstrip("/"),
        "inference_base_url": (inference_base_url or DEFAULT_VALOS_INFERENCE_URL).rstrip("/"),
        "token_type": token_type or "Bearer",
        "scope": scope or DEFAULT_VALOS_SCOPE,
...
def refresh_valos_oauth_from_state(
    state: Dict[str, Any],
    *,
    min_key_ttl_seconds: int = DEFAULT_AGENT_KEY_MIN_TTL_SECONDS,
    timeout_seconds: float = 15.0,
    ...
    """Refresh ValOs OAuth from a state dict. Thin wrapper around refresh_valos_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_valos_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "valos-agent-cli"),
        state.get("portal_base_url", DEFAULT_VALOS_PORTAL_URL),
        state.get("inference_base_url", DEFAULT_VALOS_INFERENCE_URL),
        token_type=state.get("token_type", "Bearer"),
        scope=state.get("scope", DEFAULT_VALOS_SCOPE),
...
def resolve_valos_runtime_credentials(
    *,
    min_key_ttl_seconds: int = DEFAULT_AGENT_KEY_MIN_TTL_SECONDS,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ...
    """
    Resolve ValOs inference credentials for runtime use.
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("ValOs is not logged into ValOs Portal.",
                            provider="valos", relogin_required=True)
        ...
        inference_base_url = (
            ...
            or os.getenv("VALOS_INFERENCE_BASE_URL")
            or DEFAULT_VALOS_INFERENCE_URL
        ).rstrip("/")
        client_id = str(state.get("client_id") or DEFAULT_VALOS_CLIENT_ID)
        ...
        timeout = httpx.Timeout(timeout_seconds if timeout_seconds else 15.0)
        _oauth_trace(
            "valos_runtime_credentials_start",
            sequence_id=sequence_id,
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for ValOs Portal login.",
                                provider="valos", relogin_required=True)
        ...
        _persist_state("resolve_valos_runtime_credentials_final")

    api_key = state.get("agent_key")
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a ValOs inference API key",
                        provider="valos", code="server_error")
...
def get_valos_auth_status() -> Dict[str, Any]:
    """Status snapshot for `valos-agent status` output."""
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("VALOS_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("VALOS_COPILOT_ACP_ARGS", "").strip()
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set VALOS_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def detect_external_credentials() -> List[Dict[str, Any]]:
    """Scan for credentials from other CLI tools that ValOs can reuse.

    Returns a list of dicts, each with:
      - provider: str   -- ValOs provider id (e.g. "openai-codex")
    ...
    if cli_tokens:
        ...
        found.append({
            "provider": "openai-codex",
            "path": str(codex_path),
            "label": f"Codex CLI credentials found ({codex_path}) — run `valos-agent login` to create a separate session",
        })
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from valos_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'valos-agent model' or 'valos-agent setup' instead."""
    print("The 'valos-agent login' command has been removed.")
    print("Use 'valos-agent model' to select a provider and model,")
    print("or 'valos-agent setup' for full interactive setup.")
    raise SystemExit(0)


def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.valos-agent/auth.json."""

    # Check for existing ValOs-owned credentials
    try:
        existing = resolve_codex_runtime_credentials()
        print("Existing Codex credentials found in ValOs auth store.")
        try:
    ...
    if cli_tokens:
        ...
        print("ValOs will create its own session to avoid conflicts with Codex CLI / VS Code.")
        try:
        ...
        if do_import in ("y", "yes"):
            _save_codex_tokens(cli_tokens)
            base_url = os.getenv("VALOS_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
            config_path = _update_config_for_provider("openai-codex", base_url)
            print()
            print("Credentials imported. Note: if Codex CLI refreshes its token,")
            print("ValOs will keep working independently with its own session.")
            print(f"  Config updated: {config_path} (model.provider=openai-codex)")
            return

    # Run a fresh device code flow — ValOs gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(ValOs creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to ValOs auth store
...
def _valos_device_code_login(
    *,
    ...
    """Run the ValOs device-code flow and return full OAuth state without persisting."""
    pconfig = PROVIDER_REGISTRY["valos"]
    portal_base_url = (
        portal_base_url
        or os.getenv("VALOS_PORTAL_BASE_URL")
        or os.getenv("VALOS_PORTAL_BASE_URL")
    ...
    print(f"Starting ValOs login via {pconfig.name}...")
    ...
    return refresh_valos_oauth_from_state(
        auth_state,
        min_key_ttl_seconds=min_key_ttl_seconds,
        timeout_seconds=timeout_seconds,
        force_refresh=False,
...
def _login_valos(args, pconfig: ProviderConfig) -> None:
    """ValOs Portal device authorization flow."""
    ...
    try:
        auth_state = _valos_device_code_login(
        ...
        with _auth_store_lock():
            auth_store = _load_auth_store()
            _save_provider_state(auth_store, "valos", auth_state)
            saved_to = _save_auth_store(auth_store)

        config_path = _update_config_for_provider("valos", inference_base_url)
        print()
        print("Login successful!")
        print(f"  Auth state: {saved_to}")
        print(f"  Config updated: {config_path} (model.provider=valos)")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            print("ValOs will use OpenRouter for inference.")
        else:
            print("Run `valos-agent model` or configure an API key to use ValOs.")
    else:
