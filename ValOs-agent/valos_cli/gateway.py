"""
Gateway subcommand for valos-agent CLI.

Handles: valos-agent gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from valos_cli.config import get_env_value, get_valos_home, save_env_value, is_managed, managed_error
# display_valos_home is imported lazily at call sites to avoid ImportError
# when valos_constants is cached from a pre-update version during `valos-agent update`.
from valos_cli.setup import (
...
def find_gateway_pids() -> list:
    ...
    pids = []
    patterns = [
        "valos_cli.main gateway",
        "valos_cli/main.py gateway",
        "valos-agent gateway",
        "gateway/run.py",
    ]
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (VALOS_HOME-scoped).
...
_SERVICE_BASE = "valos-agent-gateway"
SERVICE_DESCRIPTION = "ValOs Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current VALOS_HOME.

    Returns ``""`` for the default ``~/.valos-agent``, the profile name for
    ``~/.valos-agent/profiles/<name>``, or a short hash for any other custom
    VALOS_HOME path.
    """
    import hashlib
    import re
    from pathlib import Path as _Path
    home = get_valos_home().resolve()
    default = (_Path.home() / ".valos-agent").resolve()
    if home == default:
        return ""
    # Detect ~/.valos-agent/profiles/<name> pattern → use the profile name
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary VALOS_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def get_service_name() -> str:
    """Derive a systemd service name scoped to this VALOS_HOME.

    Default ``~/.valos-agent`` returns ``valos-agent-gateway`` (backward compatible).
    Profile ``~/.valos-agent/profiles/coder`` returns ``valos-agent-gateway-coder``.
    Any other VALOS_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    valos-agent gateway uninstall")
    print_info("    sudo valos-agent gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so ValOs can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo valos-agent gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo valos-agent gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo valos-agent gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.valos-agent`` → ``ai.valos-agent.gateway.plist`` (backward compatible).
    Profile ``~/.valos-agent/profiles/coder`` → ``ai.valos-agent.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.valos-agent.gateway-{suffix}" if suffix else "ai.valos-agent.gateway"
...
def get_valos_cli_path() -> str:
    """Get the path to the valos-agent CLI."""
    ...
    return f"{get_python_path()} -m valos_cli.main"
...
def _valos_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current VALOS_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_valos_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.valos-agent                    → /home/alice/.valos-agent
      /root/.valos-agent/profiles/coder     → /home/alice/.valos-agent/profiles/coder
      /opt/custom-valos-agent               → /opt/custom-valos-agent  (kept as-is)
    """
    current_valos = get_valos_home().resolve()
    current_default = (Path.home() / ".valos-agent").resolve()
    target_default = Path(target_home_dir) / ".valos-agent"

    # Default ~/.valos-agent → remap to target user's default
    if current_valos == current_default:
        return str(target_default)

    # Profile or subdir of ~/.valos-agent → preserve the relative structure
    try:
        relative = current_valos.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.valos-agent) — keep as-is
        return str(current_valos)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        valos_home = _valos_home_for_target_user(home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m valos_cli.main gateway run --replace
...
Environment="VALOS_HOME={valos_home}"
Restart=on-failure
RestartSec=30
KillMode=mixed
KillSignal=SIGTERM
...
"""
    ...
    valos_home = str(get_valos_home().resolve())
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current ValOs install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}valos-agent gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}valos-agent gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}valos-agent gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}valos-agent gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}valos-agent gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.valos-agent.gateway-{suffix}" if suffix else "ai.valos-agent.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current ValOs install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  valos-agent gateway status             # Check status")
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current ValOs install")
    else:
        print("⚠ Service definition is stale relative to the current ValOs install")
        print("  Run: valos-agent gateway start")
    ...
    if deep:
        log_file = get_valos_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and ValOs will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @valos-agent:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. valos-agent) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where ValOs delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your ValOs agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address ValOs will use (e.g., valos-agent@gmail.com)."},
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_valos_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'valos-agent pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: valos-agent pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'valos-agent gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from valos_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"ValOsAgent\"")
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: valos-agent gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: valos-agent gateway")
    else:
        print()
        print_info("No platforms configured. Run 'valos-agent gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: valos-agent gateway run")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if is_linux():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print("  Then restart the gateway:")
                    print("    valos-agent gateway restart")
                    return
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: valos-agent gateway start")
                sys.exit(1)
        ...
        if is_linux() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                print("  valos-agent gateway install")
                print("  sudo valos-agent gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  valos-agent gateway          # Run in foreground")
                print("  valos-agent gateway install  # Install as user service")
                print("  sudo valos-agent gateway install --system  # Install as boot-time system service")
