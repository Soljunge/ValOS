"""
Configuration management for ValOs.

Config files are stored in ~/.valos-agent/ for easy access:
- ~/.valos-agent/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.valos-agent/.env         - API keys and secrets

This module provides:
- valos-agent config          - Show current configuration
- valos-agent config edit     - Open config in editor
- valos-agent config set      - Set a specific value
- valos-agent config wizard   - Re-run setup wizard
"""
...
from tools.tool_backend_helpers import managed_valos_tools_enabled as _managed_valos_tools_enabled
...
from valos_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("VALOS_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_valos_home() / ".managed"
...
def is_managed() -> bool:
    """Check if ValOs is running in package-manager-managed mode.

    Two signals: the VALOS_MANAGED env var (set by the systemd service),
    or a .managed marker file in VALOS_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade valos-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "valos-agent update"


def format_managed_message(action: str = "modify this ValOs installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("VALOS_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this ValOs installation is managed by NixOS "
            f"(VALOS_MANAGED={env_hint}).\n"
            "Edit services.valos-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this ValOs installation is managed by Homebrew "
            f"(VALOS_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade valos-agent"
        )

    return (
        f"Cannot {action}: this ValOs installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall ValOs."
    )
...
# Re-export from valos_constants — canonical definition lives there.
from valos_constants import get_valos_home  # noqa: F811,E402

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_valos_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_valos_home() / ".env"
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into VALOS_HOME if the user doesn't have one yet."""
...
def ensure_valos_home():
    """Ensure ~/.valos-agent directory structure exists with secure permissions."""
    home = get_valos_home()
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when ValOs
    ...
    "browser": {
        ...
        "camofox": {
            # When true, ValOs sends a stable profile-scoped userId to Camofox
    ...
    "auxiliary": {
        "vision": {
            "provider": "auto",    # auto | openrouter | valos | codex | custom
    ...
    # configured providers (OpenRouter, ValOs, Z.ai, Kimi, etc.) are supported.
    "delegation": {
    ...
    # always goes to ~/.valos-agent/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for valos-agent-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *ValOs*" header.
...
# selection step (ValOs Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "VALOS_BASE_URL": {
        "description": "ValOs Portal base URL override",
        "prompt": "ValOs Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for ValOs Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for ValOs Subscribers only, used to derive vendor hosts, e.g. valos-agent.com -> firecrawl-gateway.valos-agent.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for ValOs Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit ValOs Subscriber access token for tool-gateway requests (optional; otherwise read from the ValOs auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @valos-agent:example.org)",
    ...
    # VALOS_TOOL_PROGRESS and VALOS_TOOL_PROGRESS_MODE are deprecated —
...
if not _managed_valos_tools_enabled():
    for _hidden_var in (
        "FIRECRAWL_GATEWAY_URL",
        "TOOL_GATEWAY_DOMAIN",
        "TOOL_GATEWAY_SCHEME",
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("VALOS_TOOL_PROGRESS")
            old_mode = get_env_value("VALOS_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from VALOS_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from VALOS_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("VALOS_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from VALOS_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: valos-agent config set <key> <value>")
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.valos-agent/config.yaml."""
    import copy
    ensure_valos_home()
...
#   openai-codex (OAuth — valos-agent login) — OpenAI Codex
#   valos         (OAuth — valos-agent login) — ValOs Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.valos-agent/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.valos-agent/.env."""
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real ValOs env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.valos-agent/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.valos-agent/.env."""
    if is_managed():
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.valos-agent/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ ValOs Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  valos-agent config edit     # Edit config file", Colors.DIM))
    print(color("  valos-agent config set <key> <value>", Colors.DIM))
    print(color("  valos-agent setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: valos-agent config set <key> <value>")
            print()
            print("Examples:")
            print("  valos-agent config set model anthropic/claude-sonnet-4")
            print("  valos-agent config set terminal.backend docker")
            print("  valos-agent config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'valos-agent config migrate' to add them")
        ...
        print("  valos-agent config           Show current configuration")
        print("  valos-agent config edit      Open config in editor")
        print("  valos-agent config set <key> <value>   Set a config value")
        print("  valos-agent config check     Check for missing/outdated config")
        print("  valos-agent config migrate   Update config with new options")
        print("  valos-agent config path      Show config file path")
        print("  valos-agent config env-path  Show .env file path")
        sys.exit(1)
