"""Helpers for ValOs subscription managed-tool capabilities."""
...
from valos_cli.auth import get_valos_auth_status
from valos_cli.config import get_env_value, load_config
...
class ValOsFeatureState:
    key: str
    label: str
    included_by_default: bool
    available: bool
    active: bool
    managed_by_valos: bool
...
@dataclass(frozen=True)
class ValOsSubscriptionFeatures:
    subscribed: bool
    valos_auth_present: bool
    provider_is_valos: bool
    features: Dict[str, ValOsFeatureState]

    @property
    def web(self) -> ValOsFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> ValOsFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> ValOsFeatureState:
        return self.features["tts"]

    @property
    def browser(self) -> ValOsFeatureState:
        return self.features["browser"]

    @property
    def modal(self) -> ValOsFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[ValOsFeatureState]:
...
def get_valos_subscription_features(
    config: Optional[Dict[str, object]] = None,
) -> ValOsSubscriptionFeatures:
    if config is None:
        config = load_config() or {}
    config = dict(config)
    model_cfg = _model_config_dict(config)
    provider_is_valos = str(model_cfg.get("provider") or "").strip().lower() == "valos"

    try:
        valos_status = get_valos_auth_status()
    except Exception:
        valos_status = {}

    managed_tools_flag = managed_valos_tools_enabled()
    valos_auth_present = bool(valos_status.get("logged_in"))
    subscribed = provider_is_valos or valos_auth_present
    ...
    managed_web_available = managed_tools_flag and valos_auth_present and is_managed_tool_gateway_ready("firecrawl")
    managed_image_available = managed_tools_flag and valos_auth_present and is_managed_tool_gateway_ready("fal-queue")
    managed_tts_available = managed_tools_flag and valos_auth_present and is_managed_tool_gateway_ready("openai-audio")
    managed_browser_available = managed_tools_flag and valos_auth_present and is_managed_tool_gateway_ready("browserbase")
    managed_modal_available = managed_tools_flag and valos_auth_present and is_managed_tool_gateway_ready("modal")
    ...
    features = {
        "web": ValOsFeatureState(
            key="web",
            label="Web tools",
            included_by_default=True,
            available=web_available,
            active=web_active,
            managed_by_valos=web_managed,
        ...
        "image_gen": ValOsFeatureState(
            key="image_gen",
            label="Image generation",
            included_by_default=True,
            available=image_available,
            active=image_active,
            managed_by_valos=image_managed,
            direct_override=image_active and not image_managed,
            toolset_enabled=image_tool_enabled,
            current_provider="FAL" if direct_fal else ("ValOs Subscription" if image_managed else ""),
            explicit_configured=direct_fal,
        ),
        "tts": ValOsFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
            available=tts_available,
            active=tts_active,
            managed_by_valos=tts_managed,
        ...
        "browser": ValOsFeatureState(
            key="browser",
            label="Browser automation",
            included_by_default=True,
            available=browser_available,
            active=browser_active,
            managed_by_valos=browser_managed,
        ...
        "modal": ValOsFeatureState(
            key="modal",
            label="Modal execution",
            included_by_default=False,
            available=modal_available,
            active=modal_active,
            managed_by_valos=modal_managed,
    ...
    return ValOsSubscriptionFeatures(
        subscribed=subscribed,
        valos_auth_present=valos_auth_present,
        provider_is_valos=provider_is_valos,
        features=features,
    )


def get_valos_subscription_explainer_lines() -> list[str]:
    if not managed_valos_tools_enabled():
        return []

    return [
        "ValOs subscription enables managed web tools, image generation, OpenAI TTS, and browser automation by default.",
        "Those managed tools bill to your ValOs subscription. Modal execution is optional and can bill to your subscription too.",
        "Change these later with: valos-agent setup tools, valos-agent setup terminal, or valos-agent status.",
    ]


def apply_valos_provider_defaults(config: Dict[str, object]) -> set[str]:
    """Apply provider-level ValOs defaults shared by `valos-agent setup` and `valos-agent model`."""
    if not managed_valos_tools_enabled():
        return set()

    features = get_valos_subscription_features(config)
    if not features.provider_is_valos:
        return set()
...
def apply_valos_managed_defaults(
