"""valos-agent webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.valos-agent/webhook_subscriptions.json and are
...
def _valos_home() -> Path:
    return Path(
        os.getenv("VALOS_HOME", str(Path.home() / ".valos-agent"))
    ).expanduser()


def _subscriptions_path() -> Path:
    return _valos_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: valos-agent gateway run
"""
...
def webhook_command(args):
    """Entry point for 'valos-agent webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: valos-agent webhook {subscribe|list|remove|test}")
        print("Run 'valos-agent webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (valos-agent gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: valos-agent webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from valos-agent webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (valos-agent gateway run)")
