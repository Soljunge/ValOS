...
Add, remove, or reorder entries here — both `valos-agent setup` and
`valos-agent` provider-selection will pick up the change automatically.
"""
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    _PROVIDER_ORDER = [
        "openrouter", "openai", "valos", "openai-codex", "copilot", "copilot-acp",
    ...
    for pid in _PROVIDER_ORDER:
        ...
        has_creds = False
        try:
            from valos_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        valos:valos-agent-3                           →  ("valos", "valos-agent-3")
...
def curated_models_for_provider(provider: Optional[str]) -> list[tuple[str, str]]:
    ...
    # Try live API first (Codex, ValOs, etc. all support /models)
...
def detect_provider_for_model(
    ...
    openai_name = normalize_openai_model_id(name).lower()

    # --- Step 0: bare provider name typed as model ---
    # If someone types `/model valos` or `/model anthropic`, treat it as a
    ...
    _AGGREGATORS = {"valos", "openrouter"}

    # If the model belongs to the current provider's catalog, don't suggest switching
    current_models = _PROVIDER_MODELS.get(current_provider, [])
    if current_provider == "openai":
        if any(openai_name == m.lower() for m in current_models):
            return None
    elif any(name_lower == m.lower() for m in current_models):
        return None
    ...
    for pid, models in _PROVIDER_MODELS.items():
        if pid == current_provider or pid in _AGGREGATORS:
            continue
        if pid == "openai":
            if any(openai_name == m.lower() for m in models):
                direct_match = pid
                break
            continue
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to ValOs' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``valos_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from valos_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str]) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, ValOs),
    ...
    if normalized == "openai":
        try:
            ...
            creds = resolve_api_key_provider_credentials("openai")
            live = fetch_api_models(creds.get("api_key", ""), creds.get("base_url", ""))
            if live:
                return live
        except Exception:
            pass
        return list(_PROVIDER_MODELS.get("openai", []))
    ...
    if normalized == "valos":
        # Try live ValOs Portal /models endpoint
        try:
            from valos_cli.auth import fetch_valos_models, resolve_valos_runtime_credentials
            creds = resolve_valos_runtime_credentials()
            if creds:
                live = fetch_valos_models(api_key=creds.get("api_key", ""), inference_base_url=creds.get("base_url", ""))
                if live:
                    return live
        except Exception:
            pass
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from valos_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "ValOsAgent/1.0",
...
def normalize_openai_model_id(model_id: Optional[str]) -> str:
    """Normalize OpenAI model IDs to the bare slug used by the API."""
    current = str(model_id or "").strip()
    if not current:
        return current
    if current.lower().startswith("openai/"):
        return current.split("/", 1)[1].strip()
    return current


def openai_model_api_mode(model_id: Optional[str]) -> str:
    """Return the API mode for a direct OpenAI model.

    GPT-5 and O-series models require the Responses API; older chat models
    use Chat Completions.
    """
    raw = normalize_openai_model_id(model_id).lower()
    if not raw:
        return "chat_completions"
    if raw.startswith(("gpt-5", "o1", "o3", "o4")):
        return "codex_responses"
    return "chat_completions"
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    if not base_url:
        from valos_constants import AI_GATEWAY_BASE_URL
...
def validate_requested_model(
    ...
    *,
    ...
    if normalized == "copilot":
        ...
        requested_for_lookup = normalize_openai_model_id(requested)
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"ValOs will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
