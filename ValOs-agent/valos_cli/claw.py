"""valos-agent claw — OpenClaw migration commands.

Usage:
    valos-agent claw migrate              # Interactive migration from ~/.openclaw
    valos-agent claw migrate --dry-run    # Preview what would be migrated
    valos-agent claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    valos-agent claw cleanup              # Archive leftover OpenClaw directories
    valos-agent claw cleanup --dry-run    # Preview what would be archived
"""
...
from valos_cli.config import get_valos_home, get_config_path, load_config, save_config
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_valos.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_valos", script_path)
...
def claw_command(args):
    """Route valos-agent claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: valos-agent claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to ValOs")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'valos-agent claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → ValOs migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: valos-agent claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {valos_home}")
...
def _offer_source_archival(source_dir: Path, auto_yes: bool = False):
    ...
    if auto_yes or prompt_yes_no(f"Archive {source_dir} now?", default=True):
        ...
        print_info("Skipped. You can archive later with: valos-agent claw cleanup")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  valos-agent claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  valos-agent claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  valos-agent config set OPENROUTER_API_KEY sk-or-v1-...")
