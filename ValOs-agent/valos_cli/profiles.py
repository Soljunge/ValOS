"""
Profile management for multiple isolated ValOs instances.

Each profile is a fully independent VALOS_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.valos-agent/profiles/<name>/`` by default.

The "default" profile is ``~/.valos-agent`` itself — backward compatible,
zero migration needed.

Usage::

    valos-agent profile create coder          # fresh profile + bundled skills
    valos-agent profile create coder --clone  # also copy config, .env, SOUL.md
    valos-agent profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    valos-agent -p coder chat                 # or via flag
    valos-agent profile use coder             # set as sticky default
    valos-agent profile delete coder          # remove profile + alias + service
"""
...
# Directories/files to exclude when exporting the default (~/.valos-agent) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "valos-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "valos-agent", "default", "test", "tmp", "root", "sudo",
})

# ValOs subcommands that cannot be used as profile names/aliases
_VALOS_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Always ``~/.valos-agent/profiles/`` — anchored to the user's home,
    NOT to the current VALOS_HOME (which may itself be a profile).
    This ensures ``coder profile list`` can see all profiles.
    """
    return Path.home() / ".valos-agent" / "profiles"


def _get_default_valos_home() -> Path:
    """Return the default (pre-profile) VALOS_HOME path."""
    return Path.home() / ".valos-agent"


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_valos_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.valos-agent
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its VALOS_HOME directory."""
    if name == "default":
        return _get_default_valos_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, valos-agent subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _VALOS_SUBCOMMANDS:
        return f"'{name}' conflicts with a valos-agent subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_valos_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.valos-agent)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches VALOS_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "VALOS_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.valos-agent).\n"
            "To remove everything, use: valos-agent uninstall"
        )
    ...
    # Check for service
    from valos_cli.gateway import _profile_suffix, get_service_name
    wrapper_path = _get_wrapper_dir() / name
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set VALOS_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("VALOS_HOME")
    try:
        os.environ["VALOS_HOME"] = str(profile_dir)
        from valos_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["VALOS_HOME"] = old_home
        elif "VALOS_HOME" in os.environ:
            del os.environ["VALOS_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.valos-agent/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: valos-agent profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from VALOS_HOME.

    Returns ``"default"`` if VALOS_HOME is not set or points to ``~/.valos-agent``.
    Returns the profile name if VALOS_HOME points into ``~/.valos-agent/profiles/<name>``.
    Returns ``"custom"`` if VALOS_HOME is set to an unrecognized path.
    """
    from valos_constants import get_valos_home
    valos_home = get_valos_home()
    resolved = valos_home.resolve()

    default_resolved = _get_default_valos_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.valos-agent itself — its parent is ~/ and its
        # directory name is ".valos-agent", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: valos-agent profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.valos-agent itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.valos-agent). "
            "Specify a different name: valos-agent profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for valos-agent profile names."""
    return '''# ValOs profile completion
# Add to ~/.bashrc: eval "$(valos-agent completion bash)"

_valos_profiles() {
    local profiles_dir="$HOME/.valos-agent/profiles"
...
_valos_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_valos_profiles)" -- "$cur"))
        return
    fi
...
complete -F _valos_completion valos-agent
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for valos-agent profile names."""
    return '''#compdef valos-agent
# ValOs profile completion
# Add to ~/.zshrc: eval "$(valos-agent completion zsh)"

_valos() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.valos-agent/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.valos-agent/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a VALOS_HOME path string.

    Called early in the CLI entry point, before any valos-agent modules
    are imported, to set the VALOS_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: valos-agent profile create {profile_name}"
        )
