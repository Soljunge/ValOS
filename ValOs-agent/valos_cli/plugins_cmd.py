"""``valos-agent plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.valos-agent/plugins/``.
...
def _plugins_dir() -> Path:
    ...
    valos_home = os.environ.get("VALOS_HOME", os.path.expanduser("~/.valos-agent"))
    plugins = Path(valos_home) / "plugins"
...
def cmd_install(identifier: str, force: bool = False) -> None:
        ...
        if target.exists():
            if not force:
                console.print(
                    ...
                    f"[bold]valos-agent plugins update {plugin_name}[/bold] to pull latest."
                )
                sys.exit(1)
    ...
    if not (target / "plugin.yaml").exists() and not (target / "__init__.py").exists():
        console.print(
            ...
            f"or __init__.py. It may not be a valid ValOs plugin."
        )
    ...
    console.print("[dim]  valos-agent gateway restart[/dim]")
    console.print()
...
def cmd_list() -> None:
    ...
    if not dirs:
        ...
        console.print("[dim]Install with:[/dim] valos-agent plugins install owner/repo")
        return
    ...
    for d in dirs:
        ...
        # Check if it's a git repo (installed via valos-agent plugins install)
        if (d / ".git").exists():
            source = "git"
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] valos-agent plugins")
    console.print("[dim]Enable/disable:[/dim] valos-agent plugins enable/disable <name>")
...
def plugins_command(args) -> None:
    """Dispatch valos-agent plugins subcommands."""
