...
Official optional skills live in the repository under `optional-skills/`. Install them with `valos-agent skills install official/<category>/<skill>` or browse them with `valos-agent skills browse --source official`.
...
| Skill | Description | Path |
|-------|-------------|------|
| `blender-mcp` | Control Blender directly from ValOs via socket connection to the blender-mcp addon. Create 3D objects, materials, animations, and run arbitrary Blender Python. | `creative/blender-mcp` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `openclaw-migration` | Migrate a user's OpenClaw customization footprint into ValOs. Imports ValOs-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why. | `migration/openclaw-migration` |

## productivity

| Skill | Description | Path |
|-------|-------------|------|
| `telephony` | Give ValOs phone capabilities — provision a Twilio number, send/receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi. | `productivity/telephony` |
