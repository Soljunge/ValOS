...
ValOs has two slash-command surfaces, both driven by a central `COMMAND_REGISTRY` in `valos_cli/commands.py`:
...
Installed skills are also exposed as dynamic slash commands on both surfaces. That includes bundled skills like `/plan`, which opens plan mode and saves markdown plans under `.valos-agent/plans/` relative to the active workspace/backend working directory.
...
| `/plan [request]` | Load the bundled `plan` skill to write a markdown plan instead of executing the work. Plans are saved under `.valos-agent/plans/` relative to the active workspace/backend working directory. |
...
User-defined quick commands from `quick_commands` in `~/.valos-agent/config.yaml` are also available as slash commands. These are resolved at dispatch time, not shown in the built-in autocomplete/help tables.

## Messaging slash commands

The messaging gateway supports the following built-in commands inside supported gateway chats, including Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, Feishu/Lark, WeCom, SMS, Email, webhooks, Home Assistant, and API Server clients:
