...
description: "Frequently asked questions and solutions to common issues with ValOs"
---
...
### What LLM providers work with ValOs?

ValOs works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **ValOs Portal** — ValOs's own inference endpoint
...
Set your provider with `valos-agent model` or by editing `~/.valos-agent/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** ValOs requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run ValOs from inside it. The standard install command works perfectly in WSL2:
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). ValOs does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.valos-agent/`.

### Can I use it offline / with local models?

Yes. Run `valos-agent model`, select **Custom endpoint**, and enter your server's URL:
...
ValOs persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in ValOs — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

### How much does it cost?

ValOs itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same ValOs instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use ValOs programmatically:

```python
from valos-agent.agent import AIAgent

agent = AIAgent(model="openrouter/valos/valos-agent-3-llama-3.1-70b")
...
#### `valos-agent: command not found` after installation
...
**Cause:** ValOs requires Python 3.11 or newer.
...
sudo rm /usr/local/bin/valos-agent
...
valos-agent config show

# Re-configure your provider
valos-agent model

# Or set directly
valos-agent config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.valos-agent/.env` for conflicting entries.
:::
...
# Set a valid model
valos-agent config set VALOS_MODEL openrouter/valos/valos-agent-3-llama-3.1-70b

# Or specify per-session
valos-agent chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `valos-agent chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or ValOs detected the wrong context length for your model.
...
valos-agent chat --model openrouter/google/gemini-2.0-flash-001
```

If this happens on the first long conversation, ValOs may have the wrong context length for your model. Check what it detected:
...
**Cause:** ValOs detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — ValOs never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `valos-agent chat`
...
valos-agent gateway status

# Start the gateway
valos-agent gateway start

# Check logs for errors
cat ~/.valos-agent/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `valos-agent gateway setup`
- Check gateway logs: `cat ~/.valos-agent/logs/gateway.log | tail -50`
...
Configure in `~/.valos-agent/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install "valos-agent[telegram]"   # or [discord], [slack], [whatsapp]
...
**Solution:** The gateway captures your shell PATH when you run `valos-agent gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
valos-agent gateway install    # Re-snapshots your current PATH
valos-agent gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.valos-agent.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `valos-agent chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `valos-agent chat -t "terminal"`
...
cd ~/.valos-agent/valos-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.valos-agent/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
valos-agent config show | grep -A 12 mcp_servers

# Restart ValOs or reload MCP after config changes
valos-agent chat
```

See also:
- [MCP (Model Context Protocol)](/docs/user-guide/features/mcp)
- [Use MCP with ValOs](/docs/guides/use-mcp-with-valos-agent)
...
:::warning
If an MCP server crashes mid-request, ValOs will report a timeout. Check the server's own logs (not just ValOs logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `valos-agent profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `valos-agent update`?

`valos-agent update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `valos-agent update` once — it covers every profile on the machine.
...
valos-agent profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
valos-agent profile import ./work-backup.tar.gz work
```
...
There is no hard limit. Each profile is just a directory under `~/.valos-agent/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** ValOs can route subagents to a different model automatically. Set this in `~/.valos-agent/config.yaml`:
...
Now when you tell ValOs "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can ValOs do this?

**Current limitation:** ValOs profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, ValOs reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `valos-agent skills config` settings don't seem to take effect.

**Solution:** Use `valos-agent skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`valos-agent gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** ValOs creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting ValOs to another machine
...
1. Install ValOs on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/horatiu.sol/valos-agent/main/scripts/install.sh | bash
   ```

2. Copy your entire `~/.valos-agent/` directory **except** the `valos-agent` subdirectory (that's the code repo — the new install has its own):
   ```bash
   # On the source machine
   rsync -av --exclude='valos-agent' ~/.valos-agent/ newmachine:~/.valos-agent/
   ```

   Or use profile export/import:
   ```bash
   # On source machine
   valos-agent profile export default ./valos-agent-backup.tar.gz

   # On target machine
   valos-agent profile import ./valos-agent-backup.tar.gz default
   ```

3. On the new machine, run `valos-agent setup` to verify API keys and provider config are working. Re-authenticate any messaging platforms (especially WhatsApp, which uses QR pairing).

The `~/.valos-agent/` directory contains everything: `config.yaml`, `.env`, `SOUL.md`, `memories/`, `skills/`, `state.db` (sessions), `cron/`, and any custom plugins. The code itself lives in `~/.valos-agent/valos-agent/` and is installed fresh.

### Permission denied when reloading shell after install

**Scenario:** After running the ValOs installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a ValOs-specific issue — it's a shell config permissions problem.
...
valos-agent config show | head -20

# Re-run model selection
valos-agent model

# Or test with a known-good model
valos-agent chat -q "hello" --model anthropic/claude-sonnet-4.6
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/horatiu.sol/valos-agent/issues)
2. **Ask the community:** [ValOs Discord](https://discord.gg/valos-agent)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), ValOs version (`valos-agent --version`), and the full error message
