...
sidebar_position: 2
title: "Installation"
description: "Install ValOs on Linux, macOS, or WSL2"
---

# Installation

Get ValOs up and running in under two minutes with the one-line installer, or follow the manual steps for full control.
...
:::warning Windows
Native Windows is **not supported**. Please install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run ValOs from there. The install command above works inside WSL2.
:::

### What the Installer Does

The installer handles everything automatically — all dependencies (Python, Node.js, ripgrep, ffmpeg), the repo clone, virtual environment, global `valos-agent` command setup, and LLM provider configuration. By the end, you're ready to chat.
...
valos-agent gateway setup  # Set up messaging platforms
valos-agent config set     # Set individual config values
valos-agent setup          # Or run the full setup wizard to configure everything at once
```
...
:::tip
You do **not** need to activate the venv to use `valos-agent`. The entry point has a hardcoded shebang pointing to the venv Python, so it works globally once symlinked.
:::
...
mkdir -p ~/.valos-agent/{cron,sessions,logs,memories,skills,pairing,hooks,image_cache,audio_cache,whatsapp/session}
...
Open `~/.valos-agent/.env` and add at minimum an LLM provider key:
...
```bash
valos-agent config set OPENROUTER_API_KEY sk-or-v1-your-key-here
```

### Step 8: Add `valos-agent` to Your PATH
...
```bash
valos-agent model       # Select your LLM provider and model
```

### Step 10: Verify the Installation

```bash
valos-agent version    # Check that the command is available
valos-agent doctor     # Run diagnostics to verify everything is working
valos-agent status     # Check your configuration
valos-agent chat -q "Hello! What tools do you have available?"
```
...
# Make valos-agent available globally
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/valos-agent" ~/.local/bin/valos-agent
...
| Problem | Solution |
|---------|----------|
| `valos-agent: command not found` | Reload your shell (`source ~/.bashrc`) or check PATH |
| `API key not set` | Run `valos-agent model` to configure your provider, or `valos-agent config set OPENROUTER_API_KEY your_key` |
| Missing config after update | Run `valos-agent config check` then `valos-agent config migrate` |

For more diagnostics, run `valos-agent doctor` — it will tell you exactly what's missing and how to fix it.
