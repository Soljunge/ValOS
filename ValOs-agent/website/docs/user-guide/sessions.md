...
ValOs automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.valos-agent/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.valos-agent/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`valos-agent` or `valos-agent chat`) |
...
valos-agent -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
valos-agent -c "my project"   # → resumes "my project #3"
```
...
valos-agent --resume 20250305_091523_a1b2c3d4
valos-agent -r 20250305_091523_a1b2c3d4

# Resume by title
valos-agent --resume "refactoring auth"

# Or with the chat subcommand
valos-agent chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `valos-agent sessions list`.

### Conversation Recap on Resume

When you resume a session, ValOs displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a ValOs session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.valos-agent/config.yaml`:
...
ValOs automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `valos-agent sessions list` or `valos-agent sessions browse`.
...
```bash
valos-agent sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), ValOs creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`valos-agent -c "my project"`), it automatically picks the most recent session in the lineage.
...
ValOs provides a full set of session management commands via `valos-agent sessions`:
...
# Filter by platform
valos-agent sessions list --source telegram

# Show more sessions
valos-agent sessions list --limit 50
```
...
valos-agent sessions export backup.jsonl

# Export sessions from a specific platform
valos-agent sessions export telegram-history.jsonl --source telegram

# Export a single session
valos-agent sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
valos-agent sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
valos-agent sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
valos-agent sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
valos-agent sessions prune

# Custom age threshold
valos-agent sessions prune --older-than 30

# Only prune sessions from a specific platform
valos-agent sessions prune --source telegram --older-than 60

# Skip confirmation
valos-agent sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`valos-agent insights`](/docs/reference/cli-commands#valos-agent-insights).
...
When ValOs cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, ValOs uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to ValOs in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.valos-agent/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.valos-agent/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.valos-agent/sessions/sessions.json` | Maps session keys to active session IDs |
...
valos-agent sessions delete <session_id>
