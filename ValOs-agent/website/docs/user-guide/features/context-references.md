...
Type `@` followed by a reference to inject content directly into your message. ValOs expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- ValOs env: `$VALOS_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$VALOS_HOME/skills/.hub/`
