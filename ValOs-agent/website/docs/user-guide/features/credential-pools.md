...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, ValOs automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, ValOs auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
valos-agent auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
valos-agent auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (Claude Code subscription)
valos-agent auth add anthropic --type oauth
...
Run `valos-agent auth` with no subcommand for an interactive wizard:
...
For providers that support both API keys and OAuth (Anthropic, ValOs, Codex), the add flow asks which type:
...
| Command | Description |
|---------|-------------|
| `valos-agent auth` | Interactive pool management wizard |
| `valos-agent auth list` | Show all pools and credentials |
| `valos-agent auth list <provider>` | Show a specific provider's pool |
| `valos-agent auth add <provider>` | Add a credential (prompts for type and key) |
| `valos-agent auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `valos-agent auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `valos-agent auth remove <provider> <index>` | Remove credential by 1-based index |
| `valos-agent auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `valos-agent auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `valos-agent model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via valos-agent model:
valos-agent auth list
# Shows:
...
valos-agent auth add Together.ai --api-key sk-together-second-key
```
...
ValOs automatically discovers credentials from multiple sources and seeds the pool on startup:

| Source | Example | Auto-seeded? |
|--------|---------|-------------|
| Environment variables | `OPENROUTER_API_KEY`, `ANTHROPIC_API_KEY` | Yes |
| OAuth tokens (auth.json) | Codex device code, ValOs device code | Yes |
| Claude Code credentials | `~/.claude/.credentials.json` | Yes (Anthropic) |
| ValOs PKCE OAuth | `~/.valos-agent/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `valos-agent auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `valos-agent auth add`) are never auto-pruned.
...
2. **`valos_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`valos_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.valos-agent/auth.json` under the `credential_pool` key:
