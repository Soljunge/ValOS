...
ValOs has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — ValOs can automatically switch to a backup provider:model pair mid-session without losing your conversation.

### Configuration

Add a `fallback_model` section to `~/.valos-agent/config.yaml`:
...
| ValOs Portal | `valos` | `valos-agent login` (OAuth) |
| OpenAI Codex | `openai-codex` | `valos-agent model` (ChatGPT OAuth) |
...
**ValOs Portal as fallback for OpenRouter:**
```yaml
model:
  provider: openrouter
...
ValOs uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
When a task's provider is set to `"auto"` (the default), ValOs tries providers in order until one works:

**For text tasks (compression, web extract, etc.):**

```text
OpenRouter → ValOs Portal → Custom endpoint → Codex OAuth →
...
```text
Main provider (if vision-capable) → OpenRouter → ValOs Portal →
Codex OAuth → Anthropic → Custom endpoint → give up
```

If the resolved provider fails at call time, ValOs also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
auxiliary:
  vision:
    provider: "auto"              # auto | openrouter | valos | codex | main | anthropic
...
| `"codex"` | Force Codex OAuth | `valos-agent model` → Codex |
...
`base_url` takes precedence over `provider`. ValOs uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.
...
```yaml
compression:
  summary_provider: "auto"                    # auto | openrouter | valos | main
  summary_model: "google/gemini-3-flash-preview"
```
...
If no provider is available for compression, ValOs drops middle conversation turns without generating a summary rather than failing the session.
