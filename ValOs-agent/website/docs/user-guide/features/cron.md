...
Schedule tasks to run automatically with natural language or cron expressions. ValOs exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. ValOs disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
valos-agent cron create "every 2h" "Check server status"
valos-agent cron create "every 1h" "Summarize new feed items" --skill blogwatcher
valos-agent cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill find-nearby \
  --name "Skill combo"
```
...
ValOs will use the unified `cronjob` tool internally.
...
```bash
valos-agent cron edit <job_id> --schedule "every 4h"
valos-agent cron edit <job_id> --prompt "Use the revised task"
valos-agent cron edit <job_id> --skill blogwatcher --skill find-nearby
valos-agent cron edit <job_id> --add-skill find-nearby
valos-agent cron edit <job_id> --remove-skill blogwatcher
valos-agent cron edit <job_id> --clear-skills
```
...
```bash
valos-agent cron list
valos-agent cron pause <job_id>
valos-agent cron resume <job_id>
valos-agent cron run <job_id>
valos-agent cron remove <job_id>
valos-agent cron status
valos-agent cron tick
```
...
sudo valos-agent gateway install --system   # Linux: boot-time system service for servers
valos-agent gateway             # Or run in foreground
...
1. loads jobs from `~/.valos-agent/cron/jobs.json`
...
A file lock at `~/.valos-agent/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.valos-agent/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.valos-agent/cron/output/`), but no message is sent to the delivery target.
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, ValOs skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Jobs are stored in `~/.valos-agent/cron/jobs.json`. Output from job runs is saved to `~/.valos-agent/cron/output/{job_id}/{timestamp}.md`.
