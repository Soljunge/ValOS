...
sidebar_position: 14
title: "API Server"
description: "Expose valos-agent as an OpenAI-compatible API for any frontend"
---

# API Server

The API server exposes valos-agent as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox, and hundreds more — can connect to valos-agent and use it as a backend.
...
# Optional: only if a browser must call ValOs directly
...
curl http://localhost:8642/v1/chat/completions \
  ...
  -d '{"model": "valos-agent", "messages": [{"role": "user", "content": "Hello!"}]}'
```
...
Lists `valos-agent` as an available model. Required by most frontends for model discovery.
...
When a frontend sends a `system` message (Chat Completions) or `instructions` field (Responses API), valos-agent **layers it on top** of its core system prompt. Your agent keeps all its tools, memory, and skills — the frontend's system prompt adds extra instructions.
...
Configure the key via `API_SERVER_KEY` env var. If you need a browser to call ValOs directly, also set `API_SERVER_CORS_ORIGINS` to an explicit allowlist.

:::warning Security
The API server gives full access to valos-agent's toolset, **including terminal commands**. If you change the bind address to `0.0.0.0` (network-accessible), **always set `API_SERVER_KEY`** and keep `API_SERVER_CORS_ORIGINS` narrow — without that, remote callers may be able to execute arbitrary commands on your machine.
