...
description: "Overview of ValOs's tools — what's available, how toolsets work, and terminal backends"
---
...
ValOs ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, Home Assistant, and more.
...
valos-agent chat --toolsets "web,terminal"
...
See [Toolsets Reference](/docs/reference/toolsets-reference) for the full set, including platform presets such as `valos-agent-cli`, `valos-agent-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
```
```bash
# Set credentials in ~/.valos-agent/.env
...
# Configure
valos-agent config set terminal.backend singularity
valos-agent config set terminal.singularity_image ~/python.sif
```
...
modal setup
valos-agent config set terminal.backend modal
```
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.valos-agent/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.valos-agent/.env`.
:::
