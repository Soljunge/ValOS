...
description: "Customize ValOs's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

ValOs's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `VALOS_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who ValOs is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

ValOs now seeds a default `SOUL.md` automatically in:

```text
~/.valos-agent/SOUL.md
```

More precisely, it uses the current instance's `VALOS_HOME`, so if you run ValOs with a custom home directory, it will use:
...
- ValOs creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- ValOs loads `SOUL.md` only from `VALOS_HOME`
- ValOs does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, ValOs falls back to a built-in default identity
...
If ValOs loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `VALOS_HOME`, the personality belongs to the ValOs instance itself.

That also makes it easier to teach users:
- "Edit `~/.valos-agent/SOUL.md` to change ValOs' default personality."
...
- how ValOs should handle uncertainty, disagreement, or ambiguity
...
## What ValOs injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, ValOs falls back to a built-in default identity ("You are ValOs, an intelligent AI assistant created by ValOs..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
ValOs ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain ValOs, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives ValOs its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.valos-agent/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.valos-agent/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-valos-agent)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how ValOs speaks
- `display.skin` and `/skin` affect how ValOs looks in the terminal
