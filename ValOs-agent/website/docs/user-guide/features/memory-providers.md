...
ValOs ships with 7 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
valos-agent memory setup      # interactive picker + configuration
valos-agent memory status     # check what's active
valos-agent memory off        # disable external provider
```

Or set manually in `~/.valos-agent/config.yaml`:
...
When a memory provider is active, ValOs automatically:
...
# Or manually:
valos-agent config set memory.provider honcho
echo "HONCHO_API_KEY=your-key" >> ~/.valos-agent/.env
```

**Config:** `$VALOS_HOME/honcho.json` — existing Honcho users' configuration and data are fully preserved.

:::tip Migrating from `valos-agent honcho`
If you previously used `valos-agent honcho setup`, your config and all server-side data are intact. Just set `memory.provider: honcho` to reactivate via the new system.
:::
...
valos-agent memory setup    # select "openviking"
# Or manually:
valos-agent config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.valos-agent/.env
```
...
**Setup:**
```bash
valos-agent memory setup    # select "mem0"
# Or manually:
valos-agent config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.valos-agent/.env
```

**Config:** `$VALOS_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `valos-agent-user` | User identifier |
| `agent_id` | `valos-agent` | Agent identifier |
...
**Setup:**
```bash
valos-agent memory setup    # select "hindsight"
# Or manually:
valos-agent config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.valos-agent/.env
```

**Config:** `$VALOS_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `valos-agent` | Memory bank identifier |
...
**Setup:**
```bash
valos-agent memory setup    # select "holographic"
# Or manually:
valos-agent config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.valos-agent-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$VALOS_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
valos-agent memory setup    # select "retaindb"
# Or manually:
valos-agent config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.valos-agent/.env
```
...
# Then configure ValOs
valos-agent memory setup    # select "byterover"
# Or manually:
valos-agent config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$VALOS_HOME/byterover/` (profile-scoped)
...
- **Local storage providers** (Holographic, ByteRover) use `$VALOS_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight) store config in `$VALOS_HOME/` so each profile has its own credentials
