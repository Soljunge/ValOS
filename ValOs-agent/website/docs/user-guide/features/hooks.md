...
ValOs has two hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.valos-agent/hooks/` | Gateway only | Logging, alerts, webhooks |
...
Each hook is a directory under `~/.valos-agent/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".valos-agent" / "hooks" / "my-hook" / "activity.log"
...
The gateway ships with a built-in `boot-md` hook that looks for `~/.valos-agent/BOOT.md` on every startup. If the file exists, the agent runs its instructions in a background session. No installation needed — just create the file.

**Create `~/.valos-agent/BOOT.md`:**

```markdown
# Startup Checklist

1. Check if any cron jobs failed overnight — run `valos-agent cron list`
...
```yaml
# ~/.valos-agent/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.valos-agent/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.valos-agent/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.valos-agent/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".valos-agent" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.valos-agent/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.valos-agent/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/valos-agent-events"
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.valos-agent/hooks/`
...
```python
# ~/.valos-agent/plugins/tool-guard/__init__.py
