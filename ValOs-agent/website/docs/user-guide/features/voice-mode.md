...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with ValOs — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

ValOs supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with ValOs](/docs/guides/use-voice-mode-with-valos-agent).

## Prerequisites

Before using voice features, make sure you have:

1. **ValOs installed** — `pip install valos-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `valos-agent model` or set your preferred provider credentials in `~/.valos-agent/.env`
3. **A working base setup** — run `valos-agent` to verify the agent responds to text before enabling voice

:::tip
The `~/.valos-agent/` directory and default `config.yaml` are created automatically the first time you run `valos-agent`. You only need to create `~/.valos-agent/.env` manually for API keys.
:::
...
pip install "valos-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "valos-agent[all]"
```
...
```bash
valos-agent                # Start the interactive CLI
```
...
1. Start the CLI with `valos-agent` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.valos-agent/config.yaml` (default: `ctrl+b`).
:::
...
```bash
valos-agent gateway        # Start the gateway (connects to configured platforms)
valos-agent gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@valosbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.valos-agent/.env`:
```bash
...
```bash
valos-agent gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.valos-agent/.env`
...
- Review gateway logs: `tail -f ~/.valos-agent/logs/gateway.log`
