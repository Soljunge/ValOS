...
sidebar_position: 2
title: "Configuration"
description: "Configure ValOs — config.yaml, providers, models, API keys, and more"
---

# Configuration

All settings are stored in the `~/.valos-agent/` directory for easy access.
...
├── auth.json       # OAuth provider credentials (ValOs Portal, etc.)
...
```bash
valos-agent config              # View current configuration
valos-agent config edit         # Open config.yaml in your editor
valos-agent config set KEY VAL  # Set a specific value
valos-agent config check        # Check for missing options (after updates)
valos-agent config migrate      # Interactively add missing options

# Examples:
valos-agent config set model anthropic/claude-opus-4
valos-agent config set terminal.backend docker
valos-agent config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `valos-agent config set` command automatically routes values to the right file — API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** — e.g., `valos-agent chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.valos-agent/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.valos-agent/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
ValOs supports six terminal backends. Each determines where the agent's shell commands actually execute — your local machine, a Docker container, a remote server via SSH, a Modal cloud sandbox, a Daytona workspace, or a Singularity/Apptainer container.
...
For cloud sandboxes such as Modal and Daytona, `container_persistent: true` means ValOs will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
:::warning
The agent has the same filesystem access as your user account. Use `valos-agent tools` to disable tools you don't want, or switch to Docker for sandboxing.
:::
...
**Requirements:** Docker Desktop or Docker Engine installed and running. ValOs probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle).
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.valos-agent/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.valos-agent/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.valos-agent/` (OAuth tokens, etc.) and synced before each command.
...
**Persistence:** When enabled, sandboxes are stopped (not deleted) on cleanup and resumed on next session. Sandbox names follow the pattern `valos-agent-{task_id}`.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/valos-agent` (HPC convention) → `~/.valos-agent/sandboxes/singularity`.
...
- **Docker** — Run `docker version` to verify Docker is working. If it fails, fix Docker or `valos-agent config set terminal.backend local`.
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. ValOs logs a clear error if either is missing.
- **Modal** — Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `valos-agent doctor` to check.
...
ValOs resolves each listed variable from your current shell first, then falls back to `~/.valos-agent/.env` if it was saved with `valos-agent config set`.
...
Docker sandboxes stay isolated by default. ValOs does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch ValOs from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched ValOs from
...
```bash
valos-agent config set terminal.persistent_shell false
```
...
```yaml
worktree: true    # Always create a worktree (same as valos-agent -w)
...
ValOs automatically compresses long conversations to stay within your model's context window. The compression summarizer is a separate LLM call — you can point it at any provider or endpoint.
...
compression:
  ...
  summary_provider: "auto"                          # Provider: "auto", "openrouter", "valos", "codex", "main", etc.
...
compression:
  ...
  threshold: 0.50
```
Uses the first available provider (OpenRouter → ValOs → Codex) with Gemini Flash.

**Force a specific provider** (OAuth or API-key based):
```yaml
compression:
  summary_provider: valos
  summary_model: gemini-3-flash
```
Works with any provider: `valos`, `openrouter`, `codex`, `anthropic`, `main`, etc.
...
| `valos` / `openrouter` / etc. | not set | Force that provider, use its auth |
...
ValOs uses lightweight "auxiliary" models for side tasks like image analysis, web page summarization, and browser screenshot analysis. By default, these use **Gemini Flash** via auto-detection — you don't need to configure anything.

### The universal config pattern

Every model slot in ValOs — auxiliary tasks, compression, fallback — uses the same three knobs:
...
When `base_url` is set, ValOs ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, ValOs uses that provider's built-in auth and base URL.

Available providers: `auto`, `openrouter`, `valos`, `codex`, `copilot`, `anthropic`, `main`, `zai`, `kimi-coding`, `minimax`, and any provider registered in the [provider registry](/docs/reference/environment-variables).
...
auxiliary:
  ...
  vision:
    provider: "auto"           # "auto", "openrouter", "valos", "codex", "main", etc.
...
Or via environment variable (in `~/.valos-agent/.env`):
...
|----------|-------------|-------------|
| `"auto"` | Best available (default). Vision tries OpenRouter → ValOs → Codex. | — |
| `"openrouter"` | Force OpenRouter — routes to any model (Gemini, GPT-4o, Claude, etc.) | `OPENROUTER_API_KEY` |
| `"valos"` | Force ValOs Portal | `valos-agent login` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `valos-agent model` → Codex |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `valos-agent model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. | Custom endpoint credentials + base URL |
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, ValOs uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
`provider: "main"` follows the same custom endpoint ValOs uses for normal chat. That endpoint can be set directly with `OPENAI_BASE_URL`, or saved once through `valos-agent model` and persisted in `config.yaml`.
...
:::tip
Run `valos-agent config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
If the requested provider is unavailable, ValOs falls back automatically in this order: `local` → `groq` → `openai`.
...
:::note
Streaming is disabled by default. Enable it in `~/.valos-agent/config.yaml` to try the streaming UX.
:::
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want ValOs to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. ValOs still keys DMs by chat/DM ID as usual.
...
Control what ValOs does when an unknown user sends a direct message:
...
- `pair` is the default. ValOs denies access, but replies with a one-time pairing code in DMs.
...
quick_commands:
  status:
    type: exec
    command: systemctl status valos-agent
  disk:
    type: exec
    command: df -h /
  update:
    type: exec
    command: cd ~/.valos-agent/valos-agent && git pull && pip install -e .
  gpu:
    type: exec
...
- **Works everywhere** — CLI, Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, Feishu/Lark, WeCom, SMS, Email, webhooks, Home Assistant, API Server
...
The `web_search`, `web_extract`, and `web_crawl` tools support four backend providers. Configure the backend in `config.yaml` or via `valos-agent tools`:
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.valos-agent/browser_recordings/
  camofox:
...
security:
  website_blocklist:
    ...
    shared_files:                # Load additional rules from external files
      - "/etc/valos-agent/blocked-sites.txt"
```
...
Control how ValOs handles potentially dangerous commands:
...
| `off` | Skip all approval checks. Equivalent to `VALOS_YOLO_MODE=true`. **Use with caution.** |
...
```yaml
checkpoints:
  enabled: true                  # Enable automatic checkpoints (also: valos-agent --checkpoints)
...
**Direct endpoint override:** If you want the obvious custom-endpoint path, set `delegation.base_url`, `delegation.api_key`, and `delegation.model`. That sends subagents directly to that OpenAI-compatible endpoint and takes precedence over `delegation.provider`. If `delegation.api_key` is omitted, ValOs falls back to `OPENAI_API_KEY` only.

The delegation provider uses the same credential resolution as CLI/gateway startup. All configured providers are supported: `openrouter`, `valos`, `copilot`, `zai`, `kimi-coding`, `minimax`, `minimax-cn`. When a provider is set, the system automatically resolves the correct base URL, API key, and API mode — no manual credential wiring needed.
...
ValOs uses two different context scopes:

| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.valos-agent/SOUL.md` or `$VALOS_HOME/SOUL.md` |
| `.valos-agent.md` / `VALOS.md` | Project-specific instructions (highest priority) | Walks to git root |
...
- If SOUL.md is missing, empty, or cannot be loaded, ValOs falls back to a built-in default identity.
- **Project context files use a priority system** — only ONE type is loaded (first match wins): `.valos-agent.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. SOUL.md is always loaded independently.
- **AGENTS.md** is hierarchical: if subdirectories also have AGENTS.md, all are combined.
- ValOs automatically seeds a default `SOUL.md` if one does not already exist.
...
| Context | Default |
|---------|---------|
| **CLI (`valos-agent`)** | Current directory where you run the command |
| **Messaging gateway** | Home directory `~` (override with `MESSAGING_CWD`) |
| **Docker / Singularity / Modal / SSH** | User's home directory inside the container or remote machine |

Override the working directory:
```bash
# In ~/.valos-agent/.env or ~/.valos-agent/config.yaml:
