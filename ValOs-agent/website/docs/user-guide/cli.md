...
sidebar_position: 1
title: "CLI Interface"
description: "Master the ValOs terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

ValOs's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.
...
valos-agent chat --model "anthropic/claude-sonnet-4"

# With a specific provider
valos-agent chat --provider valos        # Use ValOs Portal
valos-agent chat --provider openrouter  # Force OpenRouter
...
valos-agent chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
valos-agent --continue             # Resume the most recent CLI session (-c)
valos-agent --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
valos-agent chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
valos-agent -w                         # Interactive mode in worktree
valos-agent -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the ValOs CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The ValOs CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
When resuming a previous session (`valos-agent -c` or `valos-agent --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend ValOs to background (Unix only). Run `fg` in the shell to resume. |
| `Tab` | Accept auto-suggestion (ghost text) or autocomplete slash commands |

## Slash Commands

Type `/` to see the autocomplete dropdown. ValOs supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for ValOs replies |
...
```bash
valos-agent -s valos-agent-dev,github-auth
valos-agent chat -s github-pr-workflow -s github-auth
```

ValOs loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.valos-agent/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.valos-agent/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain ValOs..."
  # Add your own!
```
...
On Unix systems, press **`Ctrl+Z`** to suspend ValOs to the background — just like any terminal process. The shell prints a confirmation:

```
ValOs has been suspended. Run `fg` to bring ValOs back.
```
...
Resume this session with:
  valos-agent --resume 20260225_143052_a1b2c3
...
```bash
valos-agent --continue                          # Resume the most recent CLI session
valos-agent -c                                  # Short form
valos-agent -c "my project"                     # Resume a named session (latest in lineage)
valos-agent --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
valos-agent --resume "refactoring auth"         # Resume by title
valos-agent -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `valos-agent sessions rename <id> <title>` from the command line. Use `valos-agent sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in ValOs's SQLite state database under `~/.valos-agent/state.db`. The database keeps:
...
ValOs immediately confirms the task and gives you back the prompt:
