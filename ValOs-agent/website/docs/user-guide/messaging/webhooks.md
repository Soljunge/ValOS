...
1. Enable via `valos-agent gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `valos-agent webhook subscribe`
...
Add the `github-pr` route to your `~/.valos-agent/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, ValOs processes the event, and posts a review comment on the PR.
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `valos-agent webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.

### Create a subscription

```bash
valos-agent webhook subscribe github-issues \
  --events "issues" \
...
```bash
valos-agent webhook list
```

### Remove a subscription

```bash
valos-agent webhook remove github-issues
```

### Test a subscription

```bash
valos-agent webhook test github-issues
valos-agent webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.valos-agent/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `valos-agent webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `valos-agent gateway run`
