...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up ValOs as a Feishu or Lark bot"
---

# Feishu / Lark Setup

ValOs integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; ValOs opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | ValOs responds to every message. |
| Group chats | ValOs responds only when the bot is @mentioned in the chat. |
...
Use WebSocket mode when ValOs runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run ValOs behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, ValOs starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
Add the following to `~/.valos-agent/.env`:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how ValOs responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | ValOs responds to @mentions from any user in any group. |
| `allowlist` | ValOs only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | ValOs ignores all group messages entirely. |
...
|---------|---------|---------|
| Quiet period | `VALOS_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `VALOS_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `VALOS_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `VALOS_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.valos-agent/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `VALOS_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |
...
| `VALOS_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `VALOS_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `VALOS_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `VALOS_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `VALOS_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `valos-agent gateway setup` |
| `Another local ValOs gateway is already using this Feishu app_id` | Only one ValOs instance can use the same app_id at a time. Stop the other gateway first. |
...
Feishu / Lark uses the `valos-agent-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
