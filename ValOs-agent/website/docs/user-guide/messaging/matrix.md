...
sidebar_position: 9
title: "Matrix"
description: "Set up ValOs as a Matrix bot"
---

# Matrix Setup

ValOs integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `matrix-nio` Python SDK, processes messages through the ValOs pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

ValOs works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how ValOs behaves once it's connected.

## How ValOs Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | ValOs responds to every message. No `@mention` needed. Each DM has its own session. |
| **Rooms** | ValOs responds to all messages in rooms it has joined. Room invites are auto-accepted. |
| **Threads** | ValOs supports Matrix threads (MSC3440). If you reply in a thread, ValOs keeps the thread context isolated from the main room timeline. |
| **Shared rooms with multiple users** | By default, ValOs isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
2. Choose a username like `valos-agent` — the full user ID will be `@valos-agent:your-server.org`.

### Option B: Use matrix.org or Another Public Homeserver

1. Go to [Element Web](https://app.element.io) and create a new account.
2. Pick a username for your bot (e.g., `valos-agent-bot`).

### Option C: Use Your Own Account

You can also run ValOs as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

ValOs needs an access token to authenticate with the homeserver. You have two options:
...
curl -X POST https://your-server/_matrix/client/v3/login \
  ...
  -d '{
    ...
    "user": "@valos-agent:your-server.org",
...
Instead of providing an access token, you can give ValOs the bot's user ID and password. ValOs will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.

```bash
MATRIX_USER_ID=@valos-agent:your-server.org
MATRIX_PASSWORD=your-password
```

## Step 3: Find Your Matrix User ID

ValOs uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
Add the following to your `~/.valos-agent/.env` file:
...
# MATRIX_USER_ID=@valos-agent:matrix.example.org
...
Optional behavior settings in `~/.valos-agent/config.yaml`:
...
:::tip
You can run `valos-agent gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::

## End-to-End Encryption (E2EE)

ValOs supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with valos-agent extras
pip install 'valos-agent[matrix]'
```
...
Add to your `~/.valos-agent/.env`:

```bash
MATRIX_ENCRYPTION=true
```

When E2EE is enabled, ValOs:

- Stores encryption keys in `~/.valos-agent/matrix/store/`
...
:::warning
If you delete the `~/.valos-agent/matrix/store/` directory, the bot loses its encryption keys. You'll need to verify the device again in your Matrix client. Back up this directory if you want to preserve encrypted sessions.
:::
...
Add this to your `~/.valos-agent/.env`:
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the ValOs logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The ValOs gateway isn't running, or it failed to connect.

**Fix**: Check that `valos-agent gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.valos-agent/.env` and restart the gateway. Use the full `@user:server` format.
...
For more information on securing your ValOs deployment, see the [Security Guide](../security.md).
...
- **Media support**: ValOs can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
