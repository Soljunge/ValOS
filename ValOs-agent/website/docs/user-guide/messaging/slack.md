...
sidebar_position: 4
title: "Slack"
description: "Set up ValOs as a Slack bot using Socket Mode"
---

# Slack Setup

Connect ValOs to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your ValOs instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. ValOs uses the modern
...
4. Enter an app name (e.g., "ValOs") and select your workspace
5. Click **Create App**
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `valos-agent-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a ValOs configuration issue.
:::
...
ValOs uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
valos-agent gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
valos-agent gateway              # Foreground
valos-agent gateway install      # Install as a user service
sudo valos-agent gateway install --system   # Linux only: boot-time system service
```
...
Understanding how ValOs behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@ValOs what time is it?`). In channels, ValOs replies in a thread attached to that message. |
| **Threads** | If you @mention ValOs inside an existing thread, it replies in that same thread. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.valos-agent/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to ValOs in `#general` will have separate histories and contexts.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where ValOs will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @ValOs`).

---

## Multi-Workspace Support

ValOs can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, ValOs also loads tokens from an **OAuth token file** at:

```
~/.valos-agent/platforms/slack/slack_tokens.json
```
...
- When a message arrives, ValOs uses the correct workspace-specific client to respond.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @ValOs` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @ValOs` |
...
7. ✅ Bot was **invited** to the channel (`/invite @ValOs`)
...
- Tokens should be stored in `~/.valos-agent/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your ValOs config directory
