...
sidebar_position: 1
title: "Telegram"
description: "Set up ValOs as a Telegram bot"
---

# Telegram Setup

ValOs integrates with Telegram as a full-featured conversational bot. Once connected, you can chat with your agent from any device, send voice memos that get auto-transcribed, receive scheduled task results, and use the agent in group chats. The integration is built on [python-telegram-bot](https://python-telegram-bot.org/) and supports text, voice, images, and file attachments.
...
2. Send `/newbot`
3. Choose a **display name** (e.g., "ValOs") — this can be anything
4. Choose a **username** — this must be unique and end in `bot` (e.g., `my_valos_bot`)
...
ValOs uses numeric Telegram user IDs to control access. Your user ID is **not** your username — it's a number like `123456789`.
...
By default, ValOs connects to Telegram using **long polling** — the gateway makes outbound requests to Telegram's servers to fetch new updates. This works well for local and always-on deployments.
...
You can also set it manually in `~/.valos-agent/.env`:
...
Voice messages you send on Telegram are automatically transcribed by ValOs's configured STT provider and injected as text into the conversation.

- `local` uses `faster-whisper` on the machine running ValOs — no API key required
...
ValOs works in Telegram group chats with a few considerations:
...
- If `telegram.require_mention` is left unset or false, ValOs keeps the previous open-group behavior and responds to normal group messages it can see

### Example group trigger configuration

Add this to `~/.valos-agent/config.yaml`:
...
Telegram Bot API 9.4 (February 2026) introduced **Private Chat Topics** — bots can create forum-style topic threads directly in 1-on-1 DM chats, no supergroup needed. This lets you run multiple isolated workspaces within your existing DM with ValOs.
...
Add topics under `platforms.telegram.extra.dm_topics` in `~/.valos-agent/config.yaml`:
...
1. On gateway startup, ValOs calls `createForumTopic` for each topic that doesn't have a `thread_id` yet
...
Add topic bindings under `platforms.telegram.extra.group_topics` in `~/.valos-agent/config.yaml`:
...
1. When a message arrives in a mapped group topic, ValOs looks up the `chat_id` and `thread_id` in `group_topics` config
...
| Topic creation | ValOs creates topics via API if `thread_id` is missing | Admin creates topics in Telegram UI |
...
| Problem | Solution |
|---------|----------|
| Bot not responding at all | Verify `TELEGRAM_BOT_TOKEN` is correct. Check `valos-agent gateway` logs for errors. |
...
| Voice messages not transcribed | Verify STT is available: install `faster-whisper` for local transcription, or set `GROQ_API_KEY` / `VOICE_TOOLS_OPENAI_KEY` in `~/.valos-agent/.env`. |
