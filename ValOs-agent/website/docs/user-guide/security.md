...
ValOs is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, ValOs checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.valos-agent/config.yaml`:
...
1. **CLI flag**: Start a session with `valos-agent --yolo` or `valos-agent chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `VALOS_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `VALOS_YOLO_MODE` environment variable which is checked before every command execution.
...
Configure the timeout in `~/.valos-agent/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.valos-agent/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.valos-agent/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` valos-agent/gateway | Self-termination prevention |
...
The `VALOS_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.valos-agent/config.yaml`:
...
:::tip
Use `valos-agent config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, ValOs controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.valos-agent/.env`:
...
For more flexible authorization, ValOs includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `valos-agent pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.valos-agent/config.yaml`:
...
valos-agent pairing list

# Approve a pairing code
valos-agent pairing approve telegram ABC12DEF

# Revoke a user's access
valos-agent pairing revoke telegram 123456789

# Clear all pending codes
valos-agent pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.valos-agent/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, ValOs applies strict security hardening to every container.
...
Container resources are configurable in `~/.valos-agent/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.valos-agent/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `VALOS_HOME`. Skills declare these in frontmatter:
...
When loaded, ValOs checks if these files exist in the active profile's `VALOS_HOME` and registers them for mounting:
...
Paths are relative to `~/.valos-agent/`. Files are mounted to `/root/.valos-agent/` inside the container.
...
| **terminal** (local) | Blocks explicit ValOs infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- ValOs infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
ValOs integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
4. **Store secrets securely** — keep API keys in `~/.valos-agent/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.valos-agent/logs/` for unauthorized access attempts
10. **Keep updated** — run `valos-agent update` regularly for security patches

### Securing API Keys

```bash
# Set proper permissions on the .env file
chmod 600 ~/.valos-agent/.env
...
terminal:
  ...
  ssh_user: "valos-agent"
  ssh_key: "~/.ssh/valos_agent_key"
```
