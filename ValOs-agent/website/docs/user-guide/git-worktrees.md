...
title: "Git Worktrees"
description: "Run multiple ValOs agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

ValOs is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with ValOs so each session has a clean, isolated working directory.

## Why Use Worktrees with ValOs?

ValOs treats the **current working directory** as the project root:

- CLI: the directory where you run `valos-agent` or `valos-agent chat`
...
git worktree add ../repo-feature feature/valos-agent-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/valos-agent-experiment` checked out in that directory

Now you can `cd` into the new worktree and run ValOs there:

```bash
cd ../repo-feature

# Start ValOs in the worktree
valos-agent
```
...
git worktree add ../repo-experiment-a feature/valos-agent-a
git worktree add ../repo-experiment-b feature/valos-agent-b
```
...
- Works on its own branch (`feature/valos-agent-a` vs `feature/valos-agent-b`).
...
- ValOs checkpoint data under `~/.valos-agent/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per ValOs experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/valos-agent-checkpoints-docs`, `feature/valos-agent-refactor-tests`.
- **Commit frequently**
...
- **Avoid running ValOs from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `valos-agent -w` (Automatic Worktree Mode)

ValOs has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `valos-agent/valos-agent-<hash>`).
...
```bash
valos-agent -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `valos-agent -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each ValOs session its own clean checkout.
