---
sidebar_position: 5
title: "Use MCP with ValOs"
description: "A practical guide to connecting MCP servers to ValOs, filtering their tools, and using them safely in real workflows"
---

# Use MCP with ValOs

This guide shows how to actually use MCP with ValOs in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native ValOs tool
- you want ValOs to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect ValOs to internal APIs, databases, or company systems without modifying ValOs core

Do not use MCP when:
- a built-in ValOs tool already solves the job well
...
- ValOs remains the agent
- MCP servers contribute tools
- ValOs discovers those tools at startup or reload time
...
If you installed ValOs with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- ValOs banner/status should show MCP integration when configured
- ask ValOs what tools it has available
...
There are two categories of MCP-exposed functionality in ValOs:
...
2. ValOs-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So ValOs will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want ValOs to reason over a bounded workspace.
...
Now ValOs can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing ValOs core.
...
Because ValOs now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
