...
This tutorial walks you through setting up a Telegram bot powered by ValOs that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **ValOs installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/learning-path) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.valos-agent/.env`

:::tip
A $5/month VPS is plenty for running the gateway. ValOs itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team ValOs Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_valos_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by ValOs. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.valos-agent/.env`:
...
```
[Gateway] Starting ValOs Gateway...
...
# View live logs
journalctl --user -u valos-agent-gateway -f
...
valos-agent gateway start
valos-agent gateway stop
tail -f ~/.valos-agent/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `valos-agent gateway install` to update the plist.
:::
...
```bash
valos-agent gateway stop && valos-agent gateway start
```
...
valos-agent pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.valos-agent/.env`:
...
Control how much detail the bot shows when using tools. In `~/.valos-agent/config.yaml`:
...
Customize how the bot communicates by editing `~/.valos-agent/SOUL.md`:

For a full guide, see [Use SOUL.md with ValOs](/docs/guides/use-soul-with-valos-agent).
...
```bash
# From the CLI
valos-agent cron list          # View all scheduled jobs
valos-agent cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u valos-agent-gateway` (Linux) or `~/.valos-agent/logs/gateway.log` (macOS) |
| Cron job output | `~/.valos-agent/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.valos-agent/cron/jobs.json` |
| Pairing data | `~/.valos-agent/pairing/` |
| Session history | `~/.valos-agent/sessions/` |
