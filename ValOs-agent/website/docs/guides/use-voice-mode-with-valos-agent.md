---
sidebar_position: 7
title: "Use Voice Mode with ValOs"
description: "A practical guide to setting up and using ValOs voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with ValOs
...
- you want ValOs sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in ValOs.
...
## Step 1: make sure normal ValOs works first
...
ValOs supports both local and cloud speech stacks.
...
### If you use `valos-agent setup`

If you choose NeuTTS in the setup wizard, ValOs checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. ValOs transcribes and responds
...
- asking ValOs to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full ValOs loop.

## Tuning CLI behavior

### Silence threshold

If ValOs starts/stops too aggressively, tune:
...
ValOs stays a normal chat bot, but can speak replies.
...
- you want ValOs to function like a portable research or ops assistant
...
ValOs joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- ValOs detects speech boundaries
- transcripts are posted in the associated text channel
- ValOs responds in text and audio
...
1. get text ValOs working
2. install `valos-agent[voice]`
