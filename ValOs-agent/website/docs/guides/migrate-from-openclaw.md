...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to ValOs — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`valos-agent claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into ValOs. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.
...
| What | OpenClaw source | ValOs destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.valos-agent/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.valos-agent/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.valos-agent/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.valos-agent/memories/MEMORY.md` | All daily files merged into main memory. |

All workspace files also check `workspace.default/` as a fallback path.

### Skills (4 sources)

| Source | OpenClaw location | ValOs destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.valos-agent/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.valos-agent/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.valos-agent/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.valos-agent/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing ValOs skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | ValOs destination | Notes |
|------|---------------------|-------------------|-------|
...
| Provider API keys | `models.providers.*.apiKey` | `~/.valos-agent/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | ValOs config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | ValOs config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | ValOs field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.valos-agent/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | ValOs `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | ValOs path | Notes |
|------|-------------|-------------|-------|
...
| Gateway auth token | `gateway.auth.token` | `.env` → `VALOS_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
| Working directory | `agents.defaults.workspace` | `.env` → `MESSAGING_CWD` | |

### Archived (no direct ValOs equivalent)

These are saved to `~/.valos-agent/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in ValOs |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | ValOs has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `valos-agent cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](../user-guide/features/hooks.md) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `valos-agent webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `valos-agent honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `valos-agent skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use ValOs profiles |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env`. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — those values must be added to ValOs manually after migration.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.valos-agent/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Verify API keys** — run `valos-agent status` to check provider authentication.

4. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart valos-agent-gateway`

5. **Check session policies** — verify `valos-agent config get session_reset` matches your expectations.

6. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `valos-agent whatsapp` to pair.
...
Imported skills land in `~/.valos-agent/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `valos-agent config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
