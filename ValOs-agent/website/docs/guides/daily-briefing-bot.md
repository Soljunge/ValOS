...
2. **ValOs spins up** a fresh agent session with your prompt
...
- **ValOs installed** — see the [Installation guide](/docs/getting-started/installation)
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.valos-agent/cron/output/` and you can read them anytime.
:::
...
ValOs will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by ValOs
```
...
ValOs will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell ValOs to delegate each topic to a sub-agent:
...
ValOs will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
