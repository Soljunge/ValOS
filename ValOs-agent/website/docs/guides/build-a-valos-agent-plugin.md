...
title: "Build a ValOs Plugin"
description: "Step-by-step guide to building a complete ValOs plugin with tools, hooks, data files, and skills"
---

# Build a ValOs Plugin

This guide walks through building a complete ValOs plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.
...
```bash
mkdir -p ~/.valos-agent/plugins/calculator
cd ~/.valos-agent/plugins/calculator
```
...
This tells ValOs: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** ValOs may pass additional context in the future.
...
- If this function crashes, the plugin is disabled but ValOs continues fine
...
```
~/.valos-agent/plugins/calculator/
...
def _install_skill():
    """Copy our skill to ~/.valos-agent/skills/ on first load."""
    try:
        from valos_cli.config import get_valos_home
        dest = get_valos_home() / "skills" / "my-plugin" / "SKILL.md"
    except Exception:
        dest = Path.home() / ".valos-agent" / "skills" / "my-plugin" / "SKILL.md"
...
```toml
# pyproject.toml
[project.entry-points."valos_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install valos-agent-plugin-calculator
# Plugin auto-discovered on next valos-agent startup
```
...
```python
# Wrong — will break if ValOs passes extra context
def handler(args):
    ...
