---
sidebar_position: 6
title: "Use SOUL.md with ValOs"
description: "How to use SOUL.md to shape ValOs's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with ValOs

`SOUL.md` is the **primary identity** for your ValOs instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want ValOs to feel like the same assistant every time you talk to it — or if you want to replace the ValOs persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm ValOs should be
- what ValOs should avoid stylistically
- how ValOs should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who ValOs is and how ValOs speaks
...
ValOs now uses only the global SOUL file for the current instance:

```text
~/.valos-agent/SOUL.md
```

If you run ValOs with a custom home directory, it becomes:
...
ValOs automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, ValOs does not overwrite it
- if the file exists but is empty, ValOs adds nothing from it to the prompt

## How ValOs uses it

When ValOs starts a session, it reads `SOUL.md` from `VALOS_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how ValOs feels.
...
ValOs already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How ValOs should sound.

# Avoid
What ValOs should not do.

# Defaults
How ValOs should behave when ambiguity appears.
```
...
```bash
nano ~/.valos-agent/SOUL.md
```

or

```bash
vim ~/.valos-agent/SOUL.md
```

Then restart ValOs or start a new session.
...
### I edited SOUL.md but ValOs still sounds the same

Check:
- you edited `~/.valos-agent/SOUL.md` or `$VALOS_HOME/SOUL.md`
...
### ValOs is ignoring parts of my SOUL.md
