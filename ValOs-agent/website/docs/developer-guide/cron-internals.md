...
description: "How ValOs stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---

# Cron Internals

ValOs cron support is implemented primarily in:
...
Cron jobs are stored in ValOs-managed local state (`~/.valos-agent/cron/jobs.json`) with atomic write semantics.
...
A cron job may attach multiple skills. At runtime, ValOs loads those skills in order and then appends the job prompt as the task instruction.
...
ValOs uses lock-based protections so overlapping scheduler ticks do not execute the same due-job batch twice.
