...
description: "How to add a new inference provider to ValOs — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

ValOs can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `valos-agent model` menu entries
...
1. `valos_cli/auth.py` decides how credentials are found.
2. `valos_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `valos_cli/models.py` and `valos_cli/main.py` make the provider show up in the CLI. (`valos_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `valos_cli/auth.py`
2. `valos_cli/models.py`
3. `valos_cli/runtime_provider.py`
4. `valos_cli/main.py`
...
:::tip
`valos_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `valos-agent setup`.
:::
...
- `PROVIDER_REGISTRY` in `valos_cli/auth.py`
- `_PROVIDER_LABELS` in `valos_cli/models.py`
- `_PROVIDER_ALIASES` in both `valos_cli/auth.py` and `valos_cli/models.py`
- CLI `--provider` choices in `valos_cli/main.py`
...
## Step 2: Add auth metadata in `valos_cli/auth.py`
...
- OAuth / auth-store path: ValOs, OpenAI Codex

Questions to answer here:

- What env vars should ValOs check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `valos_cli/models.py`
...
## Step 4: Resolve runtime data in `valos_cli/runtime_provider.py`
...
Be careful with API-key precedence. ValOs already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `valos_cli/main.py`

A provider is not discoverable until it shows up in the interactive `valos-agent model` flow.

Update these in `valos_cli/main.py`:
...
:::tip
`valos_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `valos-agent model` and `valos-agent setup` automatically.
:::
...
When you add a native provider, double-check that ValOs is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m valos_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `valos_cli/auth.py`
- [ ] aliases added in `valos_cli/auth.py` and `valos_cli/models.py`
- [ ] model catalog added in `valos_cli/models.py`
- [ ] runtime branch added in `valos_cli/runtime_provider.py`
- [ ] CLI wiring added in `valos_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `valos-agent model` but not `valos-agent setup`
