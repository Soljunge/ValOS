...
description: "How to build a memory provider plugin for ValOs"
---

# Building a Memory Provider Plugin

Memory provider plugins give ValOs persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          valos_home (str): Active VALOS_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `valos-agent memory setup` | **Yes** |
| `save_config(values, valos_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
`get_config_schema()` returns a list of field descriptors used by `valos-agent memory setup`:
...
```python
def save_config(self, values: dict, valos_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(valos_home) / "my-provider.json"
...
All storage paths **must** use the `valos_home` kwarg from `initialize()`, not hardcoded `~/.valos-agent`:

```python
# CORRECT — profile-scoped
from valos_constants import get_valos_home
data_dir = get_valos_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.valos-agent/my-provider").expanduser()
```
