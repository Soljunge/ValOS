...
description: "How ValOs builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

ValOs deliberately separates:
...
1. agent identity — `SOUL.md` from `VALOS_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.valos-agent/SOUL.md)
You are ValOs, an AI assistant created by ValOs.
...
`SOUL.md` lives at `~/.valos-agent/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_valos_md(cwd_path)       # 1. .valos-agent.md / VALOS.md (walks to git root)
    ...
    # SOUL.md from VALOS_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.valos-agent.md`, `VALOS.md` | CWD up to git root | ValOs-native project config |
...
- **YAML frontmatter stripped** — `.valos-agent.md` frontmatter is removed (reserved for future config overrides)
...
1. `.valos-agent.md` / `VALOS.md` (walks to git root)
