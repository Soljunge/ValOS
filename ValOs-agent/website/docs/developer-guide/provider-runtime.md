...
description: "How ValOs resolves providers, credentials, API modes, and auxiliary models at runtime"
---

# Provider Runtime Resolution

ValOs has a shared provider runtime resolver used across:
...
- `valos_cli/runtime_provider.py` — credential resolution, `_resolve_custom_runtime()`
- `valos_cli/auth.py` — provider registry, `resolve_provider()`
- `valos_cli/model_switch.py` — shared `/model` switch pipeline (CLI + gateway)
...
That ordering matters because ValOs treats the saved model/provider choice as the source of truth for normal runs. This prevents a stale shell export from silently overriding the endpoint a user last selected in `valos-agent model`.
...
This resolver is the main reason ValOs can share auth/runtime logic between:
...
Set `AI_GATEWAY_API_KEY` in `~/.valos-agent/.env` and run with `--provider ai-gateway`. ValOs fetches available models from the gateway's `/models` endpoint, filtering to language models with tool-use support.

## OpenRouter, AI Gateway, and custom OpenAI-compatible base URLs

ValOs contains logic to avoid leaking the wrong API key to a custom endpoint when multiple provider keys exist (e.g. `OPENROUTER_API_KEY`, `AI_GATEWAY_API_KEY`, and `OPENAI_API_KEY`).
...
ValOs also distinguishes between:
...
When provider resolution selects `anthropic`, ValOs uses:
...
- ValOs preflights Anthropic credential refresh before native Messages API calls
- ValOs still retries once on a 401 after rebuilding the Anthropic client, as a fallback path
...
When an auxiliary task is configured with provider `main`, ValOs resolves that through the same shared runtime path as normal chat. In practice that means:

- env-driven custom endpoints still work
- custom endpoints saved via `valos-agent model` / `config.yaml` also work
- auxiliary routing can tell the difference between a real saved custom endpoint and the OpenRouter fallback

## Fallback models

ValOs supports a configured fallback model/provider pair, allowing runtime failover when the primary model encounters errors.
