...
ValOs currently supports three API execution modes:
...
ValOs wraps API requests so they can be interrupted from the CLI or gateway.
...
ValOs uses two execution strategies:
...
ValOs tracks a shared iteration budget across parent and subagents. It also injects budget pressure hints near the end of the available iteration window.
...
Before and during long runs, ValOs may:
