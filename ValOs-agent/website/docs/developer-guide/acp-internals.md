...
The ACP adapter wraps ValOs' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
valos-agent acp / valos-agent-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.valos-agent/.env
  -> configure stderr logging
  -> construct ValOsACPAgent
  -> acp.run_agent(agent)
```
...
- `allow_once` -> ValOs `once`
- `allow_always` -> ValOs `always`
- reject options -> ValOs `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps ValOs tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["valos-agent-acp"])
...
Instead it reuses ValOs' runtime resolver:

- `acp_adapter/auth.py`
- `valos_cli/runtime_provider.py`

So ACP advertises and uses the currently configured ValOs provider/credentials.
...
- `toolsets.py` — `valos-agent-acp` toolset definition
- `valos_cli/main.py` — `valos-agent acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `valos-agent-acp` script
