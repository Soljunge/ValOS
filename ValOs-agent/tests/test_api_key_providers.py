...
class TestProviderRegistry:
    ...
    def test_oauth_providers_unchanged(self):
        ...
        assert "valos" in PROVIDER_REGISTRY
        assert PROVIDER_REGISTRY["valos"].auth_type == "oauth_device_code"
...
PROVIDER_ENV_VARS = (
    ...
    "VALOS_API_KEY", "GITHUB_TOKEN", "GH_TOKEN",
    "OPENAI_BASE_URL", "VALOS_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
    "VALOS_COPILOT_ACP_ARGS", "COPILOT_ACP_BASE_URL",
)
...
def _clear_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("valos_cli.auth._load_auth_store", lambda: {})
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("valos_cli.copilot_auth._try_gh_cli_token", lambda: "gho_gh_cli_token")
    ...
    def test_copilot_acp_status_detects_local_cli(self, monkeypatch):
        monkeypatch.setenv("VALOS_COPILOT_ACP_ARGS", "--acp --stdio --debug")
        monkeypatch.setattr("valos_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
    ...
    def test_get_auth_status_dispatches_to_external_process(self, monkeypatch):
        monkeypatch.setattr("valos_cli.auth.shutil.which", lambda command: f"/opt/bin/{command}")
    ...
    def test_non_api_key_provider(self):
        status = get_api_key_provider_status("valos")
        assert status["configured"] is False
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_copilot_with_gh_cli_fallback(self, monkeypatch):
        monkeypatch.setattr("valos_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        creds = resolve_api_key_provider_credentials("copilot")
        assert creds["provider"] == "copilot"
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        monkeypatch.setattr("valos_cli.auth.shutil.which", lambda command: None)
        monkeypatch.setattr(
            "valos_cli.auth.os.path.isfile",
            lambda path: path == "/opt/homebrew/bin/gh",
        )
        monkeypatch.setattr(
            "valos_cli.auth.os.access",
        ...
        monkeypatch.setattr("valos_cli.auth.subprocess.run", _fake_run)
    ...
    def test_resolve_copilot_acp_with_local_cli(self, monkeypatch):
        monkeypatch.setenv("VALOS_COPILOT_ACP_ARGS", "--acp --stdio")
        monkeypatch.setattr("valos_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
    ...
    def test_resolve_invalid_provider_raises(self):
        with pytest.raises(AuthError):
            resolve_api_key_provider_credentials("valos")
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_copilot_uses_responses_for_gpt_5_4(self, monkeypatch):
        monkeypatch.setattr("valos_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        monkeypatch.setattr(
            "valos_cli.runtime_provider._get_model_config",
            lambda: {"provider": "copilot", "default": "gpt-5.4"},
        )
        monkeypatch.setattr(
            "valos_cli.models.fetch_github_model_catalog",
...
class TestHasAnyProviderConfigured:
    ...
    def test_glm_key_counts(self, monkeypatch, tmp_path):
        from valos_cli import config as config_module
        monkeypatch.setenv("GLM_API_KEY", "test-key")
        valos_home = tmp_path / ".valos-agent"
        valos_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: valos_home / ".env")
        monkeypatch.setattr(config_module, "get_valos_home", lambda: valos_home)
        from valos_cli.main import _has_any_provider_configured
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        """Claude Code credentials should NOT skip the wizard when ValOs is unconfigured."""
        from valos_cli import config as config_module
        valos_home = tmp_path / ".valos-agent"
        valos_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: valos_home / ".env")
    ...
    def test_config_provider_counts(self, monkeypatch, tmp_path):
        ...
        valos_home = tmp_path / ".valos-agent"
        valos_home.mkdir()
        config_file = valos_home / "config.yaml"
    ...
    def test_claude_code_creds_counted_when_valos_configured(self, monkeypatch, tmp_path):
        """Claude Code credentials should count when ValOs has been explicitly configured."""
        import yaml
        from valos_cli import config as config_module
        valos_home = tmp_path / ".valos-agent"
        valos_home.mkdir()
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from valos_cli.main import _PROVIDER_MODELS
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from valos_cli.main import _PROVIDER_MODELS as main_models
        from valos_cli.models import _PROVIDER_MODELS as models_models
