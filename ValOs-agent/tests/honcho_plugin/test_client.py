...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "valos-agent"
        assert config.workspace_id == "valos-agent"
...
class TestFromGlobalConfig:
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"valos-agent": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"valos-agent": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"valos-agent": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="valos-agent"
        ):
            result = config.resolve_session_name("/home/user/valos-agent/subdir")
        assert result == "valos-agent"
...
class TestGetLinkedWorkspaces:
    ...
    def test_excludes_own_workspace(self):
        config = HonchoClientConfig(
            workspace_id="valos-agent-ws",
            linked_hosts=["other"],
            raw={"hosts": {"other": {"workspace": "valos-agent-ws"}}},
        )
        workspaces = config.get_linked_workspaces()
        assert workspaces == []
...
class TestResolveConfigPath:
    def test_prefers_valos_home_when_exists(self, tmp_path):
        valos_home = tmp_path / "valos-agent"
        valos_home.mkdir()
        local_cfg = valos_home / "honcho.json"
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        valos_home = tmp_path / "valos-agent"
        valos_home.mkdir()
        # No honcho.json in VALOS_HOME

        with patch.dict(os.environ, {"VALOS_HOME": str(valos_home)}):
            result = resolve_config_path()
        assert result == GLOBAL_CONFIG_PATH

    def test_falls_back_to_global_without_valos_home_env(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("VALOS_HOME", None)
            result = resolve_config_path()
        assert result == GLOBAL_CONFIG_PATH
...
class TestResolveActiveHost:
    def test_default_returns_valos(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("VALOS_HONCHO_HOST", None)
            os.environ.pop("VALOS_HOME", None)
            assert resolve_active_host() == "valos-agent"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"VALOS_HONCHO_HOST": "valos-agent.coder"}):
            assert resolve_active_host() == "valos-agent.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("VALOS_HONCHO_HOST", None)
            with patch("valos_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "valos-agent.coder"

    def test_default_profile_returns_valos(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("VALOS_HONCHO_HOST", None)
            with patch("valos_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "valos-agent"

    def test_custom_profile_returns_valos(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("VALOS_HONCHO_HOST", None)
            with patch("valos_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "valos-agent"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("VALOS_HONCHO_HOST", None)
            # Temporarily remove valos_cli.profiles to simulate import failure
            saved = sys.modules.get("valos_cli.profiles")
            sys.modules["valos_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "valos-agent"
            finally:
                if saved is not None:
                    sys.modules["valos_cli.profiles"] = saved
                else:
                    sys.modules.pop("valos_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="valos-agent.coder")
        assert config.host == "valos-agent.coder"
        assert config.workspace_id == "valos-agent"  # shared workspace
        assert config.ai_peer == "valos-agent.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="valos-agent")
        assert config.host == "valos-agent"
        assert config.workspace_id == "valos-agent"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "valos-agent": {"aiPeer": "valos-agent", "peerName": "alice"},
                "valos-agent.coder": {
                    "aiPeer": "valos-agent.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="valos-agent.coder", config_path=config_file,
        )
        assert config.host == "valos-agent.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "valos-agent.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="valos-agent.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "valos-agent.dreamer"
