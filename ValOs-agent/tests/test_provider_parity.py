...
class TestDeveloperRoleSwap:
    ...
    def test_developer_role_via_valos_portal(self, monkeypatch):
        agent = _make_agent(monkeypatch, "valos", base_url="https://inference-api.valos-agent.com/v1")
        agent.model = "gpt-5"
        messages = [
            {"role": "system", "content": "You are helpful."},
            {"role": "user", "content": "hi"},
...
class TestBuildApiKwargsValOsPortal:
    def test_includes_valos_product_tags(self, monkeypatch):
        agent = _make_agent(monkeypatch, "valos", base_url="https://inference-api.valos-agent.com/v1")
        messages = [{"role": "user", "content": "hi"}]
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == ["product=valos-agent"]
...
# ── Chat completions response handling (OpenRouter/ValOs) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_valos_when_no_openrouter(self, monkeypatch):
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
        from agent.auxiliary_client import get_text_auxiliary_client
        with patch("agent.auxiliary_client._read_valos_auth", return_value={"access_token": "valos-tok"}), \
             patch("agent.auxiliary_client.OpenAI") as mock:
            client, model = get_text_auxiliary_client()
        assert model == "google/gemini-3-flash-preview"

    def test_custom_endpoint_when_no_valos(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/ValOs keys are available.
