...
def test_round_robin_strategy_rotates_priorities(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "valos-agent" / "config.yaml"
...
def test_mark_exhausted_and_rotate_persists_status(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "valos-agent" / "auth.json").read_text())
...
def test_try_refresh_current_updates_only_current_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "valos_cli.auth.refresh_codex_oauth_pure",
...
def test_load_pool_migrates_valos_provider_state(tmp_path, monkeypatch):
    monkeypatch.setenv("VALOS_HOME", str(tmp_path / "valos-agent"))
    _write_auth_store(
        tmp_path,
        {
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_valos_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_load_pool_migrates_valos_provider_state_preserves_tls(tmp_path, monkeypatch):
    monkeypatch.setenv("VALOS_HOME", str(tmp_path / "valos-agent"))
    _write_auth_store(
        tmp_path,
        {
    ...
    auth_payload = json.loads((tmp_path / "valos-agent" / "auth.json").read_text())
    assert auth_payload["credential_pool"]["valos"][0]["tls"] == {
        "insecure": True,
        "ca_bundle": "/tmp/valos-ca.pem",
    }
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    assert {entry.source for entry in entries} == {"manual:valos_pkce", "valos_pkce"}
...
def test_get_custom_provider_pool_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("VALOS_HOME", str(tmp_path / "valos-agent"))
    (tmp_path / "valos-agent").mkdir(parents=True, exist_ok=True)
    import yaml
    config_path = tmp_path / "valos-agent" / "config.yaml"
    config_path.write_text(yaml.dump({
        "custom_providers": [
