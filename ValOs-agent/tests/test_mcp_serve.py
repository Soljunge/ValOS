"""
Tests for mcp_serve — ValOs MCP server.
...
def _isolate_valos_home(tmp_path, monkeypatch):
    """Redirect VALOS_HOME to a temp directory."""
    monkeypatch.setenv("VALOS_HOME", str(tmp_path))
    try:
        import valos_constants
        monkeypatch.setattr(valos_constants, "get_valos_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking valos_state schema."""
