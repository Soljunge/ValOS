...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (valos_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "valos-agent" / "auth.json").read_text())
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_valos_oauth_login_pure",
        lambda: {
            "access_token": token,
    ...
    entry = next(item for item in entries if item["source"] == "manual:valos_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:valos_pkce"
    assert entry["refresh_token"] == "refresh-token"
    assert entry["expires_at_ms"] == 1711234567000


def test_auth_add_valos_oauth_persists_pool_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("VALOS_HOME", str(tmp_path / "valos-agent"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("valos@example.com")
    monkeypatch.setattr(
        "valos_cli.auth._valos_device_code_login",
        lambda **kwargs: {
            "portal_base_url": "https://portal.example.com",
            "inference_base_url": "https://inference.example.com/v1",
            "client_id": "valos-agent-cli",
    ...
    payload = json.loads((tmp_path / "valos-agent" / "auth.json").read_text())
    entries = payload["credential_pool"]["valos"]
    entry = next(item for item in entries if item["source"] == "manual:device_code")
    assert entry["label"] == "valos@example.com"
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "valos_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from valos_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from valos_cli.auth_commands import auth_reset_command
...
def test_clear_provider_auth_removes_provider_pool_entries(tmp_path, monkeypatch):
    ...
    from valos_cli.auth import clear_provider_auth
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from valos_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "valos_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("valos_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("valos_cli.auth_commands.time.time", lambda: 1030.0)
