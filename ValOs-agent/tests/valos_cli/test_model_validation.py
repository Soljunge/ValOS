"""Tests for provider-aware `/model` validation in valos_cli.models."""
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("valos_cli.models.fetch_api_models", return_value=api_models), \
         patch("valos_cli.models.probe_api_models", return_value=probe_payload):
...
class TestParseModelInput:
    ...
    def test_provider_colon_model_switches_provider(self):
        provider, model = parse_model_input("openrouter:anthropic/claude-sonnet-4.5", "valos")
        assert provider == "openrouter"
        assert model == "anthropic/claude-sonnet-4.5"
    ...
    def test_valos_provider_switch(self):
        provider, model = parse_model_input("valos:valos-agent-3", "openrouter")
        assert provider == "valos"
        assert model == "valos-agent-3"

    def test_empty_model_after_colon_keeps_current(self):
        provider, model = parse_model_input("openrouter:", "valos")
        assert provider == "valos"
...
class TestProviderModelIds:
    ...
    def test_copilot_prefers_live_catalog(self):
        with patch("valos_cli.auth.resolve_api_key_provider_credentials", return_value={"api_key": "gh-token"}), \
             patch("valos_cli.models._fetch_github_models", return_value=["gpt-5.4", "claude-sonnet-4.6"]):
    ...
    def test_copilot_acp_falls_back_to_copilot_defaults(self):
        with patch("valos_cli.auth.resolve_api_key_provider_credentials", side_effect=Exception("no token")), \
             patch("valos_cli.models._fetch_github_models", return_value=None):
...
class TestFetchApiModels:
    ...
    def test_returns_none_on_network_error(self):
        with patch("valos_cli.models.urllib.request.urlopen", side_effect=Exception("timeout")):
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("valos_cli.models.urllib.request.urlopen", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("valos_cli.models.urllib.request.urlopen", return_value=_Resp()) as mock_urlopen:
    ...
    def test_fetch_github_model_catalog_filters_non_chat_models(self):
        ...
        with patch("valos_cli.models.urllib.request.urlopen", return_value=_Resp()):
