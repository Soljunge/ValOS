"""Tests for valos_cli.tools_config platform tool persistence."""

from unittest.mock import patch

from valos_cli.tools_config import (
    _configure_provider,
    _get_platform_tools,
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/horatiu.sol/valos-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
    ...
    with patch("valos_cli.tools_config.save_config"):
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (valos-agent-cli, valos-agent-telegram, etc.) must NOT
    ...
    ``valos-agent tools``, but valos-agent-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "valos-agent-cli",  # <-- the culprit
    ...
    # valos-agent-cli must NOT survive — it's a platform default, not an MCP server
    assert "valos-agent-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_valos_telegram():
    """Same bug for Telegram — valos-agent-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "valos-agent-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "valos-agent-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "valos-agent-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_visible_providers_include_valos_subscription_when_logged_in(monkeypatch):
    monkeypatch.setenv("VALOS_ENABLE_VALOS_MANAGED_TOOLS", "1")
    config = {"model": {"provider": "valos"}}

    monkeypatch.setattr(
        "valos_cli.valos_subscription.get_valos_auth_status",
        lambda: {"logged_in": True},
    )

    providers = _visible_providers(TOOL_CATEGORIES["browser"], config)

    assert providers[0]["name"].startswith("ValOs Subscription")


def test_visible_providers_hide_valos_subscription_when_feature_flag_is_off(monkeypatch):
    ...
    assert all(not provider["name"].startswith("ValOs Subscription") for provider in providers)
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("valos_cli.tools_config._run_post_setup", lambda key: None)
...
def test_first_install_valos_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setenv("VALOS_ENABLE_VALOS_MANAGED_TOOLS", "1")
    config = {
        "model": {"provider": "valos"},
    ...
    monkeypatch.setattr(
        "valos_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"web", "image_gen", "tts", "browser"},
    )
    monkeypatch.setattr("valos_cli.tools_config.save_config", lambda config: None)
    monkeypatch.setattr(
        "valos_cli.valos_subscription.get_valos_auth_status",
        lambda: {"logged_in": True},
    )
    ...
    configured = []
    monkeypatch.setattr(
        "valos_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from valos_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """valos-agent-gateway includes list should cover all messaging platforms."""
        from valos_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["valos-agent-gateway"]["includes"])
        ...
        for platform, meta in PLATFORMS.items():
            ...
            assert ts_name in gateway_includes, (
                ...
                f"valos-agent-gateway includes"
            )
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from valos_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from valos_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
