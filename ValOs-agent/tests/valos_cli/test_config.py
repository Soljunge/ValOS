"""Tests for valos_cli configuration management."""
...
class TestGetValOsHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("VALOS_HOME", None)
            home = get_valos_home()
            assert home == Path.home() / ".valos-agent"

    def test_env_override(self):
        with patch.dict(os.environ, {"VALOS_HOME": "/custom/path"}):
            home = get_valos_home()
            assert home == Path("/custom/path")


class TestEnsureValOsHome:
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"VALOS_HOME": str(tmp_path)}, clear=False):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from valos_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
