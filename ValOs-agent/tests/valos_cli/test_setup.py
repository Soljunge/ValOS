...
from valos_cli.setup import setup_model_provider
...
def test_setup_syncs_valos_from_disk(tmp_path, monkeypatch):
    """ValOs OAuth writes config to disk; wizard config dict must pick it up."""
    monkeypatch.setenv("VALOS_HOME", str(tmp_path))
    _clear_provider_env(monkeypatch)
    _stub_tts(monkeypatch)
    ...
    def fake_select():
        _write_model_config(tmp_path, "valos", "https://inference.example.com/v1", "gemini-3-flash")
    ...
    reloaded = load_config()
    assert isinstance(reloaded["model"], dict)
    assert reloaded["model"]["provider"] == "valos"
...
def test_modal_setup_can_use_valos_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("valos_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("valos_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "valos_cli.setup.get_valos_subscription_features",
        lambda config: type("Features", (), {"valos_auth_present": True})(),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    from valos_cli.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("valos_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("valos_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("valos_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "valos_cli.setup.get_valos_subscription_features",
        lambda config: type("Features", (), {"valos_auth_present": True})(),
