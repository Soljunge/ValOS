"""Tests for the valos_cli models module."""
...
class TestFindOpenrouterSlug:
    def test_exact_match(self):
        from valos_cli.models import _find_openrouter_slug
...
class TestDetectProviderForModel:
    ...
    def test_openai_model_detected(self):
        """OpenAI model names should resolve to the OpenAI provider."""
        result = detect_provider_for_model("openai/gpt-4.1", "anthropic")
        assert result is not None
        assert result[0] == "openai"
        assert result[1] == "openai/gpt-4.1"


class TestOpenAIModelHelpers:
    def test_normalize_openai_model_id_strips_provider_prefix(self):
        assert normalize_openai_model_id("openai/gpt-4.1") == "gpt-4.1"

    def test_openai_model_api_mode_matches_model_family(self):
        assert openai_model_api_mode("gpt-5.4") == "codex_responses"
        assert openai_model_api_mode("gpt-4.1") == "chat_completions"

    def test_aggregator_not_suggested(self):
        """valos/openrouter should never be auto-suggested as target provider."""
        result = detect_provider_for_model("claude-opus-4-6", "openai-codex")
        assert result is not None
        assert result[0] not in ("valos",)  # valos has claude models but shouldn't be suggested
