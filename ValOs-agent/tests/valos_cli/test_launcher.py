"""Tests for the top-level `./valos-agent` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./valos-agent` should use `valos_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "valos-agent"
    called = []

    fake_main_module = types.ModuleType("valos_cli.main")

    def fake_main():
        called.append("valos_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "valos_cli.main", fake_main_module)
