"""Tests for ValOs subscription feature detection."""

from valos_cli import valos_subscription as ns


def test_get_valos_subscription_features_recognizes_direct_exa_backend(monkeypatch):
    env = {"EXA_API_KEY": "exa-test"}

    monkeypatch.setattr(ns, "get_env_value", lambda name: env.get(name, ""))
    monkeypatch.setattr(ns, "get_valos_auth_status", lambda: {})
    monkeypatch.setattr(ns, "managed_valos_tools_enabled", lambda: False)
    monkeypatch.setattr(ns, "_toolset_enabled", lambda config, key: key == "web")
    monkeypatch.setattr(ns, "_has_agent_browser", lambda: False)
    monkeypatch.setattr(ns, "resolve_openai_audio_api_key", lambda: "")
    monkeypatch.setattr(ns, "has_direct_modal_credentials", lambda: False)

    features = ns.get_valos_subscription_features({"web": {"backend": "exa"}})

    assert features.web.available is True
    assert features.web.active is True
    assert features.web.managed_by_valos is False
    assert features.web.direct_override is True
    assert features.web.current_provider == "exa"


def test_get_valos_subscription_features_prefers_managed_modal_in_auto_mode(monkeypatch):
    monkeypatch.setenv("VALOS_ENABLE_VALOS_MANAGED_TOOLS", "1")
    monkeypatch.setattr(ns, "get_env_value", lambda name: "")
    monkeypatch.setattr(ns, "get_valos_auth_status", lambda: {"logged_in": True})
    monkeypatch.setattr(ns, "managed_valos_tools_enabled", lambda: True)
    ...
    features = ns.get_valos_subscription_features(
        {"terminal": {"backend": "modal", "modal_mode": "auto"}}
    )

    assert features.modal.available is True
    assert features.modal.active is True
    assert features.modal.managed_by_valos is True
    assert features.modal.direct_override is False


def test_get_valos_subscription_features_prefers_camofox_over_managed_browserbase(monkeypatch):
    ...
    assert features.browser.managed_by_valos is False
    assert features.browser.direct_override is True
    assert features.browser.current_provider == "Camofox"


def test_get_valos_subscription_features_requires_agent_browser_for_browserbase(monkeypatch):
    env = {
