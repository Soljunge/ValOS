...
import valos_cli.gateway as gateway_cli


class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "valos-agent-gateway.service"
...
class TestLaunchdServiceRecovery:
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.valos-agent.gateway.plist"
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "start", "ai.valos-agent.gateway"])
            ),
        )
...
class TestSystemUnitValOsHome:
    """VALOS_HOME in system units must reference the target user, not root."""

    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        # Simulate sudo: Path.home() returns /root, target user is alice
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("VALOS_HOME", raising=False)
        monkeypatch.setattr(
        ...
        assert 'VALOS_HOME=/home/alice/.valos-agent' in unit
        assert '/root/.valos-agent' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: VALOS_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("VALOS_HOME", "/root/.valos-agent/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'VALOS_HOME=/home/alice/.valos-agent/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_valos_home(self, monkeypatch):
        # Custom VALOS_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("VALOS_HOME", "/opt/valos-agent-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'VALOS_HOME=/opt/valos-agent-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's VALOS_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        valos_home = str(gateway_cli.get_valos_home().resolve())
        assert f'VALOS_HOME={valos_home}' in unit


class TestValOsHomeForTargetUser:
    """Unit tests for _valos_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("VALOS_HOME", raising=False)

        result = gateway_cli._valos_home_for_target_user("/home/alice")
        assert result == "/home/alice/.valos-agent"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._valos_home_for_target_user("/home/alice")
        assert result == "/home/alice/.valos-agent/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("VALOS_HOME", "/opt/valos-agent")

        result = gateway_cli._valos_home_for_target_user("/home/alice")
        assert result == "/opt/valos-agent"
