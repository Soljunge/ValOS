...
def _isolate(tmp_path, monkeypatch):
    """Isolate VALOS_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal ValOsCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ValOsCLI.__new__(ValOsCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["valos-agent-core"]
        obj.compact = False
