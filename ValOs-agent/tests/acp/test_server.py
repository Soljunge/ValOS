"""Tests for acp_adapter.server — ValOsACPAgent ACP server."""
...
from acp_adapter.server import ValOsACPAgent, VALOS_VERSION
...
def agent(mock_manager):
    """ValOsACPAgent backed by a mock session manager."""
    return ValOsACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "valos-agent"
        assert resp.agent_info.version == VALOS_VERSION
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert VALOS_VERSION in result
    ...
    def test_model_switch_uses_requested_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("valos_cli.config.load_config", lambda: {
        ...
        with patch("run_agent.AIAgent", side_effect=fake_agent):
            acp_agent = ValOsACPAgent(session_manager=manager)
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["valos-agent-acp"]
