...
class TestProviderMapping:
    def test_all_mapped_providers_are_strings(self):
        for valos_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
            assert isinstance(valos_id, str)
    ...
    def test_unmapped_provider_not_in_dict(self):
        assert "valos" not in PROVIDER_TO_MODELS_DEV
...
class TestLookupModelsDevContext:
    ...
    def test_provider_not_mapped(self, mock_fetch):
        mock_fetch.return_value = SAMPLE_REGISTRY
        assert lookup_models_dev_context("valos", "some-model") is None
