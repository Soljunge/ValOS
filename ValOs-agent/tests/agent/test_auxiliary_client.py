...
def _clean_env(monkeypatch):
    ...
    for key in (
        ...
        "OPENAI_MODEL", "LLM_MODEL", "VALOS_INFERENCE_BASE_URL",
...
class TestReadCodexAccessToken:
    def test_valid_auth_store(self, tmp_path, monkeypatch):
        ...
        (valos_home / "auth.json").write_text(json.dumps({
            "version": 1,
            "providers": {
                "openai-codex": {
    ...
    def test_missing_returns_none(self, tmp_path, monkeypatch):
        valos_home = tmp_path / "valos-agent"
        valos_home.mkdir(parents=True, exist_ok=True)
        (valos_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
        monkeypatch.setenv("VALOS_HOME", str(valos_home))
        result = _read_codex_access_token()
...
class TestExpiredCodexFallback:
    ...
    def test_valos_oauth_file_sets_oauth_flag(self, monkeypatch):
        """OAuth-style tokens should get is_oauth=*** (token is not sk-ant-api-*)."""
        # Mock resolve_anthropic_token to return an OAuth-style token
        with patch("agent.anthropic_adapter.resolve_anthropic_token", return_value="valos-agent-oauth-jwt-token"), \
             patch("agent.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            from agent.auxiliary_client import _try_anthropic, AnthropicAuxiliaryClient
...
class TestGetTextAuxiliaryClient:
    ...
    def test_valos_takes_priority_over_codex(self, monkeypatch, codex_auth_dir):
        with patch("agent.auxiliary_client._read_valos_auth") as mock_valos, \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            mock_valos.return_value = {"access_token": "valos-tok"}
            client, model = get_text_auxiliary_client()
        assert model == "google/gemini-3-flash-preview"
    ...
    def test_custom_endpoint_over_codex(self, monkeypatch, codex_auth_dir):
        ...
        monkeypatch.setattr("valos_cli.config.load_config", lambda: config)
        monkeypatch.setattr("valos_cli.runtime_provider.load_config", lambda: config)
        # Override the autouse monkeypatch for codex
        monkeypatch.setattr(
            "agent.auxiliary_client._read_codex_access_token",
            lambda: "codex-test-token-abc123",
        )
        with patch("agent.auxiliary_client._read_valos_auth", return_value=None), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            client, model = get_text_auxiliary_client()
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("valos_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
...
class TestVisionClientFallback:
    ...
    def test_vision_returns_none_without_any_credentials(self):
        with (
            patch("agent.auxiliary_client._read_valos_auth", return_value=None),
    ...
    def test_vision_auto_includes_openai_when_configured(self, monkeypatch):
        monkeypatch.setenv("OPENAI_API_KEY", "sk-openai-direct")
        with (
            patch("agent.auxiliary_client._read_valos_auth", return_value=None),
            patch("agent.auxiliary_client._try_anthropic", return_value=(None, None)),
            patch("valos_cli.config.load_config", return_value={"model": {"provider": "openai", "default": "gpt-4.1"}}),
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
        ):
            backends = get_available_vision_backends()
...
class TestAuxiliaryPoolAwareness:
    def test_try_valos_uses_pool_entry(self):
        class _Entry:
        ...
        with (
            ...
            from agent.auxiliary_client import _try_valos
    ...
    def test_resolve_provider_client_copilot_uses_runtime_credentials(self, monkeypatch):
        ...
        with (
            patch(
                "valos_cli.auth.resolve_api_key_provider_credentials",
                return_value={
                    "provider": "copilot",
    ...
    def test_resolve_provider_client_openai_uses_direct_api_key(self, monkeypatch):
        monkeypatch.setenv("OPENAI_API_KEY", "sk-openai-direct")
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)

        with patch("agent.auxiliary_client.OpenAI") as mock_openai:
            client, model = resolve_provider_client("openai", model="gpt-4.1")

        assert client is not None
        assert model == "gpt-4.1"
        call_kwargs = mock_openai.call_args.kwargs
        assert call_kwargs["api_key"] == "sk-openai-direct"
    ...
    def test_selected_anthropic_provider_is_preferred_for_vision_auto(self, monkeypatch):
        ...
        with (
            ...
            patch("agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-api03-key"),
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
            patch("valos_cli.config.load_config", fake_load_config),
        ):
            client, model = get_vision_auxiliary_client()
    ...
    def test_selected_codex_provider_short_circuits_vision_auto(self, monkeypatch):
        ...
        with (
            ...
            patch("agent.auxiliary_client._try_valos") as mock_valos,
    ...
    def test_vision_auto_falls_back_to_custom_endpoint(self, monkeypatch):
        ...
        When no OpenRouter/ValOs/Codex is available, try the custom endpoint.
        """
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
    ...
    def test_vision_uses_valos_when_available(self, monkeypatch):
        with patch("agent.auxiliary_client._read_valos_auth") as mock_valos, \
             patch("agent.auxiliary_client.OpenAI"):
            mock_valos.return_value = {"access_token": "valos-tok"}
            client, model = get_vision_auxiliary_client()
...
class TestGetAuxiliaryProvider:
    ...
    def test_context_prefix_fallback(self, monkeypatch):
        monkeypatch.setenv("CONTEXT_COMPRESSION_PROVIDER", "valos")
        assert _get_auxiliary_provider("compression") == "valos"
...
class TestResolveForcedProvider:
    ...
    def test_forced_valos(self, monkeypatch):
        with patch("agent.auxiliary_client._read_valos_auth") as mock_valos, \
             patch("agent.auxiliary_client.OpenAI"):
            mock_valos.return_value = {"access_token": "valos-tok"}
            client, model = _resolve_forced_provider("valos")
        assert model == "google/gemini-3-flash-preview"
        assert client is not None

    def test_forced_valos_not_configured(self, monkeypatch):
        with patch("agent.auxiliary_client._read_valos_auth", return_value=None):
            client, model = _resolve_forced_provider("valos")
        assert client is None
        assert model is None
    ...
    def test_forced_main_skips_openrouter_valos(self, monkeypatch):
...
class TestTaskSpecificOverrides:
    ...
    def test_text_with_vision_provider_override(self, monkeypatch):
        ...
        monkeypatch.setenv("AUXILIARY_VISION_PROVIDER", "valos")
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client.OpenAI"):
            client, model = get_text_auxiliary_client()  # no task → auto
        assert model == "google/gemini-3-flash-preview"  # OpenRouter, not ValOs
    ...
    def test_compression_task_reads_context_prefix(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._read_valos_auth") as mock_valos, \
             patch("agent.auxiliary_client.OpenAI"):
            mock_valos.return_value = {"access_token": "***"}
            client, model = get_text_auxiliary_client("compression")
        # Config-first: model comes from config.yaml summary_model default,
        # but provider is forced to ValOs via env var
        assert client is not None
    ...
    def test_task_direct_endpoint_from_config(self, monkeypatch, tmp_path):
        valos_home = tmp_path / "valos-agent"
        valos_home.mkdir(parents=True, exist_ok=True)
        (valos_home / "config.yaml").write_text(
            """auxiliary:
  web_extract:
