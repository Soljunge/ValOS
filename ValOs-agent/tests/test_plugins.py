"""Tests for the ValOs plugin system (valos_cli.plugins)."""
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.valos-agent/plugins/ are discovered."""
        plugins_dir = tmp_path / "valos_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("VALOS_HOME", str(tmp_path / "valos_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("VALOS_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".valos-agent" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under valos_plugins.<name>."""
        plugins_dir = tmp_path / "valos_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("VALOS_HOME", str(tmp_path / "valos_test"))

        # Clean up any prior namespace module
        sys.modules.pop("valos_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "valos_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="valos_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import valos_cli.plugins as plugins_mod
...
# in PluginContext (valos_cli/plugins.py).  The tests referenced _plugin_commands,
