...
def test_resolve_managed_tool_gateway_derives_vendor_origin_from_shared_domain():
    with patch.dict(
        os.environ,
        {
            "VALOS_ENABLE_VALOS_MANAGED_TOOLS": "1",
        ...
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "valos-token",
        )

    assert result is not None
    assert result.gateway_origin == "https://firecrawl-gateway.valos-agent.com"
    assert result.valos_user_token == "valos-token"
...
def test_resolve_managed_tool_gateway_is_inactive_without_valos_token():
    with patch.dict(
        os.environ,
        {
            "VALOS_ENABLE_VALOS_MANAGED_TOOLS": "1",
...
def test_resolve_managed_tool_gateway_is_disabled_without_feature_flag():
    with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "valos-agent.com"}, clear=False):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "valos-token",
        )
...
def test_read_valos_access_token_refreshes_expiring_cached_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "valos_cli.auth.resolve_valos_access_token",
        lambda refresh_skew_seconds=120: "fresh-token",
    )

    assert managed_tool_gateway.read_valos_access_token() == "fresh-token"
