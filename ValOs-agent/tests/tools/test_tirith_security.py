...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._valos_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_valos_bin(self):
        """After _INSTALL_FAILED, manual install in VALOS_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        valos_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(valos_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(valos_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._valos_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == valos_bin
            assert _tirith_mod._resolved_path == valos_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# VALOS_HOME isolation
# ---------------------------------------------------------------------------

class TestValOsHomeIsolation:
    def test_valos_bin_dir_respects_valos_home(self):
        """_valos_bin_dir must use VALOS_HOME, not hardcoded ~/.valos-agent."""
        from tools.tirith_security import _valos_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"VALOS_HOME": tmpdir}):
            result = _valos_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_valos_home(self):
        """_failure_marker_path must use VALOS_HOME, not hardcoded ~/.valos-agent."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"VALOS_HOME": "/custom/valos-agent"}):
            result = _failure_marker_path()
        assert result == "/custom/valos-agent/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets VALOS_HOME; verify it's active."""
        valos_home = os.getenv("VALOS_HOME")
        assert valos_home is not None, "VALOS_HOME should be set by conftest"
        assert "valos_test" in valos_home, "Should point to test temp dir"

    def test_get_valos_home_fallback(self):
        """Without VALOS_HOME set, falls back to ~/.valos-agent."""
        from tools.tirith_security import _get_valos_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove VALOS_HOME entirely
            os.environ.pop("VALOS_HOME", None)
            result = _get_valos_home()
        assert result == os.path.join(os.path.expanduser("~"), ".valos-agent")
