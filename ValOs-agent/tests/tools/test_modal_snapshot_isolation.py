...
def _restore_tool_modules():
    original_valos_home = os.environ.get("VALOS_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_valos_home is None:
            os.environ.pop("VALOS_HOME", None)
        else:
            os.environ["VALOS_HOME"] = original_valos_home
        _reset_modules(("tools", "valos_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    valos_cli = types.ModuleType("valos_cli")
    valos_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["valos_cli"] = valos_cli
    valos_home = tmp_path / "valos-agent-home"
    os.environ["VALOS_HOME"] = str(valos_home)
    sys.modules["valos_cli.config"] = types.SimpleNamespace(
        get_valos_home=lambda: valos_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="valos-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": valos_home / "modal_snapshots.json",
