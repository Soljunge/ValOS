...
class TestLoadMCPConfig:
    def test_no_config_returns_empty(self):
        ...
        with patch("valos_cli.config.load_config", return_value={"model": "test"}):
    ...
    def test_valid_config_parsed(self):
        ...
        with patch("valos_cli.config.load_config", return_value={"mcp_servers": servers}):
            from tools.mcp_tool import _load_mcp_config
            result = _load_mcp_config()
    ...
    def test_mcp_servers_not_dict_returns_empty(self):
        ...
        with patch("valos_cli.config.load_config", return_value={"mcp_servers": "invalid"}):
            from tools.mcp_tool import _load_mcp_config
            result = _load_mcp_config()
            assert result == {}
...
class TestSchemaConversion:
    def test_converts_mcp_tool_to_valos_schema(self):
...
class TestToolsetInjection:
    def test_mcp_tools_added_to_all_valos_toolsets(self):
        """Discovered MCP tools are dynamically injected into all valos-agent-* toolsets."""
        from tools.mcp_tool import MCPServerTask
        ...
        fake_toolsets = {
            "valos-agent-cli": {"tools": ["terminal"], "description": "CLI", "includes": []},
            "valos-agent-telegram": {"tools": ["terminal"], "description": "TG", "includes": []},
            "valos-agent-gateway": {"tools": [], "description": "GW", "includes": []},
            "non-valos-agent": {"tools": [], "description": "other", "includes": []},
        }
        ...
        # All valos-agent-* toolsets get injection
        assert "mcp_fs_list_files" in fake_toolsets["valos-agent-cli"]["tools"]
        assert "mcp_fs_list_files" in fake_toolsets["valos-agent-telegram"]["tools"]
        assert "mcp_fs_list_files" in fake_toolsets["valos-agent-gateway"]["tools"]
        # Non-valos-agent toolset should NOT get injection
        assert "mcp_fs_list_files" not in fake_toolsets["non-valos-agent"]["tools"]
        # Original tools preserved
        assert "terminal" in fake_toolsets["valos-agent-cli"]["tools"]
    ...
    def test_server_connection_failure_skipped(self):
        ...
        fake_toolsets = {
            "valos-agent-cli": {"tools": [], "description": "CLI", "includes": []},
        }
