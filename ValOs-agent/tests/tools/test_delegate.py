...
class TestDelegationCredentialResolution(unittest.TestCase):
    ...
    @patch("valos_cli.runtime_provider.resolve_runtime_provider")
    ...
    @patch("valos_cli.runtime_provider.resolve_runtime_provider")
    def test_valos_provider_resolves_valos_credentials(self, mock_resolve):
        """ValOs provider resolves ValOs Portal base_url and api_key."""
        mock_resolve.return_value = {
            "provider": "valos",
            "base_url": "https://inference-api.valos-agent.com/v1",
            "api_key": "valos-agent-key-xyz",
            "api_mode": "chat_completions",
        }
        parent = _make_mock_parent(depth=0)
        cfg = {"model": "valos-agent-3-llama-3.1-8b", "provider": "valos"}
        creds = _resolve_delegation_credentials(cfg, parent)
        self.assertEqual(creds["provider"], "valos")
        self.assertEqual(creds["base_url"], "https://inference-api.valos-agent.com/v1")
        self.assertEqual(creds["api_key"], "valos-agent-key-xyz")
        mock_resolve.assert_called_once_with(requested="valos")
...
class TestDelegationProviderIntegration(unittest.TestCase):
    ...
    def test_cross_provider_delegation(self, mock_creds, mock_cfg):
        """Parent on ValOs, subagent on OpenRouter — full credential switch."""
        mock_cfg.return_value = {
            "max_iterations": 45,
            "model": "google/gemini-3-flash-preview",
            "provider": "openrouter",
        ...
        parent = _make_mock_parent(depth=0)
        parent.provider = "valos"
        parent.base_url = "https://inference-api.valos-agent.com/v1"
        parent.api_key = "valos-key-abc"
        ...
        with patch("run_agent.AIAgent") as MockAgent:
            ...
            _, kwargs = MockAgent.call_args
            # Child should use OpenRouter, NOT ValOs
            self.assertEqual(kwargs["provider"], "openrouter")
            self.assertEqual(kwargs["base_url"], "https://openrouter.ai/api/v1")
