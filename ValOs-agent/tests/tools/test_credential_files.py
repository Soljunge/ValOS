...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        valos_home = tmp_path / ".valos-agent"
        valos_home.mkdir()
        (valos_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"VALOS_HOME": str(valos_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(valos_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.valos-agent/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        valos_home = tmp_path / ".valos-agent"
        valos_home.mkdir()
        (valos_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        valos_home = tmp_path / ".valos-agent"
        valos_home.mkdir()
        (valos_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        valos_home = tmp_path / ".valos-agent"
        valos_home.mkdir()
        (valos_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.valos-agent/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        valos_home = tmp_path / ".valos-agent"
        (valos_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"VALOS_HOME": str(valos_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.valos-agent")

        assert mounts[0]["container_path"] == "/home/user/.valos-agent/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.valos-agent/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.valos-agent/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape VALOS_HOME."""
        monkeypatch.setenv("VALOS_HOME", str(tmp_path / ".valos-agent"))
        (tmp_path / ".valos-agent").mkdir()

        # Create a sensitive file one level above valos_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        valos_home = tmp_path / ".valos-agent"
        valos_home.mkdir()
        monkeypatch.setenv("VALOS_HOME", str(valos_home))

        # Create a fake sensitive file outside valos_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside VALOS_HOME must still be registered."""
        valos_home = tmp_path / ".valos-agent"
        valos_home.mkdir()
        monkeypatch.setenv("VALOS_HOME", str(valos_home))
        (valos_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_valos_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of VALOS_HOME must be allowed."""
        valos_home = tmp_path / ".valos-agent"
        valos_home.mkdir()
        subdir = valos_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside VALOS_HOME pointing outside must be rejected."""
        valos_home = tmp_path / ".valos-agent"
        valos_home.mkdir()
        monkeypatch.setenv("VALOS_HOME", str(valos_home))

        # Create a sensitive file outside valos_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside valos_home pointing outside
        symlink = valos_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes VALOS_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, valos_home: Path, cred_files: list):
        import yaml
        config_path = valos_home / "config.yaml"
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape VALOS_HOME."""
        valos_home = tmp_path / ".valos-agent"
        valos_home.mkdir()
        monkeypatch.setenv("VALOS_HOME", str(valos_home))
        ...
        sensitive.write_text("{}")
        self._write_config(valos_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(valos_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside VALOS_HOME via config must still mount."""
        valos_home = tmp_path / ".valos-agent"
        valos_home.mkdir()
        monkeypatch.setenv("VALOS_HOME", str(valos_home))

        (valos_home / "oauth.json").write_text("{}")
        self._write_config(valos_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        valos_home = tmp_path / ".valos-agent"
        valos_home.mkdir()
        (valos_home / "cache" / "documents").mkdir(parents=True)
        (valos_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("VALOS_HOME", str(valos_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.valos-agent/cache/documents" in paths
        assert "/root/.valos-agent/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.valos-agent/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        valos_home = tmp_path / ".valos-agent"
        valos_home.mkdir()
        # Use legacy dir name — get_valos_dir prefers old if it exists
        (valos_home / "document_cache").mkdir()
        (valos_home / "image_cache").mkdir()
        monkeypatch.setenv("VALOS_HOME", str(valos_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(valos_home / "document_cache") in host_paths
        assert str(valos_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.valos-agent/cache/documents" in container_paths
        assert "/root/.valos-agent/cache/images" in container_paths

    def test_empty_valos_home(self, tmp_path, monkeypatch):
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        valos_home = tmp_path / ".valos-agent"
        doc_dir = valos_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        valos_home = tmp_path / ".valos-agent"
        ss_dir = valos_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.valos-agent/cache/screenshots/session_abc/screen1.png"
