...
class TestSendMessageTool:
    def test_cron_duplicate_target_is_skipped_and_explained(self):
        ...
        with patch.dict(
            os.environ,
            {
                "VALOS_CRON_AUTO_DELIVER_PLATFORM": "telegram",
                "VALOS_CRON_AUTO_DELIVER_CHAT_ID": "-1001",
            },
            clear=False,
        ), \
    ...
    def test_cron_same_chat_different_thread_still_sends(self):
        ...
        with patch.dict(
            os.environ,
            {
                "VALOS_CRON_AUTO_DELIVER_PLATFORM": "telegram",
                "VALOS_CRON_AUTO_DELIVER_CHAT_ID": "-1001",
                "VALOS_CRON_AUTO_DELIVER_THREAD_ID": "17585",
            },
            clear=False,
        ), \
             patch("gateway.config.load_gateway_config", return_value=config), \
        ...
        with patch("tools.send_message_tool._send_whatsapp", async_mock):
            result = asyncio.run(
                _send_to_platform(
                    ...
                    chat_id,
                    "hello from valos-agent",
                )
            )

        assert result["success"] is True
        async_mock.assert_awaited_once_with({"bridge_port": 3000}, chat_id, "hello from valos-agent")
