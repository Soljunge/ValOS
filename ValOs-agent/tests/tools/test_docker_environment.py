...
def test_execute_uses_valos_dotenv_for_allowlisted_env(monkeypatch):
    ...
    monkeypatch.setattr(docker_env, "_load_valos_env_vars", lambda: {"GITHUB_TOKEN": "value_from_dotenv"})
...
def test_execute_prefers_shell_env_over_valos_dotenv(monkeypatch):
    env = _make_execute_only_env(["GITHUB_TOKEN"])
    popen_calls = []
...
def test_forward_env_overrides_docker_env(monkeypatch):
    ...
    monkeypatch.setattr(docker_env, "_load_valos_env_vars", lambda: {})
    monkeypatch.setattr(docker_env.subprocess, "Popen", _fake_popen)
