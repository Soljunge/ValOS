...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        for key in (
            "VALOS_ENABLE_VALOS_MANAGED_TOOLS",
            "FIRECRAWL_API_KEY",
            "FIRECRAWL_API_URL",
        ...
        os.environ["VALOS_ENABLE_VALOS_MANAGED_TOOLS"] = "1"
    ...
    def test_no_config_raises_with_helpful_message(self):
        ...
        with patch("tools.web_tools.Firecrawl"):
            with patch("tools.web_tools._read_valos_access_token", return_value=None):
                from tools.web_tools import _get_firecrawl_client
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "valos-agent.com"}):
            with patch("tools.web_tools._read_valos_access_token", return_value="valos-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    from tools.web_tools import _get_firecrawl_client
                    result = _get_firecrawl_client()
                    mock_fc.assert_called_once_with(
                        api_key="valos-token",
                        api_url="https://firecrawl-gateway.valos-agent.com",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_tool_gateway_scheme_can_switch_derived_gateway_origin_to_http(self):
        ...
        with patch.dict(os.environ, {
            "TOOL_GATEWAY_DOMAIN": "valos-agent.com",
            ...
            with patch("tools.web_tools._read_valos_access_token", return_value="valos-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="valos-token",
                        api_url="http://firecrawl-gateway.valos-agent.com",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_default_gateway_domain_targets_valos_production_origin(self):
    ...
    def test_valos_auth_token_respects_valos_home_override(self, tmp_path):
        """Auth lookup should read from VALOS_HOME/auth.json, not ~/.valos-agent/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".valos-agent").mkdir(parents=True)

        valos_home = tmp_path / "valos-agent-home"
        valos_home.mkdir()
        (valos_home / "auth.json").write_text(json.dumps({
            "providers": {
                "valos": {
                    "access_token": "valos-token",
                }
            }
        }))
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "VALOS_HOME": str(valos_home),
        }, clear=False):
            import tools.web_tools
            importlib.reload(tools.web_tools)
            assert tools.web_tools._read_valos_access_token() == "valos-token"
...
class TestBackendSelection:
    ...
    ``valos-agent tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def setup_method(self):
        ...
        for key in self._ENV_KEYS:
            if key != "VALOS_ENABLE_VALOS_MANAGED_TOOLS":
                os.environ.pop(key, None)
