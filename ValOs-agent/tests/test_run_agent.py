...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._valos_home / "logs" / "errors.log").resolve()
...
class TestBuildSystemPrompt:
    ...
    def test_includes_valos_subscription_prompt(self, agent, monkeypatch):
        monkeypatch.setattr(run_agent, "build_valos_subscription_prompt", lambda tool_names: "VALOS SUBSCRIPTION BLOCK")
        prompt = agent._build_system_prompt()
        assert "VALOS SUBSCRIPTION BLOCK" in prompt
...
class TestBuildApiKwargs:
    ...
    def test_reasoning_sent_for_valos_route(self, agent):
        agent.base_url = "https://inference-api.valos-agent.com/v1"
        agent.model = "minimax/minimax-m2.5"
        messages = [{"role": "user", "content": "hi"}]
        kwargs = agent._build_api_kwargs(messages)
        assert kwargs["extra_body"]["reasoning"]["effort"] == "medium"
...
class TestRunConversation:
    ...
    def test_valos_401_refreshes_after_remint_and_retries(self, agent):
        self._setup_agent(agent)
        agent.provider = "valos"
        ...
        with (
            ...
            patch.object(
                agent, "_try_refresh_valos_client_credentials", side_effect=_fake_refresh
            ),
        ):
            result = agent.run_conversation("hello")
...
class TestValOsCredentialRefresh:
    """Verify ValOs credential refresh rebuilds the runtime client."""

    def test_try_refresh_valos_client_credentials_rebuilds_client(
        ...
        def _fake_resolve(**kwargs):
            ...
            return {
                "api_key": "new-valos-key",
                "base_url": "https://inference-api.valos-agent.com/v1",
            }
        ...
        monkeypatch.setattr(
            "valos_cli.auth.resolve_valos_runtime_credentials", _fake_resolve
        )

        agent.client = _ExistingClient()
        with patch("run_agent.OpenAI", side_effect=_fake_openai):
            ok = agent._try_refresh_valos_client_credentials(force=True)

        assert ok is True
        assert closed["value"] is True
        assert captured["force_mint"] is True
        assert rebuilt["kwargs"]["api_key"] == "new-valos-key"
        assert (
            rebuilt["kwargs"]["base_url"] == "https://inference-api.valos-agent.com/v1"
        )
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "ValOs" in agent._cached_system_prompt
