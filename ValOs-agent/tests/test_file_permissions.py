...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("valos_cli.config.get_config_path", return_value=config_path), \
             patch("valos_cli.config.ensure_valos_home"):
            from valos_cli.config import save_config
    ...
    def test_save_env_value_sets_0600(self):
        ...
        with patch("valos_cli.config.get_env_path", return_value=env_path), \
             patch("valos_cli.config.ensure_valos_home"):
            from valos_cli.config import save_env_value
            save_env_value("TEST_KEY", "test_value")

            file_mode = stat.S_IMODE(os.stat(env_path).st_mode)
            self.assertEqual(file_mode, 0o600)

    def test_ensure_valos_home_sets_0700(self):
        home = Path(self.tmpdir) / ".valos-agent"
        with patch("valos_cli.config.get_valos_home", return_value=home):
