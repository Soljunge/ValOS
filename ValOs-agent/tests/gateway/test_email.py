...
class TestConfigEnvOverrides(unittest.TestCase):
    ...
    @patch.dict(os.environ, {
        "EMAIL_ADDRESS": "valos-agent@test.com",
    ...
    def test_email_config_loaded_from_env(self):
        ...
        self.assertEqual(config.platforms[Platform.EMAIL].extra["address"], "valos-agent@test.com")
...
class TestToolset(unittest.TestCase):
    ...
    def test_email_toolset_exists(self):
        ...
        self.assertIn("valos-agent-email", TOOLSETS)

    def test_email_in_gateway_toolset(self):
        from toolsets import TOOLSETS
        includes = TOOLSETS["valos-agent-gateway"]["includes"]
        self.assertIn("valos-agent-email", includes)
...
class TestGatewaySetup(unittest.TestCase):
    ...
    def test_email_in_platforms_list(self):
        from valos_cli.gateway import _PLATFORMS
...
class TestDispatchMessage(unittest.TestCase):
    ...
    def test_self_message_filtered(self):
        ...
        msg_data = {
            "uid": b"1",
            "sender_addr": "valos-agent@test.com",
            "sender_name": "ValOs",
            "subject": "Test",
...
class TestThreadContext(unittest.TestCase):
    ...
    def test_no_thread_context_uses_default_subject(self):
        """Without thread context, subject should be 'Re: ValOs'."""
        adapter = self._make_adapter()
        ...
        with patch("smtplib.SMTP") as mock_smtp:
            ...
            send_call = mock_server.send_message.call_args[0][0]
            self.assertEqual(send_call["Subject"], "Re: ValOs")
...
class TestSendMethods(unittest.TestCase):
    ...
    def test_send_calls_smtp(self):
        ...
        with patch("smtplib.SMTP") as mock_smtp:
            ...
            result = asyncio.run(
                adapter.send("user@test.com", "Hello from ValOs!")
            )

            self.assertTrue(result.success)
            mock_server.starttls.assert_called_once()
            mock_server.login.assert_called_once_with("valos-agent@test.com", "secret")
...
class TestSendEmailStandalone(unittest.TestCase):
    ...
    def test_send_email_tool_success(self):
        ...
        with patch("smtplib.SMTP") as mock_smtp:
            ...
            result = asyncio.run(
                _send_email({"address": "valos-agent@test.com", "smtp_host": "smtp.test.com"}, "user@test.com", "Hello")
            )
