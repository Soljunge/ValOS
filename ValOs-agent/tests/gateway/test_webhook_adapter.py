...
class TestInsecureNoAuth:
    ...
    @pytest.mark.asyncio
    async def test_insecure_no_auth_rejected_on_public_bind_connect(self):
        """INSECURE_NO_AUTH is allowed for handler tests, but not public server startup."""
        routes = {"open": {"secret": _INSECURE_NO_AUTH, "prompt": "hello"}}
        adapter = _make_adapter(routes=routes, host="0.0.0.0")
...
class TestDeliveryCleanup:
    ...
    @pytest.mark.asyncio
    async def test_delivery_info_retained_after_send_until_ttl_prune(self):
        """send() keeps delivery_info so interim status messages do not consume it."""
        adapter = _make_adapter()
        ...
        adapter._delivery_info_created[chat_id] = time.time() - 3700
        adapter._prune_delivery_info(time.time())
...
class TestDirectDelivery:
    ...
    @pytest.mark.asyncio
    async def test_deliver_only_skips_agent_and_sends_directly(self):
        routes = {
            "alerts": {
                "secret": _INSECURE_NO_AUTH,
        ...
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                ...
                headers={"X-GitHub-Delivery": "direct-001"},
            )
            assert resp.status == 200
            data = await resp.json()
        ...
        mock_tg_adapter.send.assert_awaited_once_with(
            "12345", "Alert: Disk full", metadata=None
        )
