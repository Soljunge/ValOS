...
class TestSmsToolset:
    def test_valos_sms_toolset_exists(self):
        from toolsets import get_toolset

        ts = get_toolset("valos-agent-sms")
        assert ts is not None
        assert "tools" in ts

    def test_valos_sms_in_gateway_includes(self):
        from toolsets import get_toolset

        gw = get_toolset("valos-agent-gateway")
        assert gw is not None
        assert "valos-agent-sms" in gw["includes"]
