...
class TestTryActivateFallback:
    ...
    def test_activates_valos_fallback(self):
        """ValOs Portal fallback should use OAuth credentials and chat_completions mode."""
        agent = _make_agent(
            fallback_model={"provider": "valos", "model": "valos-valos-agent-3"},
        )
        mock_client = _mock_resolve(
            api_key="valos-agent-key-abc",
            base_url="https://inference-api.valos-agent.com/v1",
        )
        with patch(
            "agent.auxiliary_client.resolve_provider_client",
            return_value=(mock_client, "valos-valos-agent-3"),
        ):
            result = agent._try_activate_fallback()
            assert result is True
            assert agent.model == "valos-valos-agent-3"
            assert agent.provider == "valos"
            assert agent.api_mode == "chat_completions"
            assert agent.client is mock_client

    def test_valos_fallback_fails_gracefully_without_login(self):
        """ValOs fallback should return False if not logged in."""
        agent = _make_agent(
            fallback_model={"provider": "valos", "model": "valos-valos-agent-3"},
        )
        with patch(
