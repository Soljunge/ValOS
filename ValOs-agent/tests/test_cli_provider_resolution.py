...
from valos_cli.auth import AuthError
from valos_cli import main as valos_main
...
def test_valos_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in ValOsCLI.__init__")

    monkeypatch.setattr("valos_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("valos_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.ValOsCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("valos_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("valos_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_cli_turn_routing_uses_cheap_model_when_simple(monkeypatch):
    ...
    shell = cli.ValOsCLI(model="anthropic/claude-sonnet-4", compact=True, max_turns=1)
    shell.provider = "openrouter"
    shell.api_mode = "chat_completions"
    shell.base_url = "https://openrouter.ai/api/v1"
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    monkeypatch.setenv("VALOS_INFERENCE_PROVIDER", "openrouter")
    ...
    shell = cli.ValOsCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("valos_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "valos_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex", "gpt-5.1-codex-mini"],
    )

    shell = cli.ValOsCLI(compact=True, max_turns=1)
...
def test_model_flow_valos_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    monkeypatch.setenv("VALOS_ENABLE_VALOS_MANAGED_TOOLS", "1")
    config = {
        "model": {"provider": "valos", "default": "claude-opus-4-6"},
        "tts": {"provider": "elevenlabs"},
        "browser": {"cloud_provider": "browser-use"},
    }

    monkeypatch.setattr(
        "valos_cli.auth.get_provider_auth_state",
        lambda provider: {"access_token": "valos-token"},
    )
    monkeypatch.setattr(
        "valos_cli.auth.resolve_valos_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "valos_cli.auth.fetch_valos_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("valos_cli.auth._prompt_model_selection", lambda model_ids, current_model="": "claude-opus-4-6")
    monkeypatch.setattr("valos_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("valos_cli.auth._update_config_for_provider", lambda provider, url: None)
    monkeypatch.setattr(
        "valos_cli.valos_subscription.get_valos_subscription_explainer_lines",
        lambda: ["ValOs subscription enables managed web tools."],
    )

    valos_main._model_flow_valos(config, current_model="claude-opus-4-6")

    out = capsys.readouterr().out
    assert "ValOs subscription enables managed web tools." in out
    assert config["tts"]["provider"] == "elevenlabs"
    assert config["browser"]["cloud_provider"] == "browser-use"


def test_model_flow_valos_applies_managed_tts_default_when_unconfigured(monkeypatch, capsys):
    monkeypatch.setenv("VALOS_ENABLE_VALOS_MANAGED_TOOLS", "1")
    config = {
        "model": {"provider": "valos", "default": "claude-opus-4-6"},
        "tts": {"provider": "edge"},
    ...
    out = capsys.readouterr().out
    assert "ValOs subscription enables managed web tools." in out
    assert "OpenAI TTS via your ValOs subscription" in out
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.ValOsCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.ValOsCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    monkeypatch.setattr(
        "valos_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "invalid-provider"}},
    )
    monkeypatch.setattr("valos_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("valos_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("valos_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("valos_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(valos_main, "_prompt_provider_choice", lambda choices: len(choices) - 1)
    monkeypatch.setattr("sys.stdin", type("FakeTTY", (), {"isatty": lambda self: True})())

    valos_main.cmd_model(SimpleNamespace())
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "valos_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("valos_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("valos_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("valos_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("valos_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "valos_cli.models.probe_api_models",
...
def test_cmd_model_forwards_valos_login_tls_options(monkeypatch):
    monkeypatch.setattr(valos_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "valos_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "valos"}},
    )
    monkeypatch.setattr("valos_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("valos_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("valos_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("valos_cli.auth.resolve_provider", lambda requested, **kwargs: "valos")
    monkeypatch.setattr("valos_cli.auth.get_provider_auth_state", lambda provider_id: None)
    monkeypatch.setattr(valos_main, "_prompt_provider_choice", lambda choices: 0)
    ...
    monkeypatch.setattr("valos_cli.auth._login_valos", _fake_login)

    valos_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.valos-agent.com",
            inference_url="https://inference.valos-agent.com/v1",
            client_id="valos-agent-local",
