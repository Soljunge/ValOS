"""Tests that provider selection via `valos-agent model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated VALOS_HOME with a minimal string-format config."""
    ...
    monkeypatch.delenv("VALOS_MODEL", raising=False)
    monkeypatch.delenv("LLM_MODEL", raising=False)
    monkeypatch.delenv("VALOS_INFERENCE_PROVIDER", raising=False)
...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from valos_cli.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        from valos_cli.main import _model_flow_api_key_provider
        ...
        with patch("valos_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("valos_cli.auth.deactivate_provider"), \
    ...
    def test_copilot_provider_saved_when_selected(self, config_home):
        ...
        from valos_cli.main import _model_flow_copilot
        from valos_cli.config import load_config
        ...
        with patch(
            ...
            "valos_cli.auth._prompt_model_selection",
            return_value="gpt-5.4",
        ), patch(
            "valos_cli.main._prompt_reasoning_effort_selection",
            return_value="high",
        ), patch(
            "valos_cli.auth.deactivate_provider",
        ):
    ...
    def test_copilot_acp_provider_saved_when_selected(self, config_home):
        ...
        from valos_cli.main import _model_flow_copilot_acp
        from valos_cli.config import load_config

        with patch(
            "valos_cli.auth.get_external_process_provider_status",
            return_value={
            ...
            "valos_cli.auth.resolve_external_process_provider_credentials",
            return_value={
    ...
    def test_opencode_go_models_are_selectable_and_persist_normalized(self, config_home, monkeypatch):
        ...
        with patch("valos_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.7"]), \
             patch("valos_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("valos_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
    ...
    def test_opencode_go_same_provider_switch_recomputes_api_mode(self, config_home, monkeypatch):
        ...
        with patch("valos_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.5"]), \
             patch("valos_cli.auth._prompt_model_selection", return_value="minimax-m2.5"), \
             patch("valos_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
