from valos_cli import runtime_provider as rp
...
def test_resolve_runtime_provider_auto_uses_openai_when_only_openai_key_exists(monkeypatch):
    monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
...
def test_named_custom_provider_does_not_shadow_builtin_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            ...
            "api_key": "valos-runtime-key",
            "source": "portal",
            "expires_at": None,
        },
    )

    resolved = rp.resolve_runtime_provider(requested="valos")

    assert resolved["provider"] == "valos"
    assert resolved["base_url"] == "https://inference-api.valos-agent.com/v1"
    assert resolved["api_key"] == "valos-runtime-key"
    assert resolved["requested_provider"] == "valos"
...
def test_resolve_provider_custom_returns_custom():
    ...
    from valos_cli.auth import resolve_provider
...
def test_resolve_provider_prefers_openai_when_only_openai_key_is_set(monkeypatch):
    """OPENAI_API_KEY should resolve to the direct OpenAI provider."""
    from valos_cli.auth import resolve_provider
