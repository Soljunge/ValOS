"""
ValOsAgentBaseEnv -- Abstract Base Environment for ValOs-Agent + Atropos

Provides the Atropos integration plumbing that all valos-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via ValOsAgentLoop
...
# Ensure the valos-agent repo root is on sys.path so that imports like
...
# Load API keys from valos-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, ValOsAgentLoop
from environments.tool_context import ToolContext

# Import valos-agent toolset infrastructure
...
class ValOsAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for valos-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of valos-agent toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: valos-agent, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
...
class ValOsAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for valos-agent Atropos integration.
    ...
    name: Optional[str] = "valos-agent"
    env_config_cls = ValOsAgentEnvConfig

    def __init__(
        self,
        config: ValOsAgentEnvConfig,
        ...
        # Set terminal environment variables so valos-agent tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY valos-agent tool (terminal, file, web,
