"""
ValOsSweEnv -- SWE-Bench Style Environment with Modal Sandboxes
...
Usage:
    ...
    vllm serve YourModel --tool-parser valos-agent
    run-api
    python environments/valos_swe_env.py serve \\
    ...
    python environments/valos_swe_env.py serve \\
        ...
        --env.tool_call_parser valos-agent \\
        --env.terminal_backend modal
"""
...
class ValOsSweEnv(ValOsAgentBaseEnv):
    ...
    name = "valos-agent-swe"
    env_config_cls = ValOsSweEnvConfig

    @classmethod
    def config_init(cls) -> Tuple[ValOsSweEnvConfig, List[APIServerConfig]]:
        """
        ...
        env_config = ValOsSweEnvConfig(
            ...
            # Atropos settings
            group_size=4,
            tokenizer_name="horatiu.sol/DeepValOs-3-Llama-3-3B-Preview",
            tool_call_parser="valos-agent",
            steps_per_eval=50,
            total_steps=500,
            use_wandb=True,
            wandb_name="valos-agent-swe",
        )

        server_configs = [
            APIServerConfig(
                base_url="http://localhost:8000/v1",
                model_name="horatiu.sol/DeepValOs-3-Llama-3-3B-Preview",
