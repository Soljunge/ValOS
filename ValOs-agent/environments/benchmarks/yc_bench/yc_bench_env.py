...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs ValOsAgentLoop with terminal tool only
...
Requires: pip install valos-agent[yc-bench]
"""
...
class YCBenchEvalConfig(ValOsAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends ValOsAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(ValOsAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The ValOsAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs ValOsAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our ValOsAgentLoop.
