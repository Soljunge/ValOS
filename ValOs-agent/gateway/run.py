...
# Resolve ValOs home directory (respects VALOS_HOME override)
from valos_constants import get_valos_home
from utils import atomic_yaml_write
_valos_home = get_valos_home()

# Load environment variables from ~/.valos-agent/.env first.
# User-managed env files should override stale shell exports on restart.
from dotenv import load_dotenv  # backward-compat for tests that monkeypatch this symbol
from valos_cli.env_loader import load_valos_dotenv
_env_path = _valos_home / '.env'
load_valos_dotenv(valos_home=_valos_home, project_env=Path(__file__).resolve().parents[1] / '.env')

# Bridge config.yaml values into the environment so os.getenv() picks them up.
# config.yaml is authoritative for terminal settings — overrides .env.
_config_path = _valos_home / 'config.yaml'
    ...
    try:
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            if "max_turns" in _agent_cfg:
                os.environ["VALOS_MAX_ITERATIONS"] = str(_agent_cfg["max_turns"])
        # Timezone: bridge config.yaml → VALOS_TIMEZONE env var.
        # VALOS_TIMEZONE from .env takes precedence (already in os.environ).
        _tz_cfg = _cfg.get("timezone", "")
        if _tz_cfg and isinstance(_tz_cfg, str) and "VALOS_TIMEZONE" not in os.environ:
            os.environ["VALOS_TIMEZONE"] = _tz_cfg.strip()
        # Security settings
        _security_cfg = _cfg.get("security", {})
        if isinstance(_security_cfg, dict):
            _redact = _security_cfg.get("redact_secrets")
            if _redact is not None:
                os.environ["VALOS_REDACT_SECRETS"] = str(_redact).lower()
    except Exception:
        pass  # Non-fatal; gateway can still run with .env values

# Gateway runs in quiet mode - suppress debug output and use cwd directly (no temp dirs)
os.environ["VALOS_QUIET"] = "1"

# Enable interactive exec approval for dangerous commands on messaging platforms
os.environ["VALOS_EXEC_ASK"] = "1"
...
from gateway.channel_policy import resolve_channel_policy, should_accept_by_policy
...
def _expand_whatsapp_auth_aliases(identifier: str) -> set:
    ...
    session_dir = _valos_home / "whatsapp" / "session"
    resolved = set()
    queue = [normalized]
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    try:
        runtime = resolve_runtime_provider(
            requested=os.getenv("VALOS_INFERENCE_PROVIDER"),
        )
            ...
            for skill_md in skills_dir.rglob("SKILL.md"):
                ...
                if name == normalized and name in disabled:
                    return (
                        ...
                        f"Enable it with: `valos-agent skills config`"
                    )

        # Check optional skills (shipped with repo but not installed)
        from valos_constants import get_valos_home, get_optional_skills_dir
            ...
            for skill_md in optional_dir.rglob("SKILL.md"):
                ...
                if name == normalized:
                    ...
                    return (
                        ...
                        f"Install it with: `valos-agent skills install {install_path}`"
                    )
    except Exception:
        pass
    return None
...
def _load_gateway_config() -> dict:
    """Load and parse ~/.valos-agent/config.yaml, returning {} on any error."""
    try:
        config_path = _valos_home / 'config.yaml'
        if config_path.exists():
            import yaml
            with open(config_path, 'r', encoding='utf-8') as f:
                return yaml.safe_load(f) or {}
    except Exception:
        logger.debug("Could not load gateway config from %s", _valos_home / 'config.yaml')
    return {}
...
def _resolve_valos_bin() -> Optional[list[str]]:
    """Resolve the ValOs update command as argv parts.

    Tries in order:
    1. ``shutil.which("valos-agent")`` — standard PATH lookup
    2. ``sys.executable -m valos_cli.main`` — fallback when ValOs is running
       from a venv/module invocation and the ``valos-agent`` shim is not on PATH
    ...
    try:
        ...
        if importlib.util.find_spec("valos_cli") is not None:
            return [sys.executable, "-m", "valos_cli.main"]
    except Exception:
        pass
...
class GatewayRunner:
    ...
    def _has_setup_skill(self) -> bool:
        """Check if the valos-agent-setup skill is installed."""
        try:
            from tools.skill_manager_tool import _find_skill
            return _find_skill("valos-agent-setup") is not None
        except Exception:
            return False

    # -- Voice mode persistence ------------------------------------------

    _VOICE_MODE_PATH = _valos_home / "gateway_voice_mode.json"
    ...
    def _load_prefill_messages() -> List[Dict[str, Any]]:
        ...
        Checks VALOS_PREFILL_MESSAGES_FILE env var first, then falls back to
        the prefill_messages_file key in ~/.valos-agent/config.yaml.
        Relative paths are resolved from ~/.valos-agent/.
        """
        import json as _json
        file_path = os.getenv("VALOS_PREFILL_MESSAGES_FILE", "")
        if not file_path:
            try:
                import yaml as _y
                cfg_path = _valos_home / "config.yaml"
                if cfg_path.exists():
    ...
    def _load_ephemeral_system_prompt() -> str:
        ...
        Checks VALOS_EPHEMERAL_SYSTEM_PROMPT env var first, then falls back to
        agent.system_prompt in ~/.valos-agent/config.yaml.
        """
        prompt = os.getenv("VALOS_EPHEMERAL_SYSTEM_PROMPT", "")
        if prompt:
            return prompt
        try:
            import yaml as _y
    ...
    def _load_reasoning_config() -> dict | None:
        ...
        VALOS_REASONING_EFFORT as a fallback. Valid: "xhigh", "high",
        ...
        try:
            ...
            pass
        if not effort:
            effort = os.getenv("VALOS_REASONING_EFFORT", "")
    ...
    def _load_background_notifications_mode() -> str:
        ...
        """
        mode = os.getenv("VALOS_BACKGROUND_NOTIFICATIONS", "")
        if not mode:
            try:
                import yaml as _y
                cfg_path = _valos_home / "config.yaml"
    ...
    async def start(self) -> bool:
        ...
        """
        logger.info("Starting ValOs Gateway...")
        ...
        _allow_all = os.getenv("GATEWAY_ALLOW_ALL_USERS", "").lower() in ("true", "1", "yes") or any(
            ...
            for v in allow_all_env_names()
        )
        if not _any_allowlist and not _allow_all:
            logger.warning(
                "No user allowlists configured. All unauthorized users will be denied. "
                "Set GATEWAY_ALLOW_ALL_USERS=true in ~/.valos-agent/.env to allow open access, "
        ...
        if not notified and any(
            path.exists()
            for path in (
                _valos_home / ".update_pending.json",
                _valos_home / ".update_pending.claimed.json",
            )
        ):
    ...
    def _create_adapter(
        ...
        registration = get_channel_registration(platform)
        if not registration:
            logger.warning("No channel registration available for %s", platform.value)
            return None
        if not registration.is_supported:
            logger.warning("%s is registered but its ValOs adapter is not implemented yet", registration.label)
            return None

        module = importlib.import_module(registration.adapter_import or "")
        requirement_check = getattr(module, registration.requirements_function or "", None)
        if callable(requirement_check) and not requirement_check():
            logger.warning(registration.requirements_warning or "%s requirements are not met", registration.label)
            return None

        adapter_cls = getattr(module, registration.adapter_class or "")
    ...
    def _is_user_authorized(self, source: SessionSource) -> bool:
        ...
        5. Default: deny
        """
        registration = get_channel_registration(source.platform)
        if registration and registration.system_authenticated:
            return True
        ...
        platform_allow_all_var = registration.allow_all_env if registration else ""
        ...
        platform_allowlist = os.getenv(
            registration.allowed_users_env if registration else "",
            "",
        ).strip()
        global_allowlist = os.getenv("GATEWAY_ALLOWED_USERS", "").strip()
        gateway_config = getattr(self, "config", None)
        platform_config = (
            gateway_config.platforms.get(source.platform)
            if gateway_config
            else None
        )
        if platform_config:
            policy = resolve_channel_policy(platform_config)
            policy_decision = should_accept_by_policy(
            ...
            if policy_decision is True:
                return True
            if policy_decision is False and not platform_allowlist and not global_allowlist:
                return False
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
                ...
                if code:
                    ...
                    if adapter:
                        await adapter.send(
                            ...
                            f"`valos-agent pairing approve {platform_name} {code}`"
                        )
                else:
                    adapter = self.adapters.get(source.platform)
                    if adapter:
        ...
        _raw_stale_timeout = float(os.getenv("VALOS_AGENT_TIMEOUT", 600))
        ...
        if _quick_key in self._running_agents:
            ...
            from valos_cli.commands import resolve_command as _resolve_cmd_inner
        ...
        # in valos_cli/commands.py — no hardcoded set to maintain here.
        from valos_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command as _resolve_cmd
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        if history and len(history) >= 4:
            ...
            _hyg_api_key = None
            try:
                _hyg_cfg_path = _valos_home / "config.yaml"
            ...
            if not os.getenv(env_key):
                ...
                if adapter:
                    await adapter.send(
                        ...
                        f"A home channel is where ValOs delivers cron job results "
                    ...
                    if _stt_adapter:
                        try:
                            _stt_msg = (
                                ...
                                "(`pip install faster-whisper` in the ValOs venv) "
                                "and set `stt.enabled: true` in config.yaml, "
                                "then /restart the gateway."
                            )
                            # Point to setup skill if it's installed
                            if self._has_setup_skill():
                                _stt_msg += "\n\nFor full setup instructions, type: `/skill valos-agent-setup`"
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        # Detect profile name from VALOS_HOME path
        # Profile paths look like: ~/.valos-agent/profiles/<name>
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from valos_cli.commands import gateway_help_lines
        lines = [
            "📖 **ValOs Commands**\n",
            *gateway_help_lines(),
        ]
    ...
    async def _handle_provider_command(self, event: MessageEvent) -> str:
        ...
        lines.append("Setup: `valos-agent setup`")
        return "\n".join(lines)
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return "No personalities configured in `~/.valos-agent/config.yaml`"
        ...
        try:
            ...
            if "pynacl" in err_lower or "nacl" in err_lower or "davey" in err_lower:
                return (
                    ...
                    "Install or reinstall ValOs with the messaging extra, e.g. "
                    "`pip install valos-agent[messaging]`."
                )
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _valos_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update ValOs to the latest version.

        Spawns ``valos-agent update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``valos-agent update`` may trigger. Marker
        ...
        import subprocess
        from datetime import datetime
        from valos_cli.config import is_managed, format_managed_message

        if is_managed():
            return f"✗ {format_managed_message('update ValOs')}"
        ...
        valos_cmd = _resolve_valos_bin()
        if not valos_cmd:
            return (
                "✗ Could not locate the `valos-agent` command. "
                "ValOs is running, but the update command could not find the "
                "executable on PATH or via the current Python interpreter. "
                "Try running `valos-agent update` manually in your terminal."
            )

        pending_path = _valos_home / ".update_pending.json"
        output_path = _valos_home / ".update_output.txt"
        exit_code_path = _valos_home / ".update_exit_code"
        pending = {
        ...
        # Spawn `valos-agent update` detached so it survives gateway restart.
        # Use setsid for portable session detach (works under system services
        # where systemd-run --user fails due to missing D-Bus session).
        valos_cmd_str = " ".join(shlex.quote(part) for part in valos_cmd)
        update_cmd = (
            f"{valos_cmd_str} update > {shlex.quote(str(output_path))} 2>&1; "
        ...
        self._schedule_update_notification_watch()
        return "⚕ Starting ValOs update… I'll notify you when it's done."
    ...
    async def _watch_for_update_completion(
        ...
        """Wait for ``valos-agent update`` to finish, then send its notification."""
        pending_path = _valos_home / ".update_pending.json"
        claimed_path = _valos_home / ".update_pending.claimed.json"
        exit_code_path = _valos_home / ".update_exit_code"
            ...
            if adapter and chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    if exit_code == 0:
                        msg = f"✅ ValOs update finished.\n\n```\n{output}\n```"
                    else:
                        msg = f"❌ ValOs update failed.\n\n```\n{output}\n```"
                else:
                    if exit_code == 0:
                        msg = "✅ ValOs update finished successfully."
                    else:
                        msg = "❌ ValOs update failed. Check the gateway logs or run `valos-agent update` manually for details."
    ...
    def _set_session_env(self, context: SessionContext) -> None:
        ...
        os.environ["VALOS_SESSION_PLATFORM"] = context.source.platform.value
        os.environ["VALOS_SESSION_CHAT_ID"] = context.source.chat_id
        if context.source.chat_name:
            os.environ["VALOS_SESSION_CHAT_NAME"] = context.source.chat_name
        if context.source.thread_id:
            os.environ["VALOS_SESSION_THREAD_ID"] = str(context.source.thread_id)
    
    def _clear_session_env(self) -> None:
        """Clear session environment variables."""
        for var in ["VALOS_SESSION_PLATFORM", "VALOS_SESSION_CHAT_ID", "VALOS_SESSION_CHAT_NAME", "VALOS_SESSION_THREAD_ID"]:
    ...
    async def _enrich_message_with_transcription(
        ...
        if not getattr(self.config, "stt_enabled", True):
            ...
            if self._has_setup_skill():
                disabled_note += (
                    " You have a skill called valos-agent-setup that can help "
                    "users configure ValOs features including voice, tools, and more."
                )
            disabled_note += "]"
            if user_text:
            ...
            try:
                ...
                if result["success"]:
                    ...
                    if (
                        ...
                        if self._has_setup_skill():
                            _no_stt_note += (
                                " You have a skill called valos-agent-setup "
                                "that can help users configure ValOs features "
    ...
    async def _run_agent(
        ...
        progress_mode = (
            _raw_tp
            or os.getenv("VALOS_TOOL_PROGRESS_MODE")
            or "all"
        )
        ...
        def run_sync():
            ...
            os.environ["VALOS_SESSION_KEY"] = session_key or ""
        ...
        try:
            ...
            _agent_timeout_raw = float(os.getenv("VALOS_AGENT_TIMEOUT", 600))
            ...
            try:
                ...
                response = {
                    "final_response": (
                        ...
                        "To increase the limit, set VALOS_AGENT_TIMEOUT in your .env "
...
def _start_cron_ticker(stop_event: threading.Event, adapters=None, interval: int = 60):
    ...
    needing a separate `valos-agent cron daemon` or system cron entry.
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Prevent two gateways from running under the same VALOS_HOME.
    # The PID file is scoped to VALOS_HOME, so future multi-profile
    # setups (each profile using a distinct VALOS_HOME) will naturally
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            valos_home = str(get_valos_home())
            logger.error(
                "Another gateway instance is already running (PID %d, VALOS_HOME=%s). "
                "Use 'valos-agent gateway restart' to replace it, or 'valos-agent gateway stop' first.",
                existing_pid, valos_home,
            )
            print(
                f"\n❌ Gateway already running (PID {existing_pid}).\n"
                f"   Use 'valos-agent gateway restart' to replace it,\n"
                f"   or 'valos-agent gateway stop' to kill it first.\n"
                f"   Or use 'valos-agent gateway run --replace' to auto-replace.\n"
            )
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="ValOs Gateway - Multi-platform messaging")
