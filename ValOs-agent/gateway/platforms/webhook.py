...
Each route defines:
  ...
  - deliver_only: if true, skip the agent and deliver the rendered prompt
...
_BUILTIN_DELIVER_PLATFORMS = {
    ...
    "feishu", "wecom", "bluebubbles", "googlechat", "imessage", "irc",
    "msteams", "line", "nextcloud_talk",
}
...
class WebhookAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Delivery info keyed by session chat_id. Keep it available for
        # interim status sends and clean it by TTL instead of popping on send.
    ...
    async def send(
        ...
        chat_id is ``webhook:{route}:{delivery_id}``. Delivery info is not
        popped here because status messages can arrive before the final agent
        response. TTL cleanup happens on POST.
        """
    ...
    def _reload_dynamic_routes(self) -> None:
        ...
        valos_home = _Path(
            os.getenv("VALOS_HOME", str(_Path.home() / ".valos-agent"))
        ).expanduser()
        subs_path = valos_home / _DYNAMIC_ROUTES_FILENAME
        ...
        try:
            ...
            if not isinstance(data, dict):
                return
            # Merge: static routes take precedence over dynamic ones. Dynamic
            # routes must still fail closed on auth.
            ...
            for k, v in data.items():
                ...
                if not effective_secret:
                    logger.warning(
                        "[webhook] Dynamic route '%s' skipped: missing secret. "
                        "Set a valid HMAC secret, or use '%s' for local tests.",
                        k,
                        _INSECURE_NO_AUTH,
                    )
                    continue
    ...
    async def _handle_webhook(self, request: "web.Request") -> "web.Response":
        ...
        # Validate HMAC signature before consuming rate-limit budget.
        ...
        # Store delivery info for send(); TTL cleanup keeps it bounded.
        deliver_config = {
            "deliver": route_config.get("deliver", "log"),
            "deliver_extra": self._render_delivery_extra(
                route_config.get("deliver_extra", {}), payload
    ...
    def _render_prompt(
        ...
        Special token ``{__raw__}`` dumps the entire payload as indented JSON
