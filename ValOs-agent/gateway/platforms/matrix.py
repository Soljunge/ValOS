...
# Uses get_valos_home() so each profile gets its own Matrix store.
from valos_constants import get_valos_dir as _get_valos_dir
_STORE_DIR = _get_valos_dir("platforms/matrix/store", "matrix/store")
...
_KEY_EXPORT_PASSPHRASE = "valos-agent-matrix-e2ee-keys"
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    async def _run_e2ee_maintenance(self) -> None:
        ...
        ValOs uses a custom sync loop instead of matrix-nio's sync_forever(),
