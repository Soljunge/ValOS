...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_valos_home() / "sessions")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        sessions_dir = get_valos_home() / "sessions"
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.valos-agent/config.yaml (primary user-facing config)
    3. ~/.valos-agent/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_valos_home()
    gw_data: dict = {}
    ...
    from gateway.channel_registry import CHANNEL_REGISTRY

    _token_env_names = {
        platform: entry.token_env
