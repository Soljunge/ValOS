#!/usr/bin/env python3
"""OpenClaw -> ValOs migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into ValOs, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into ValOs.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into ValOs memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into ValOs memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import ValOs-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of ValOs-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into ValOs command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.valos-agent/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.valos-agent/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into ValOs .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into ValOs .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into ValOs .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into ValOs .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into ValOs .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into ValOs config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into ValOs config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into ValOs.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into ValOs MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into ValOs config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'valos-agent cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into ValOs config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into ValOs session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into ValOs custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into ValOs config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into ValOs config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into ValOs config.yaml approvals section.",
    },
    "memory-backend": {
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update ValOs config.yaml")
    ensure_parent(path)
    path.write_text(
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> ValOs Migration Report",
        "",
...
class Migrator:
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "ValOs config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No ValOs-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted ValOs-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
        ...
        for oc_key, valos_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and valos_key not in secret_additions:
                secret_additions[valos_key] = val
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        valos_config = load_yaml_file(destination)
        current_model = valos_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = valos_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                valos_config["model"] = {"default": model_str}
            dump_yaml_file(destination, valos_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        valos_config = load_yaml_file(destination)
        existing_tts = valos_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct ValOs destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected ValOs-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected ValOs-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        valos_cfg_path = self.target_root / "config.yaml"
        valos_cfg = load_yaml_file(valos_cfg_path)
        existing_mcp = valos_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in ValOs config")
                continue

            valos_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                valos_srv["command"] = srv["command"]
                if srv.get("args"):
                    valos_srv["args"] = srv["args"]
                if srv.get("env"):
                    valos_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    valos_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                valos_srv["url"] = srv["url"]
                if srv.get("headers"):
                    valos_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    valos_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                valos_srv["enabled"] = False
            if srv.get("timeout"):
                valos_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                valos_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                valos_srv["tools"] = {}
                if tools_cfg.get("include"):
                    valos_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    valos_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(valos_cfg_path)
            valos_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(valos_cfg_path, valos_cfg)
    ...
    def migrate_cron_jobs(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self.archive_dir and self.execute:
            ...
            self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                        "Cron config archived. Use 'valos-agent cron' to recreate jobs manually.")
        else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = valos_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> ValOs reasoning_effort
        ...
        if compaction:
            compression = valos_cfg.get("compression") or {}
            ...
            valos_cfg["compression"] = compression
            changes = True

        # Map humanDelay
        human_delay = defaults.get("humanDelay") or {}
        if human_delay:
            hd = valos_cfg.get("human_delay") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'valos-agent gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("VALOS_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        valos_cfg_path = self.target_root / "config.yaml"
        valos_cfg = load_yaml_file(valos_cfg_path)
        sr = valos_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        valos_cfg_path = self.target_root / "config.yaml"
        valos_cfg = load_yaml_file(valos_cfg_path)
        custom_providers = valos_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(valos_cfg_path)
            valos_cfg["custom_providers"] = custom_providers
            dump_yaml_file(valos_cfg_path, valos_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to ValOs config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            valos_cfg_path = self.target_root / "config.yaml"
            valos_cfg = load_yaml_file(valos_cfg_path)
            discord_valos = valos_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_valos["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_valos["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                valos_cfg["discord"] = discord_valos
                dump_yaml_file(valos_cfg_path, valos_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        valos_cfg_path = self.target_root / "config.yaml"
        valos_cfg = load_yaml_file(valos_cfg_path)
        browser_valos = valos_cfg.get("browser") or {}
        changed = False

        # Map fields that have ValOs equivalents
        if browser.get("cdpUrl"):
            browser_valos["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_valos["headless"] = browser["headless"]
            changed = True

        if changed:
            valos_cfg["browser"] = browser_valos
            if self.execute:
                self.maybe_backup(valos_cfg_path)
                dump_yaml_file(valos_cfg_path, valos_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            valos_mode = mode_map.get(mode, "manual")
            valos_cfg.setdefault("approvals", {})["mode"] = valos_mode
            if self.execute:
                self.maybe_backup(valos_cfg_path)
                dump_yaml_file(valos_cfg_path, valos_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{valos_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> ValOs Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in ValOs. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing ValOs Config Not Overwritten)",
                "",
                "These items already existed in your ValOs config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the ValOs agent discovers these",
            "directories, it may read/write to them instead of the ValOs state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `valos-agent claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `valos-agent claw cleanup` to fix it.",
            "",
            "## ValOs-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `valos-agent claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `valos-agent setup` to configure any remaining settings",
            "- Run `valos-agent mcp list` to verify MCP servers were imported correctly",
            "- Run `valos-agent cron` to recreate scheduled tasks (see archive/cron-config.json)",
            "- Run `valos-agent gateway install` if you need the gateway service",
            "- Review `~/.valos-agent/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into ValOs.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=str(Path.home() / ".valos-agent"), help="ValOs home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing ValOs targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of ValOs-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print(f"  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> ValOs Migration   [{mode_label:>8s}]   ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.valos-agent/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.valos-agent/config.yaml")
        print("    2. Run: valos-agent mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: valos-agent cron")
