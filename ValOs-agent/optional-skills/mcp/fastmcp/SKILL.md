...
- smoke-test a server with the FastMCP CLI before wiring it into ValOs or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to ValOs. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.valos-agent/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.valos-agent/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is ValOs integration, either:

- configure the server in `~/.valos-agent/config.yaml` using the `native-mcp` skill, or
...
### ValOs cannot see the deployed server

The server-building part may be correct while the ValOs config is not. Load the `native-mcp` skill and configure the server in `~/.valos-agent/config.yaml`, then restart ValOs.
