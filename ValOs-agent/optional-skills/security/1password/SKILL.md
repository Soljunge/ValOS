...
version: 1.0.0
author: arceus77-7, enhanced by ValOs
license: MIT
metadata:
  valos-agent:
...
- `tmux` available for stable authenticated sessions during ValOs terminal calls (desktop app flow only)
...
### Service Account (recommended for ValOs)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.valos-agent/.env` (the skill will prompt for this on first load).
...
## ValOs Execution Pattern (desktop app flow)

ValOs terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/valos-agent-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/valos-agent-op.sock"
