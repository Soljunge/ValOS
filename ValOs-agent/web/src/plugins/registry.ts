 ...
 *
 * Plugins call window.__VALOS_PLUGINS__.register(name, Component)
...
import { Card, CardHeader, CardTitle, CardContent } from "@/ui/ui/components/card";
...
import { Tabs, TabsList, TabsTrigger } from "@/ui/ui/components/tabs";
 ...
 * Exposed at runtime as ``window.__VALOS_PLUGIN_SDK__.sdkVersion`` so a
...
export function exposePluginSDK() {
  window.__VALOS_PLUGINS__ = {
    register: registerPlugin,
    registerSlot,
  };

  window.__VALOS_PLUGIN_SDK__ = {
    ...
    // window.__VALOS_SESSION_TOKEN__ directly.
    authedFetch,
    ...
    // UI components — ValOs DS where available, shadcn/ui primitives elsewhere.
    components: {
      Card,
      CardHeader,
      CardTitle,
