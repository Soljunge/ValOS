/**
 * ValOs Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__VALOS_PLUGIN_SDK__`` and
 * ``window.__VALOS_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__VALOS_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, ``@/ui``, …). Deriving the type would
 ...
 *
 * Versioning: bump ``ValOsPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@valos/dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls @/ui types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__VALOS_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__VALOS_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__VALOS_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface ValOsPluginSDK {
  ...
  /**
   * Shared UI primitives (ValOs DS / shadcn). Typed permissively at the
...
declare global {
  interface Window {
    __VALOS_PLUGIN_SDK__?: ValOsPluginSDK;
    __VALOS_PLUGINS__?: PluginRegistry;
  }
}
