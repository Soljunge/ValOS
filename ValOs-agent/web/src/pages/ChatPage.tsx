/**
 * ChatPage — embeds `valos --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (valos_cli/web_server.py)                          .
 *          ▼                                                              .
...
import { Typography } from "@/ui/ui/components/typography/index";
import { VALOS_BASE_PATH, buildWsAuthParam } from "@/lib/api";
...
function buildWsUrl(
  ...
  return `${proto}//${window.location.host}${VALOS_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__VALOS_SESSION_TOKEN__ &&
    !window.__VALOS_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `valos dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__VALOS_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner ValOs TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[valos-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__VALOS_AUTH_REQUIRED__``) this
    ...
    // forwarding them into ValOs. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="valos-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
