 ...
 * `_BUILTIN_DASHBOARD_THEMES` list in `valos_cli/web_server.py`.
 */
...
export const defaultTheme: DashboardTheme = {
  name: "default",
  label: "ValOs Teal",
  description: "Classic dark teal — the canonical ValOs look",
  palette: {
...
/**
 * ValOs Blue — the inverted "light mode" ValOs look, ported from the
 * LENS_5I overlay preset in `@/ui`.
 *
 ...
 * the orange midground reads as #0053FD ValOs-blue on screen, against a
 * cream `#E8F2FD` canvas.
 *
 * Note on bg blend mode: the DS Lens uses `multiply` for LENS_5I because
 * valos-web's <body> is white; valos-agent's App root is `bg-black`,
 ...
 */
export const valosBlueTheme: DashboardTheme = {
  name: "valos-blue",
  label: "ValOs Blue",
  description: "Light mode — vivid ValOs-blue accents on cream canvas",
  palette: {
    background: { hex: "#170d02", alpha: 1 },
    midground: { hex: "#FFAC02", alpha: 1 },
    foreground: { hex: "#FFFFFF", alpha: 1 },
    // Same warm-amber as valos-web's overlay glow; after the FG
  ...
  // bright ValOs-blue canvas — the coral is the "red" users see for
  // Output values without these overrides. Source → on-screen:
  //   Input:  #ffe6cb → #001934 (dark navy)        ← unchanged
  //   Output: #ffac02 → #0053fd (vivid ValOs-blue)  ← brand accent
  ...
  // ValOs-blue so the "primary" series in token-flow charts reads as
  ...
  //   white → vivid ValOs-blue → cream/light-blue
...
export const defaultLargeTheme: DashboardTheme = {
  name: "default-large",
  label: "ValOs Teal (Large)",
  description: "ValOs Teal with bigger fonts and roomier spacing",
