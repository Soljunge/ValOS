...
// (e.g. https://mission-control.tilos.com/valos/). The Python backend
// injects ``window.__VALOS_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__VALOS_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const VALOS_BASE_PATH = readBasePath();
const BASE = VALOS_BASE_PATH;
...
declare global {
  interface Window {
    __VALOS_SESSION_TOKEN__?: string;
    __VALOS_BASE_PATH__?: string;
    ...
    __VALOS_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-ValOs-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__VALOS_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``valos update``, ``valos gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__VALOS_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__VALOS_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("valos.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("valos.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__VALOS_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("valos.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__VALOS_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the ValOs dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``valos_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__VALOS_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-ValOs-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``valos_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__VALOS_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateValOs: () =>
    fetchJSON<ActionResponse>("/api/valos/update", { method: "POST" }),
  checkValOsUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/valos/update/check${force ? "?force=true" : ""}`,
    ),
 ...
 * ValOs Portal contract V1 (the access token has no email/name claims —
...
export interface SkillHubSource {
  ...
  /** valos-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
  ...
  /** Phase 7: registered ``DashboardAuthProvider`` names (e.g. ``["valos"]``).
