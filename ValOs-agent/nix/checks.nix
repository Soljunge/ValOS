...
{ inputs, ... }: {
  perSystem = { pkgs, system, lib, ... }:
    let
      valos-agent = inputs.self.packages.${system}.default;
      ...
      configKeys = pkgs.runCommand "valos-agent-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${valosVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = lib.optionalAttrs pkgs.stdenv.hostPlatform.isLinux {
        ...
        package-contents = pkgs.runCommand "valos-agent-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${valos-agent}/bin/valos-agent || (echo "FAIL: valos-agent binary missing"; exit 1)
          test -x ${valos-agent}/bin/valos-agent || (echo "FAIL: valos-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${valos-agent}/bin/valos-agent version 2>&1 | grep -qi "valos-agent" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "valos-agent-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in valos-agent valos-agent valos-agent-acp; do
            test -x ${valos-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "valos-agent-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking valos-agent --help ==="
          ${valos-agent}/bin/valos-agent --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${valos-agent}/bin/valos-agent --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "valos-agent-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${valos-agent}/share/valos-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${valos-agent}/share/valos-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "VALOS_BUNDLED_SKILLS" ${valos-agent}/bin/valos-agent || \
            (echo "FAIL: VALOS_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: VALOS_BUNDLED_SKILLS set in wrapper"
        ...
        # Verify VALOS_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "valos-agent-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(VALOS_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking VALOS_MANAGED guards ==="
          check_blocked "config set" ${valos-agent}/bin/valos-agent config set model foo
          check_blocked "config edit" ${valos-agent}/bin/valos-agent config edit
        ...
        in pkgs.runCommand "valos-agent-config-roundtrip" {
          ...
          merge_and_load() {
            local valos_home="$1"
            export VALOS_HOME="$valos_home"
            ${configMergeScript} ${nixSettings} "$valos_home/config.yaml"
            ${valosVenv}/bin/python3 -c '
import json, sys
