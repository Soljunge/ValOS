# nix/nixosModules.nix — NixOS module for valos-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: valos-agent runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $VALOS_HOME/.env
# and read by valos-agent at startup — no container recreation needed for env changes.
#
# Tool resolution: the valos-agent wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.valos-agent;
    valos-agent = inputs.self.packages.${pkgs.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-valos-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "valos-agent-config-attrs";
      description = "ValOs YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "valos-agent-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "valos-agent-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'VALOS_DOC_EOF'\n${value}\nVALOS_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "valos-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/valos-agent";
    ...
    # valos-agent user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "valos-agent-container-entrypoint" ''
      set -eu

      VALOS_UID="''${VALOS_UID:?VALOS_UID must be set}"
      VALOS_GID="''${VALOS_GID:?VALOS_GID must be set}"

      # ── Group: ensure a group with GID=$VALOS_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$VALOS_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="valos-agent"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$VALOS_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$VALOS_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$VALOS_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$VALOS_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="valos-agent"
        TARGET_HOME="/home/valos-agent"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$VALOS_UID" -g "$VALOS_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$VALOS_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$VALOS_UID:$VALOS_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure VALOS_HOME is owned by the target user
      if [ -n "''${VALOS_HOME:-}" ] && [ -d "$VALOS_HOME" ]; then
        chown -R "$VALOS_UID:$VALOS_GID" "$VALOS_HOME"
      fi
      ...
      if [ ! -f /var/lib/valos-agent-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        echo "First boot: provisioning agent tools..."
        apt-get update -qq
        apt-get install -y -qq sudo nodejs npm curl
        touch /var/lib/valos-agent-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/valos-agent ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/valos-agent
        chmod 0440 /etc/sudoers.d/valos-agent
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$VALOS_UID" --regid="$VALOS_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $VALOS_HOME/.env.
    ...
    # Default: /var/lib/valos-agent/workspace → /data/workspace.
  ...
  in {
    options.services.valos-agent = with lib; {
      enable = mkEnableOption "ValOs gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = valos-agent;
        description = "The valos-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/valos-agent";
        description = "State directory. Contains .valos-agent/ subdir (VALOS_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative ValOs config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $VALOS_HOME/.env at activation time.
          ValOs reads this file on every startup via load_valos_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $VALOS_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $VALOS_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `valos-agent gateway`.";
      };
      ...
      addToSystemPackages = mkOption {
        type = types.bool;
        default = false;
        description = "Add valos-agent CLI to environment.systemPackages.";
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.valos-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.valos-agent        0750 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."valos-agent-setup" = lib.stringAfter [ "users" ] ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.valos-agent
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.valos-agent ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 0750 ${cfg.stateDir} ${cfg.stateDir}/.valos-agent ${cfg.stateDir}/home ${cfg.workingDirectory}

          # Merge Nix settings into existing config.yaml.
          # Preserves user-added keys (skills, streaming, etc.); Nix keys win.
          # If configFile is user-provided (not generated), overwrite instead of merge.
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.valos-agent/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.valos-agent/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.valos-agent/config.yaml
            chmod 0640 ${cfg.stateDir}/.valos-agent/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.valos-agent/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.valos-agent/.managed
          chmod 0644 ${cfg.stateDir}/.valos-agent/.managed

          # Seed auth file if provided
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.valos-agent/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.valos-agent/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.valos-agent/auth.json
              fi
            ''}
          ''}
          ...
          # ValOs reads $VALOS_HOME/.env at startup via load_valos_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.valos-agent/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0600 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'VALOS_NIX_ENV_EOF'
${envFileContent}
VALOS_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.valos-agent = {
          description = "ValOs Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            VALOS_HOME = "${cfg.stateDir}/.valos-agent";
            VALOS_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $VALOS_HOME/.env by the activation script. load_valos_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${cfg.package}/bin/valos-agent"
              "gateway"
            ] ++ cfg.extraArgs);
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.valos-agent = {
          description = "ValOs Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              VALOS_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              VALOS_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env VALOS_UID="$VALOS_UID" \
                --env VALOS_GID="$VALOS_GID" \
                --env VALOS_HOME=${containerDataDir}/.valos-agent \
                --env VALOS_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/valos-agent gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
