"""
Timezone-aware clock for ValOs.

Provides a single ``now()`` helper that returns a timezone-aware datetime
based on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``VALOS_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.valos-agent/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — ValOs never
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — ValOs requires 3.9+)
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("VALOS_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
