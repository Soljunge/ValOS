# Contributing to ValOs

Thank you for contributing to ValOs! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want ValOs to work everywhere.
...
Bundled skills (in `skills/`) ship with every ValOs install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `valos-agent skills browse` (labeled "official") and install it with `valos-agent skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [ValOs Discord](https://discord.gg/horatiu.sol). Users can install it with `valos-agent skills install`.
...
```bash
git clone --recurse-submodules https://github.com/horatiu.sol/valos-agent.git
cd valos-agent
...
```bash
mkdir -p ~/.valos-agent/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.valos-agent/config.yaml
touch ~/.valos-agent/.env

# Add at minimum an LLM provider key:
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.valos-agent/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/valos-agent" ~/.local/bin/valos-agent

# Verify
valos-agent doctor
valos-agent chat -q "Hello"
```
...
├── cli.py                    # ValOsCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (valos-agent-cli, valos-agent-telegram, etc.)
├── valos_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── valos_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── config.py                 # Config management, migration, env var definitions
│   ├── setup.py                  # Interactive setup wizard
│   ├── auth.py                   # Provider resolution, OAuth, ValOs Portal
...
│
├── skills/                   # Bundled skills (copied to ~/.valos-agent/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (valos-agent.valos-agent.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.valos-agent/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.valos-agent/`)

| Path | Purpose |
|------|---------|
| `~/.valos-agent/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.valos-agent/.env` | API keys and secrets |
| `~/.valos-agent/auth.json` | OAuth credentials (ValOs Portal) |
| `~/.valos-agent/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.valos-agent/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.valos-agent/state.db` | SQLite session database |
| `~/.valos-agent/sessions/` | JSON session logs |
| `~/.valos-agent/cron/` | Scheduled job data |
| `~/.valos-agent/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`valos_state.py`) with full-text search and unique session titles. JSON logs go to `~/.valos-agent/sessions/`.
- **Ephemeral injection**: System prompts and prefill messages are injected at API call time, never persisted to the database or logs.
- **Provider abstraction**: The agent works with any OpenAI-compatible API. Provider resolution happens at init time (ValOs Portal OAuth, OpenRouter API key, or custom endpoint).
...
Four fields are supported under `metadata.valos-agent`:
...
The user may skip setup and keep loading the skill. ValOs only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `valos-agent setup` or update `~/.valos-agent/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing ValOs tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `valos-agent --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

ValOs uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.valos-agent/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `valos_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `valos_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

ValOs runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
ValOs has terminal access. Security matters.
...
2. **Test manually**: Run `valos-agent` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/horatiu.sol/valos-agent/issues)
- Include: OS, Python version, ValOs version (`valos-agent version`), full error traceback
...
- **Discord**: [discord.gg/horatiu.sol](https://discord.gg/horatiu.sol) — for questions, showcasing projects, and sharing skills
