...
# 5. Symlinks the 'valos-agent' CLI command into ~/.local/bin
...
# ============================================================================
# PATH setup — symlink valos-agent into ~/.local/bin
# ============================================================================

echo -e "${CYAN}→${NC} Setting up valos-agent command..."

VALOS_BIN="$SCRIPT_DIR/venv/bin/valos-agent"
mkdir -p "$HOME/.local/bin"
ln -sf "$VALOS_BIN" "$HOME/.local/bin/valos-agent"
echo -e "${GREEN}✓${NC} Symlinked valos-agent → ~/.local/bin/valos-agent"
...
# ============================================================================
# Seed bundled skills into ~/.valos-agent/skills/
# ============================================================================

VALOS_SKILLS_DIR="${VALOS_HOME:-$HOME/.valos-agent}/skills"
mkdir -p "$VALOS_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.valos-agent/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$VALOS_SKILLS_DIR/" 2>/dev/null || true
...
echo "     valos-agent setup"
echo ""
echo "  3. Start chatting:"
echo "     valos-agent"
echo ""
echo "Other commands:"
echo "  valos-agent status        # Check configuration"
echo "  valos-agent gateway install # Install gateway service (messaging + cron)"
echo "  valos-agent cron list     # View scheduled jobs"
echo "  valos-agent doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m valos_cli.main setup
fi
