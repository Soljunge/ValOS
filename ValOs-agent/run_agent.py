...
_loaded_env_paths = load_valos_dotenv(valos_home=_valos_home, project_env=_project_env)
...
from agent.prompt_builder import (
    ...
    build_valos_subscription_prompt,
)
...
class _SafeWriter:
    ...
    When valos-agent runs as a systemd service, Docker container, or headless
...
class AIAgent:
    ...
    def __init__(
        ...
        # Optional post-processing pass to make plain-text final answers read
        # more naturally while preserving meaning from the model output.
        self.humanize_responses = env_var_enabled("VALOS_HUMANIZE_RESPONSES", "1")
        ...
        # ValOs is a tool-using client by default.
        ...
        # Persistent error log -- always writes WARNING+ to ~/.valos-agent/logs/errors.log
        ...
        error_log_dir = _valos_home / "logs"
            ...
            if api_key and base_url:
                ...
                if "openrouter" in effective_base.lower():
                    client_kwargs["default_headers"] = {
                        "HTTP-Referer": "https://valos-agent.valos-agent.com",
                        "X-OpenRouter-Title": "ValOs",
                        "X-OpenRouter-Categories": "productivity,cli-agent",
                    }
                elif "api.githubcopilot.com" in effective_base.lower():
                    from valos_cli.models import copilot_default_headers
                ...
                if _routed_client is not None:
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        raise RuntimeError(
                            ...
                            f"variable, or switch to a different provider with `valos-agent model`."
                        )
        ...
        # Session logs go into ~/.valos-agent/sessions/ alongside gateway sessions
        valos_home = get_valos_home()
        self.logs_dir = valos_home / "sessions"
        ...
        if self._session_db:
            try:
                self._session_db.create_session(
                    ...
                    source=self.platform or os.environ.get("VALOS_SESSION_SOURCE", "cli"),
                    model=self.model,
                    model_config={
        ...
        try:
            from valos_cli.config import load_config as _load_agent_config
            ...
            try:
                ...
                if not _mem_provider_name:
                    try:
                        ...
                        if _hcfg.enabled and (_hcfg.api_key or _hcfg.base_url):
                            ...
                            try:
                                from valos_cli.config import load_config as _lc, save_config as _sc
                                _cfg = _lc()
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        from valos_constants import get_valos_home as _ghh
                        _init_kwargs = {
                            "session_id": self.session_id,
                        ...
                        try:
                            ...
                            _init_kwargs["agent_workspace"] = "valos-agent"
                        except Exception:
                            pass
    ...
    def _should_humanize_response(self, text: str) -> bool:
        """Return True when the response is safe for tone-only rewriting."""
        if not self.humanize_responses:
            return False
        ...
        # Preserve structured / machine-oriented output exactly.
        if "```" in stripped:
            return False
        if lower.startswith(("error:", "api call failed", "operation interrupted:")):
            return False
    ...
    def _humanize_response(self, text: str) -> str:
        """Apply a lightweight readability pass without changing factual content."""
        if not self._should_humanize_response(text):
            return text

        out = text
        # Remove stiff lead-ins that often sound robotic.
        out = re.sub(r"^\s*(Certainly|Of course|Absolutely|Sure)[,!\s]+", "", out, flags=re.IGNORECASE)
        # Prefer natural contractions in plain prose.
        out = re.sub(r"\bI will\b", "I'll", out)
        out = re.sub(r"\bI cannot\b", "I can't", out)
        out = re.sub(r"\bdo not\b", "don't", out, flags=re.IGNORECASE)
        out = re.sub(r"\bis not\b", "isn't", out, flags=re.IGNORECASE)
        # Tidy spacing artifacts from replacements.
        out = re.sub(r"[ \t]+\n", "\n", out)
        out = re.sub(r"\n{3,}", "\n\n", out)
        return out.strip()
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if env_var_enabled("VALOS_DUMP_REQUEST_STDOUT"):
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        valos_subscription_prompt = build_valos_subscription_prompt(self.valid_tool_names)
        if valos_subscription_prompt:
            prompt_parts.append(valos_subscription_prompt)
        ...
        if not self.skip_context_files:
            ...
            # mode).  The gateway process runs from the valos-agent install
        ...
        from valos_time import now as _valos_now
        now = _valos_now()
    ...
    def _try_refresh_valos_client_credentials(self, *, force: bool = True) -> bool:
        if self.api_mode != "chat_completions" or self.provider != "valos":
            return False

        try:
            from valos_cli.auth import resolve_valos_runtime_credentials

            creds = resolve_valos_runtime_credentials(
                min_key_ttl_seconds=max(60, int(os.getenv("VALOS_VALOS_MIN_KEY_TTL_SECONDS", "1800"))),
                timeout_seconds=float(os.getenv("VALOS_VALOS_TIMEOUT_SECONDS", "15")),
                force_mint=force,
            )
        except Exception as exc:
            logger.debug("ValOs credential refresh failed: %s", exc)
            return False
        ...
        self._client_kwargs["api_key"] = self.api_key
        self._client_kwargs["base_url"] = self.base_url
        # ValOs requests should not inherit OpenRouter-only attribution headers.
        self._client_kwargs.pop("default_headers", None)

        if not self._replace_primary_openai_client(reason="valos_credential_refresh"):
            return False
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            import httpx as _httpx
            _base_timeout = float(os.getenv("VALOS_API_TIMEOUT", 1800.0))
            _stream_read_timeout = float(os.getenv("VALOS_STREAM_READ_TIMEOUT", 60.0))
            stream_kwargs = {
                **api_kwargs,
                "stream": True,
        ...
        def _call():
            ...
            _max_stream_retries = int(os.getenv("VALOS_STREAM_RETRIES", 2))
        ...
        _stream_stale_timeout_base = float(os.getenv("VALOS_STREAM_STALE_TIMEOUT", 180.0))
    ...
    def _try_recover_primary_transport(
        ...
        Skipped for proxy/aggregator providers (OpenRouter, ValOs) which
        ...
        if provider_lower in ("valos", "valos-ui"):
            return False
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        api_kwargs = {
            ...
            "timeout": float(os.getenv("VALOS_API_TIMEOUT", 1800.0)),
        }
        if self.tools:
        ...
        # TODO: ValOs Portal will add transparent proxy support — re-enable
        # for _is_valos when their backend is updated.
        if provider_preferences and _is_openrouter:
            extra_body["provider"] = provider_preferences
        _is_valos = "valos-agent" in self._base_url_lower
        ...
        if self._supports_reasoning_extra_body():
            if _is_github_models:
                ...
                if self.reasoning_config is not None:
                    ...
                    # ValOs Portal requires reasoning enabled — don't send
                    # enabled=false to it (would cause 400).
                    if _is_valos and rc.get("enabled") is False:
                        pass  # omit reasoning entirely for ValOs when disabled
                    else:
        ...
        # ValOs Portal product attribution
        if _is_valos:
            extra_body["tags"] = ["product=valos-agent"]
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        known reasoning-capable model families and direct ValOs Portal.
        """
        if "valos-agent" in self._base_url_lower:
            return True
        ...
        if "models.github.ai" in self._base_url_lower or "api.githubcopilot.com" in self._base_url_lower:
            try:
                from valos_cli.models import github_model_reasoning_efforts
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            if _is_valos:
                summary_extra_body["tags"] = ["product=valos-agent"]
    ...
    def run_conversation(
        ...
        while api_call_count < self.max_iterations and self.iteration_budget.remaining > 0:
            ...
            valos_auth_retry_attempted=False
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if env_var_enabled("VALOS_DUMP_REQUESTS"):
                    ...
                    if (
                        self.api_mode == "chat_completions"
                        and self.provider == "valos"
                        and status_code == 401
                        and not valos_auth_retry_attempted
                    ):
                        valos_auth_retry_attempted = True
                        if self._try_refresh_valos_client_credentials(force=True):
                            print(f"{self.log_prefix}🔐 ValOs agent key refreshed after 401. Retrying request...")
                            continue
                    if (
                        self.api_mode == "anthropic_messages"
                        and status_code == 401
                        ...
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for ValOs-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://console.anthropic.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Clear stale keys: valos-agent config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Legacy cleanup: valos-agent config set ANTHROPIC_API_KEY \"\"")
                    ...
                    is_context_length_error = any(phrase in error_msg for phrase in [
                        ...
                        'request entity too large',  # OpenRouter/ValOs 413 safety net
                    ...
                    if is_client_error:
                        ...
                        if status_code in (401, 403) or "unauthorized" in error_msg or "forbidden" in error_msg or "permission" in error_msg:
                            ...
                            self._vprint(f"{self.log_prefix}      • Is the key valid? Run: valos-agent setup", force=True)
                        ...
                        if self._empty_content_retries < 3:
                            ...
                            if empty_response_info["is_local_custom"]:
                                error_message = (
                                    ...
                                    "Likely causes: wrong /v1 endpoint, runtime context window smaller than ValOs expects, "
        ...
        # Final tone pass after model output is assembled.
        if final_response:
            humanized = self._humanize_response(final_response)
            if humanized != final_response:
                final_response = humanized
