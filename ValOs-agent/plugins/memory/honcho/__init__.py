...
Config: Uses the existing Honcho config chain:
  1. $VALOS_HOME/honcho.json (profile-scoped)
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, valos_home):
        """Write config to $VALOS_HOME/honcho.json (Honcho SDK native format)."""
        import json
        from pathlib import Path
        config_path = Path(valos_home) / "honcho.json"
        existing = {}
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            try:
                valos_home = kwargs.get("valos_home", "")
                if valos_home and not cfg.raw.get("aiPeer"):
                    soul_path = Path(valos_home) / "SOUL.md"
                    if soul_path.exists():
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        try:
            if not session.messages:
                ...
                mem_dir = str(get_valos_home() / "memories")
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "valos-agent-default",
