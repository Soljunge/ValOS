...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID   — memory bank identifier (default: valos-agent)
  HINDSIGHT_BUDGET    — recall budget: low/mid/high (default: mid)
  HINDSIGHT_API_URL   — API endpoint
  HINDSIGHT_MODE      — cloud or local (default: cloud)

Or via $VALOS_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $VALOS_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_valos_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "valos-agent": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "valos-agent"),
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, valos_home):
        """Write config to $VALOS_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(valos_home) / "hindsight"
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank identifier", "default": "valos-agent"},
    ...
    def _make_client(self):
        ...
        if self._mode == "local":
            ...
            return HindsightEmbedded(
                profile=embed.get("profile", "valos-agent"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = self._config.get("banks", {}).get("valos-agent", {})
        self._bank_id = banks.get("bankId", "valos-agent")
