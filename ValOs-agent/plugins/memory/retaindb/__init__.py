...
Config via environment variables:
  ...
  RETAINDB_PROJECT    — Project identifier (default: valos-agent)
"""
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "project", "description": "Project identifier", "default": "valos-agent"},
        ]
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        if explicit_project:
            ...
            profile_name = os.path.basename(valos_home) if valos_home else ""
            # Default profile (~/.valos-agent) → "valos-agent"; named profiles → "valos-agent-<name>"
            if profile_name and profile_name != ".valos-agent":
                self._project = f"valos-agent-{profile_name}"
            else:
                self._project = "valos-agent"
