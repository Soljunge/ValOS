...
Data is cached in memory (1hr TTL) and on disk (~/.valos-agent/models_dev_cache.json)
...
# Provider ID mapping: ValOs provider names → models.dev provider IDs
...
def _get_cache_path() -> Path:
    ...
    env_val = os.environ.get("VALOS_HOME", "")
    valos_home = Path(env_val) if env_val else Path.home() / ".valos-agent"
    return valos_home / "models_dev_cache.json"
