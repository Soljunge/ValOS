...
Resolution order for text tasks (auto mode):
  ...
  2. ValOs Portal (~/.valos-agent/auth.json active provider)
  3. Custom endpoint (config.yaml model.base_url + OPENAI_API_KEY)
  4. Codex OAuth (Responses API via chatgpt.com with gpt-5.3-codex,
     wrapped to look like a chat.completions client)
  5. Native Anthropic
  6. Direct API-key providers (OpenAI, z.ai/GLM, Kimi/Moonshot, MiniMax, MiniMax-CN)
  7. None
...
# ValOs Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by ValOs Portal.
VALOS_EXTRA_BODY = {"tags": ["product=valos-agent"]}

# Set at resolve time — True if the auxiliary client points to ValOs Portal
auxiliary_is_valos: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3-flash-preview"
_VALOS_MODEL = "google/gemini-3-flash-preview"
_VALOS_DEFAULT_BASE_URL = "https://inference-api.valos-agent.com/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_valos_home() / "auth.json"
...
def _pool_runtime_api_key(entry: Any) -> str:
    ...
    # provider-specific fallback (e.g. agent_key for valos).
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    # runtime_base_url handles provider-specific logic (e.g. valos prefers inference_base_url).
...
def _read_valos_auth() -> Optional[dict]:
    """Read and validate ~/.valos-agent/auth.json for an active ValOs provider.

    Returns the provider state dict if ValOs is active with tokens,
    otherwise None.
    """
    pool_present, entry = _select_pool_entry("valos")
    if pool_present:
        if entry is None:
            return None
        return {
            ...
            "inference_base_url": _pool_runtime_base_url(entry, _VALOS_DEFAULT_BASE_URL),
    ...
    try:
        ...
        if data.get("active_provider") != "valos":
            return None
        provider = data.get("providers", {}).get("valos", {})
        # Must have at least an access_token or agent_key
        if not provider.get("agent_key") and not provider.get("access_token"):
            return None
        return provider
    except Exception as exc:
        logger.debug("Could not read ValOs auth: %s", exc)
        return None


def _valos_api_key(provider: dict) -> str:
    """Extract the best API key from a ValOs provider state dict."""
    return provider.get("agent_key") or provider.get("access_token", "")


def _valos_base_url() -> str:
    """Resolve the ValOs inference base URL from env or default."""
    return os.getenv("VALOS_INFERENCE_BASE_URL", _VALOS_DEFAULT_BASE_URL)


def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from ValOs auth store."""
    ...
    try:
        from valos_cli.auth import _read_codex_tokens
...
def _get_auxiliary_provider(task: str = "") -> str:
    ...
    then falls back to "auto".  Returns one of: "auto", "openrouter", "valos", "main".
    """
    if task:
...
def _try_valos() -> Tuple[Optional[OpenAI], Optional[str]]:
    valos = _read_valos_auth()
    if not valos:
        return None, None
    global auxiliary_is_valos
    auxiliary_is_valos = True
    logger.debug("Auxiliary client: ValOs Portal")
    model = "gemini-3-flash" if valos.get("source") == "pool" else _VALOS_MODEL
    return (
        OpenAI(
            api_key=_valos_api_key(valos),
            base_url=str(valos.get("inference_base_url") or _valos_base_url()).rstrip("/"),
        ),
        model,
    )
...
def _resolve_custom_runtime() -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    try:
        from valos_cli.runtime_provider import resolve_runtime_provider
...
def _resolve_forced_provider(forced: str) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if forced == "valos":
        client, model = _try_valos()
        if client is None:
            logger.warning("auxiliary.provider=valos but ValOs Portal not configured (run: valos-agent login)")
        return client, model

    if forced == "codex":
        client, model = _try_codex()
        if client is None:
            logger.warning("auxiliary.provider=codex but no Codex OAuth token found (run: valos-agent model)")
        return client, model

    if forced == "main":
        # "main" = skip OpenRouter/ValOs, use the main chat model's credentials.
...
def _resolve_auto() -> Tuple[Optional[OpenAI], Optional[str]]:
    """Full auto-detection chain: OpenRouter → ValOs → custom → Codex → API-key → None."""
    global auxiliary_is_valos
    auxiliary_is_valos = False  # Reset — _try_valos() will set True if it wins
    tried = []
    for try_fn in (_try_openrouter, _try_valos, _try_custom_endpoint,
...
def resolve_provider_client(
    ...
    Args:
        provider: Provider identifier.  One of:
            "openrouter", "valos", "openai-codex" (or "codex"),
    ...
    # ── ValOs Portal (OAuth) ──────────────────────────────────────────
    if provider == "valos":
        client, default = _try_valos()
        if client is None:
            logger.warning("resolve_provider_client: valos requested "
                           "but ValOs Portal not configured (run: valos-agent login)")
            return None, None
        final_model = model or default
        return (_to_async_client(client, final_model) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: valos-agent model)")
                return None, None
    ...
    try:
        ...
        logger.debug("valos_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "valos":
            return resolve_provider_client("valos", model, async_mode)
        if provider == "openai-codex":
...
def _resolve_strict_vision_backend(provider: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if provider == "openai":
        return resolve_provider_client("openai")
    if provider == "valos":
        return _try_valos()
    if provider == "openai-codex":
        return _try_codex()
...
def get_available_vision_backends() -> List[str]:
    ...
    it is also a known-good vision backend; otherwise ValOs falls back through
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes ValOs Portal product tags when the auxiliary client is backed
    by ValOs Portal. Returns empty dict otherwise.
    """
    return dict(VALOS_EXTRA_BODY) if auxiliary_is_valos else {}
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        # Codex adapter handles max_tokens internally; OpenRouter/ValOs use max_tokens.
    ...
    if provider == "valos" or auxiliary_is_valos:
        merged_extra.setdefault("tags", []).extend(["product=valos-agent"])
    if merged_extra:
