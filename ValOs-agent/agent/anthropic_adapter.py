"""Anthropic Messages API adapter for ValOs.

Translates between ValOs's internal OpenAI-style message format and
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    ValOs historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by ValOs)
    ...
    # 1. ValOs-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration ValOs configs.
    api_key = os.getenv("ANTHROPIC_API_KEY", "").strip()
    if api_key:
        return api_key
...
# ── ValOs-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.valos-agent/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_VALOS_OAUTH_FILE = get_valos_home() / ".anthropic_oauth.json"
...
def run_valos_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run ValOs-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize ValOs with your Claude Pro/Max subscription.")
    print()
...
def run_valos_oauth_login() -> Optional[str]:
    """Run ValOs-native OAuth PKCE flow for Claude Pro/Max subscription.

    Opens a browser to claude.ai for authorization, prompts for the code,
    exchanges it for tokens, and stores them in ~/.valos-agent/.anthropic_oauth.json.

    Returns the access token on success, None on failure.
    """
    result = run_valos_oauth_login_pure()
    if not result:
        return None
    ...
    _save_valos_oauth_credentials(access_token, refresh_token, expires_at_ms)
...
def _save_valos_oauth_credentials(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Save OAuth credentials to ~/.valos-agent/.anthropic_oauth.json."""
    data = {
        "accessToken": access_token,
        "refreshToken": refresh_token,
        "expiresAt": expires_at_ms,
    ...
    try:
        _VALOS_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
        _VALOS_OAUTH_FILE.write_text(json.dumps(data, indent=2), encoding="utf-8")
        _VALOS_OAUTH_FILE.chmod(0o600)
    except (OSError, IOError) as e:
        logger.debug("Failed to save ValOs OAuth credentials: %s", e)


def read_valos_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read ValOs-managed OAuth credentials from ~/.valos-agent/.anthropic_oauth.json."""
    if _VALOS_OAUTH_FILE.exists():
        try:
            data = json.loads(_VALOS_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read ValOs OAuth credentials: %s", e)
    return None


def refresh_valos_oauth_token() -> Optional[str]:
    """Refresh the ValOs-managed OAuth token using the stored refresh token.

    Returns the new access token, or None if refresh fails.
    """
    creds = read_valos_oauth_credentials()
    ...
    try:
        ...
        logger.debug("Successfully refreshed ValOs OAuth token")
        return refreshed["access_token"]
    except Exception as e:
        logger.debug("Failed to refresh ValOs OAuth token: %s", e)
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("ValOs", "Claude Code")
                text = text.replace("ValOs agent", "Claude Code")
                text = text.replace("valos-agent", "claude-code")
                text = text.replace("ValOs", "Anthropic")
                block["text"] = text
