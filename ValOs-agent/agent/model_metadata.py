...
_PROVIDER_PREFIXES: frozenset[str] = frozenset({
    "openrouter", "valos", "openai-codex", "copilot", "copilot-acp",
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.valos-agent.com": "valos",
...
def _get_context_cache_path() -> Path:
    ...
    valos_home = Path(os.environ.get("VALOS_HOME", Path.home() / ".valos-agent"))
    return valos_home / "context_length_cache.yaml"
...
def _normalize_model_version(model: str) -> str:
    ...
    ValOs uses dashes: claude-opus-4-6, claude-sonnet-4-5
...
def _resolve_valos_context_length(model: str) -> Optional[int]:
    """Resolve ValOs Portal model context length via OpenRouter metadata.

    ValOs model IDs are bare (e.g. 'claude-opus-4-6') while OpenRouter uses
...
def get_model_context_length(
    ...
    6. ValOs suffix-match via OpenRouter cache
    ...
    if effective_provider == "valos":
        ctx = _resolve_valos_context_length(model)
        if ctx:
            return ctx
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets ValOs sends to providers:
