...
it is automatically downloaded from GitHub releases to $VALOS_HOME/bin/tirith.
...
def _get_valos_home() -> str:
    """Return the ValOs home directory, respecting VALOS_HOME env var."""
    return str(get_valos_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_valos_home(), ".tirith-install-failed")
...
def _valos_bin_dir() -> str:
    """Return $VALOS_HOME/bin, creating it if needed."""
    d = os.path.join(_get_valos_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $VALOS_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_valos_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $VALOS_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $VALOS_HOME/bin/tirith
    ...
    valos_bin = os.path.join(_valos_bin_dir(), "tirith")
    if os.path.isfile(valos_bin) and os.access(valos_bin, os.X_OK):
        _resolved_path = valos_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return valos_bin
