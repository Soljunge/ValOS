"""Generic managed-tool gateway helpers for ValOs-hosted vendor passthroughs."""
...
_DEFAULT_TOOL_GATEWAY_DOMAIN = "valos-agent.com"
_DEFAULT_TOOL_GATEWAY_SCHEME = "https"
_VALOS_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120
...
class ManagedToolGatewayConfig:
    vendor: str
    gateway_origin: str
    valos_user_token: str
    managed_mode: bool


def auth_json_path():
    """Return the ValOs auth store path, respecting VALOS_HOME overrides."""
    return get_valos_home() / "auth.json"


def _read_valos_provider_state() -> Optional[dict]:
    try:
        path = auth_json_path()
        if not path.is_file():
            return None
        ...
        if not isinstance(providers, dict):
            return None
        valos_provider = providers.get("valos", {})
        if isinstance(valos_provider, dict):
            return valos_provider
    except Exception:
        pass
    return None
...
def read_valos_access_token() -> Optional[str]:
    """Read a ValOs Subscriber OAuth access token from auth store or env override."""
    explicit = os.getenv("TOOL_GATEWAY_USER_TOKEN")
    if isinstance(explicit, str) and explicit.strip():
        return explicit.strip()

    valos_provider = _read_valos_provider_state() or {}
    access_token = valos_provider.get("access_token")
    ...
    if cached_token and not _access_token_is_expiring(
        valos_provider.get("expires_at"),
        _VALOS_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
    ):
        return cached_token

    try:
        from valos_cli.auth import resolve_valos_access_token

        refreshed_token = resolve_valos_access_token(
            refresh_skew_seconds=_VALOS_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
        )
        if isinstance(refreshed_token, str) and refreshed_token.strip():
            return refreshed_token.strip()
    except Exception as exc:
        logger.debug("ValOs access token refresh failed: %s", exc)
...
def resolve_managed_tool_gateway(
    ...
    resolved_token_reader = token_reader or read_valos_access_token

    gateway_origin = resolved_gateway_builder(vendor)
    valos_user_token = resolved_token_reader()
    if not gateway_origin or not valos_user_token:
        return None

    return ManagedToolGatewayConfig(
        vendor=vendor,
        gateway_origin=gateway_origin,
        valos_user_token=valos_user_token,
        managed_mode=True,
    )
...
def is_managed_tool_gateway_ready(
    ...
    """Return True when gateway URL and ValOs access token are available."""
