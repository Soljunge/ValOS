...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on ValOs Portal).
    """
    ...
    # Extract parent's API key so subagents inherit auth (e.g. ValOs Portal).
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, valos, zai, kimi-coding, minimax."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'valos-agent login'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (valos_cli/config.py load_config()) so that
