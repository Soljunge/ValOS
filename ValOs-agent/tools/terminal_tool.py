...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or ValOs exit
...
# display_valos_home imported lazily at call site (stale-module safety during valos-agent update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of valos-agent directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "valos-agent-*")):
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = os.getenv("VALOS_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (VALOS_INTERACTIVE=1).
    ...
    try:
        os.environ["VALOS_SPINNER_PAUSE"] = "1"
        ...
        if "VALOS_SPINNER_PAUSE" in os.environ:
            del os.environ["VALOS_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str) -> tuple[str, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (VALOS_INTERACTIVE=1):
    ...
    if not sudo_password:
        ...
        if os.getenv("VALOS_INTERACTIVE"):
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.valos-agent/.env or environment variables."
        )
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "VALOS_ENABLE_VALOS_MANAGED_TOOLS is not enabled and no direct "
...
def get_active_environments_info() -> Dict[str, Any]:
    ...
    for task_id in _active_environments.keys():
        scratch_dir = _get_scratch_dir()
        pattern = f"valos-agent-*{task_id[:8]}*"
        import glob
        ...
        if background:
            ...
            try:
                ...
                if check_interval and background:
                    ...
                    watcher_platform = os.getenv("VALOS_SESSION_PLATFORM", "")
                    watcher_chat_id = os.getenv("VALOS_SESSION_CHAT_ID", "")
                    watcher_thread_id = os.getenv("VALOS_SESSION_THREAD_ID", "")
