...
def _resolve_valos_home() -> Path:
    return Path(os.environ.get("VALOS_HOME", Path.home() / ".valos-agent"))


def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.valos-agent",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``VALOS_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside VALOS_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    valos_home = _resolve_valos_home()

    # Reject absolute paths — they bypass the VALOS_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to VALOS_HOME)",
            relative_path,
        )
        return False

    host_path = valos_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape VALOS_HOME.
    try:
        resolved = host_path.resolve()
        valos_home_resolved = valos_home.resolve()
        resolved.relative_to(valos_home_resolved)  # raises ValueError if outside
    except ValueError:
        logger.warning(
            "credential_files: rejected path traversal %r "
            "(resolves to %s, outside VALOS_HOME %s)",
            relative_path,
            resolved,
            valos_home_resolved,
        )
                ...
                for item in cred_files:
                    if isinstance(item, str) and item.strip():
                        ...
                        if os.path.isabs(rel):
                            ...
                            continue
                        host_path = (valos_home / rel).resolve()
                        try:
                            host_path.relative_to(valos_home_resolved)
                        except ValueError:
                            logger.warning(
                                "credential_files: rejected config path traversal %r "
                                "(resolves to %s, outside VALOS_HOME %s)",
                                rel, host_path, valos_home_resolved,
                            )
                            continue
                        if host_path.is_file():
                            container_path = f"/root/.valos-agent/{rel}"
                            result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="valos-agent-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching valos_constants.get_valos_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_valos_dir()`` for backward compatibility with old directory layouts.
    """
    from valos_constants import get_valos_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_valos_dir(new_subpath, old_name)
        if host_dir.is_dir():
