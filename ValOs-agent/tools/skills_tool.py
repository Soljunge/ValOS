...
# All skills live in ~/.valos-agent/skills/ (seeded from bundled skills/ on install).
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from VALOS_HOME/.env."""
    env_path = get_valos_home() / ".env"
...
def _is_gateway_surface() -> bool:
    if os.getenv("VALOS_GATEWAY_SESSION"):
        return True
    return bool(os.getenv("VALOS_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return "Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to ~/.valos-agent/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.valos-agent/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("VALOS_PLATFORM")
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.valos-agent/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``valos-agent skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": "No skills found. Skills directory created at ~/.valos-agent/skills/",
                },
                ensure_ascii=False,
            )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.valos-agent/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `valos-agent skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.valos-agent.* first (agentskills.io convention), fall back to top-level
        valos_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            valos_meta = metadata.get("valos-agent", {}) or {}

        tags = _parse_tags(valos_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            valos_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
