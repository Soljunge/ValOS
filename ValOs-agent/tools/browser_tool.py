...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-valos_…`` the resulting socket path exceeds the
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, ValOs-managed
    ...
    # Build an extended search PATH including Homebrew and ValOs-managed dirs.
    ...
    valos_home = get_valos_home()
    valos_node_bin = str(valos_home / "node" / "bin")
    if os.path.isdir(valos_node_bin):
        extra_dirs.append(valos_node_bin)
...
def _run_browser_command(
    ...
    try:
        ...
        # Ensure PATH includes ValOs-managed Node first, Homebrew versioned
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = valos_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    screenshots_dir = get_valos_dir("cache/screenshots", "browser_screenshots")
