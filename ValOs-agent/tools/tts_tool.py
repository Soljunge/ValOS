...
Configuration is loaded from ~/.valos-agent/config.yaml under the 'tts:' key.
...
from tools.tool_backend_helpers import managed_valos_tools_enabled, resolve_openai_audio_api_key
...
def _get_default_output_dir() -> str:
    ...
    return str(get_valos_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.valos-agent/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.valos-agent/config.yaml.
    ...
    try:
        ...
        logger.debug("valos_cli.config not available, using default TTS config")
        return {}
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.valos-agent/config.yaml (tts: section).
    ...
    platform = os.getenv("VALOS_SESSION_PLATFORM", "").lower()
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run valos-agent setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
...
def _resolve_openai_audio_client_config() -> tuple[str, str]:
    ...
    return managed_gateway.valos_user_token, urljoin(
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": "Optional custom file path to save the audio. Defaults to ~/.valos-agent/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
