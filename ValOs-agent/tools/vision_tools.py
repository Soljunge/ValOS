...
ValOs, Codex, native Anthropic, or a custom OpenAI-compatible endpoint.
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("VALOS_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
if __name__ == "__main__":
    ...
    if not api_available:
        ...
        print("Configure a supported multimodal backend (OpenRouter, ValOs, Codex, Anthropic, or a custom OpenAI-compatible endpoint).")
        exit(1)
    else:
