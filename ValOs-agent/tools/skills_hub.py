#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the ValOs Skills Hub.
...
Used by valos_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            valos_meta = metadata.get("valos-agent", {})
            if isinstance(valos_meta, dict):
                tags = valos_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by ValOs) but not activated
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                valos_meta = meta_block.get("valos-agent", {})
                if isinstance(valos_meta, dict):
                    tags = valos_meta.get("tags", [])
