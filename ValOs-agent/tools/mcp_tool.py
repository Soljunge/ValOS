...
discovers their tools, and registers them into the valos-agent tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.valos-agent/config.yaml under the ``mcp_servers`` key.
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            valos_home = os.path.expanduser(
                os.getenv(
                    "VALOS_HOME", os.path.join(os.path.expanduser("~"), ".valos-agent")
                )
            )
            candidates = [
                os.path.join(valos_home, "node", "bin", resolved_command),
...
class MCPServerTask:
    ...
    async def _refresh_tools(self):
        ...
        async with self._refresh_lock:
            ...
            # 2. Remove old tools from valos-agent-* umbrella toolsets
            for ts_name, ts in TOOLSETS.items():
                if ts_name.startswith("valos-agent-"):
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the ValOs config file.
    ...
    ``os.environ`` (which includes ``~/.valos-agent/.env`` loaded at startup).
    """
    try:
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves ValOs's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the ValOs registry schema format.
...
def _sync_mcp_toolsets(server_names: Optional[List[str]] = None) -> None:
    """Expose each MCP server as a standalone toolset and inject into valos-agent-* sets.
    ...
    Also injects all MCP tools into valos-agent-* umbrella toolsets for the
    default behavior.
    ...
    # Also inject into valos-agent-* umbrella toolsets for default behavior.
    for ts_name, ts in TOOLSETS.items():
        if not ts_name.startswith("valos-agent-"):
            continue
...
def _register_server_tools(name: str, server: MCPServerTask, config: dict) -> List[str]:
    ...
    and valos-agent-* umbrella toolset injection.
    ...
    if registered_names:
        ...
        # Inject into valos-agent-* umbrella toolsets for default behavior
        for ts_name, ts in TOOLSETS.items():
            if ts_name.startswith("valos-agent-"):
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``valos-agent tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the ValOs registry.
