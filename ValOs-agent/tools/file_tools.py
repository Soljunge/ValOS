...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── ValOs internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files.
        import pathlib as _pathlib
        from valos_constants import get_valos_home as _get_hh
        _resolved = _pathlib.Path(path).expanduser().resolve()
        _valos_home = _get_hh().resolve()
        _blocked_dirs = [
            _valos_home / "skills" / ".hub" / "index-cache",
            _valos_home / "skills" / ".hub",
        ]
        for _blocked in _blocked_dirs:
            try:
                ...
                return json.dumps({
                    "error": (
                        f"Access denied: {path} is an internal ValOs cache file "
