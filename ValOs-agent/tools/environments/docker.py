...
def _load_valos_env_vars() -> dict[str, str]:
    """Load ~/.valos-agent/.env values without failing Docker command execution."""
    try:
        from valos_cli.config import load_env
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.valos-agent/sandboxes/). Non-persistent
        ...
        container_name = f"valos-agent-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def execute(self, command: str, cwd: str = "", *,
        ...
        try:
            ...
            pass
        valos_env = _load_valos_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = valos_env.get(key)
