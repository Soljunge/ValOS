...
Trust levels:
  - builtin:   Ships with ValOs. Never scanned, always trusted.
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.valos-agent/\.env|\~/\.valos-agent/\.env',
     "valos_env_access", "critical", "exfiltration",
     "directly references ValOs secrets file"),
    ...
    (r'\.valos-agent/config\.yaml|\.valos-agent/SOUL\.md',
     "valos_config_mod", "critical", "persistence",
     "references ValOs configuration files directly"),
...
def _get_configured_model() -> str:
    """Load the user's configured model from ~/.valos-agent/config.yaml."""
    try:
