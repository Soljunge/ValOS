...
LOCAL_STT_COMMAND_ENV = "VALOS_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "VALOS_LOCAL_STT_LANGUAGE"
...
def get_stt_model_from_config() -> Optional[str]:
    """Read the STT model name from ~/.valos-agent/config.yaml.
    ...
    try:
        import yaml
        cfg_path = get_valos_home() / "config.yaml"
        if cfg_path.exists():
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set VALOS_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="valos-agent-local-stt-") as output_dir:
