...
def managed_valos_tools_enabled() -> bool:
    """Return True when the hidden ValOs-managed tools feature flag is enabled."""
    return env_var_enabled("VALOS_ENABLE_VALOS_MANAGED_TOOLS")
...
def resolve_modal_backend_state(
    ...
    *,
    ...
    managed_mode_blocked = (
        requested_mode == "managed" and not managed_valos_tools_enabled()
    )

    if normalized_mode == "managed":
        selected_backend = "managed" if managed_valos_tools_enabled() and managed_ready else None
    elif normalized_mode == "direct":
        selected_backend = "direct" if has_direct else None
    else:
        selected_backend = "managed" if managed_valos_tools_enabled() and managed_ready else "direct" if has_direct else None
