...
``httpx.Auth``) with ValOs-specific token storage and browser-based
...
# ---------------------------------------------------------------------------
# Token storage — persists tokens + client info to ~/.valos-agent/mcp-tokens/
# ---------------------------------------------------------------------------
...
class ValOsTokenStorage:
    ...
    def _base_dir(self) -> Path:
        home = Path(os.environ.get("VALOS_HOME", Path.home() / ".valos-agent"))
...
async def _wait_for_callback() -> tuple[str, str | None]:
    ...
    connecting and potentially hanging ValOs startup indefinitely.
    """
    global _oauth_port
    ...
    if not code:
        raise OAuthNonInteractiveError(
            ...
            "Run 'valos-agent mcp auth <server-name>' to authorize interactively."
        )
    return code, state
...
def build_oauth_auth(server_name: str, server_url: str):
    ...
    storage = ValOsTokenStorage(server_name)
    ...
    if not interactive:
        ...
        if not has_cached:
            logger.warning(
                ...
                "connect. Run 'valos-agent mcp auth %s' to authorize interactively.",
    ...
    if not interactive:
        ...
        async def _noninteractive_callback() -> tuple[str, str | None]:
            raise OAuthNonInteractiveError(
                ...
                f"(no TTY). Run 'valos-agent mcp auth {server_name}' to "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    ValOsTokenStorage(server_name).remove()
