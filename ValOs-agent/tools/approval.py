...
def get_current_session_key(default: str = "default") -> str:
    ...
    if session_key:
        return session_key
    return os.getenv("VALOS_SESSION_KEY", default)

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $VALOS_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_VALOS_ENV_PATH = (
    r'(?:~\/\.valos-agent/|'
    r'(?:\$home|\$\{home\})/\.valos-agent/|'
    r'(?:\$valos_home|\$\{valos_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart valos-agent-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart valos-agent-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(valos-agent|gateway|cli\.py)\b', "kill valos-agent/gateway process (self-termination)"),
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = os.getenv("VALOS_INTERACTIVE")
    is_gateway = os.getenv("VALOS_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        return {"approved": True, "message": None}

    if is_gateway or os.getenv("VALOS_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("VALOS_YOLO_MODE") or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("VALOS_INTERACTIVE")
    is_gateway = os.getenv("VALOS_GATEWAY_SESSION")
    is_ask = os.getenv("VALOS_EXEC_ASK")
