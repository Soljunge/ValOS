...
Backend is selected during ``valos-agent tools`` setup (web.backend in config.yaml).
When available, ValOs can route Firecrawl calls through a ValOs-hosted tool-gateway
for ValOs Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract, crawl; direct or derived firecrawl-gateway.<domain> for ValOs Subscribers)
...
from tools.managed_tool_gateway import (
    ...
    read_valos_access_token as _read_valos_access_token,
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.valos-agent/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``valos-agent tools``).
    ...
    # tool gateway is configured for ValOs subscribers.
    backend_candidates = (
...
def _is_tool_gateway_ready() -> bool:
    """Return True when gateway URL and a ValOs Subscriber token are available."""
    return resolve_managed_tool_gateway("firecrawl", token_reader=_read_valos_access_token) is not None
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if managed_valos_tools_enabled():
        message += (
            " If you have the hidden ValOs-managed tools flag enabled, you can also login to ValOs "
            "(`valos-agent model`) and provide FIRECRAWL_GATEWAY_URL or TOOL_GATEWAY_DOMAIN."
        )
...
def _firecrawl_backend_help_suffix() -> str:
    ...
    if not managed_valos_tools_enabled():
        return ""
    return (
        ", or, if you have the hidden ValOs-managed tools flag enabled, login to ValOs and use "
...
def _get_firecrawl_client():
    ...
    ValOs falls back to the Firecrawl tool-gateway for logged-in ValOs Subscribers.
    """
    ...
    if direct_config is not None:
        ...
        managed_gateway = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=_read_valos_access_token,
        )
        ...
        kwargs = {
            "api_key": managed_gateway.valos_user_token,
            "api_url": managed_gateway.gateway_origin,
        }
        client_config = (
            "tool-gateway",
            kwargs["api_url"],
            managed_gateway.valos_user_token,
        )
...
def _is_valos_auxiliary_client(client: Any) -> bool:
    """Return True when the resolved auxiliary backend is ValOs Portal."""
    from urllib.parse import urlparse

    base_url = str(getattr(client, "base_url", "") or "")
    host = (urlparse(base_url).hostname or "").lower()
    return host == "valos-agent.com" or host.endswith(".valos-agent.com")
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_valos_auxiliary_client(client):
        from agent.auxiliary_client import get_auxiliary_extra_body
        extra_body = get_auxiliary_extra_body() or {"tags": ["product=valos-agent"]}
...
def _get_exa_client():
    ...
    if _exa_client is None:
        ...
        _exa_client.headers["x-exa-integration"] = "valos-agent"
    return _exa_client
...
def check_firecrawl_api_key() -> bool:
    ...
    2) Firecrawl gateway origin + ValOs Subscriber access token
...
if __name__ == "__main__":
    ...
    valos_available = check_auxiliary_model()
    ...
    if not valos_available:
        print("❌ No auxiliary model available for LLM content processing")
        print("Set OPENROUTER_API_KEY, configure ValOs Portal, or set OPENAI_BASE_URL + OPENAI_API_KEY")
