...
Architecture:
    ~/.valos-agent/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        VALOS_WORKDIR                               — original dir path
...
CHECKPOINT_BASE = get_valos_home() / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("VALOS_CHECKPOINT_TIMEOUT", "30"))))
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "valos-agent@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "ValOs Checkpoint"], shadow_repo, working_dir)
    ...
    (shadow_repo / "VALOS_WORKDIR").write_text(
