"""ACP permission bridging — maps ACP approval requests to valos-agent approval callbacks."""
...
# Maps ACP PermissionOptionKind -> valos-agent approval result strings
_KIND_TO_VALOS = {
    "allow_once": "once",
    "allow_always": "always",
    "reject_once": "deny",
...
def make_approval_callback(
    ...
    """
    Return a valos-agent-compatible ``approval_callback(command, description) -> str``
        ...
        if isinstance(outcome, AllowedOutcome):
            ...
            for opt in options:
                if opt.option_id == option_id:
                    return _KIND_TO_VALOS.get(opt.kind, "deny")
