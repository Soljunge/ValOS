"""ACP agent server — exposes ValOs via the Agent Client Protocol."""
...
try:
    from valos_cli import __version__ as VALOS_VERSION
except Exception:
    VALOS_VERSION = "0.0.0"
...
class ValOsACPAgent(acp.Agent):
    """ACP Agent implementation wrapping ValOs AIAgent."""
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = getattr(state.agent, "enabled_toolsets", None) or ["valos-agent-acp"]
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethod(
                    ...
                    description=f"Authenticate ValOs using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="valos-agent", version=VALOS_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run ValOs on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    def _cmd_model(self, args: str, state: SessionState) -> str:
        ...
        try:
            from valos_cli.models import parse_model_input, detect_provider_for_model
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            from model_tools import get_tool_definitions
            toolsets = getattr(state.agent, "enabled_toolsets", None) or ["valos-agent-acp"]
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"ValOs v{VALOS_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when ValOs has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
