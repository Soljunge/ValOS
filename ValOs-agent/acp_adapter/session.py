"""ACP session manager — maps ACP sessions to ValOs AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.valos-agent/state.db``) so they
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed ValOs agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by ValOs AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current ValOs runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.valos-agent/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``VALOS_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_valos_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=valos_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _make_agent(
        ...
        *,
        ...
        kwargs = {
            "platform": "acp",
            "enabled_toolsets": ["valos-agent-acp"],
            "quiet_mode": True,
            "session_id": session_id,
