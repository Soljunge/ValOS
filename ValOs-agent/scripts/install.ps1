# ============================================================================
# ValOs Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   irm https://raw.githubusercontent.com/horatiubudai/valos-agent/main/scripts/install.ps1 | iex
#
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$ValOsHome = "$env:LOCALAPPDATA\valos-agent",
    [string]$InstallDir = "$env:LOCALAPPDATA\valos-agent\valos-agent"
)
...
$RepoUrlSsh = "git@github.com:horatiubudai/valos-agent.git"
$RepoUrlHttps = "https://github.com/horatiubudai/valos-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ ValOs Installer                   │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
    Write-Host "│  An open source AI agent by ValOs.              │" -ForegroundColor Magenta
    Write-Host "└─────────────────────────────────────────────────────────┘" -ForegroundColor Magenta
    Write-Host ""
}
...
function Test-Node {
    ...
    $managedNode = "$ValOsHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$ValOsHome\node;$env:Path"
        Write-Success "Node.js $version found (ValOs-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.valos-agent/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\valos-agent-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ValOsHome\node") { Remove-Item -Recurse -Force "$ValOsHome\node" }
                Move-Item $extractedDir.FullName "$ValOsHome\node"
                $env:Path = "$ValOsHome\node;$env:Path"

                $version = & "$ValOsHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.valos-agent/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/horatiubudai/valos-agent/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\valos-agent-$Branch.zip"
                $extractPath = "$env:TEMP\valos-agent-extract"
...
function Set-PathVariable {
    Write-Info "Setting up valos-agent command..."
    
    if ($NoVenv) {
        $valosBin = "$InstallDir"
    } else {
        $valosBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so valos-agent is globally available
    # On Windows, the valos-agent.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$valosBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$valosBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $valosBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set VALOS_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\valos-agent instead
    # of the Unix default ~/.valos-agent
    $currentValOsHome = [Environment]::GetEnvironmentVariable("VALOS_HOME", "User")
    if (-not $currentValOsHome -or $currentValOsHome -ne $ValOsHome) {
        [Environment]::SetEnvironmentVariable("VALOS_HOME", $ValOsHome, "User")
        Write-Success "Set VALOS_HOME=$ValOsHome"
    }
    $env:VALOS_HOME = $ValOsHome
    
    # Update current session
    $env:Path = "$valosBin;$env:Path"
    
    Write-Success "valos-agent command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.valos-agent directory structure
    New-Item -ItemType Directory -Force -Path "$ValOsHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ValOsHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ValOsHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ValOsHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ValOsHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ValOsHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ValOsHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ValOsHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ValOsHome\skills" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ValOsHome\whatsapp\session" | Out-Null
    
    # Create .env
    $envPath = "$ValOsHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.valos-agent/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.valos-agent/.env"
        }
    } else {
        Write-Info "~/.valos-agent/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$ValOsHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.valos-agent/config.yaml from template"
        }
    } else {
        Write-Info "~/.valos-agent/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$ValOsHome\SOUL.md"
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.valos-agent/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.valos-agent/"
    
    # Seed bundled skills into ~/.valos-agent/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.valos-agent/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.valos-agent/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ValOsHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.valos-agent/skills/"
            }
        }
    }
}
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m valos_cli.main setup
    } else {
        python -m valos_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $valosCmd = "$InstallDir\venv\Scripts\valos-agent.exe"
    if (-not (Test-Path $valosCmd)) {
        $valosCmd = "valos-agent"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ValOsHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'valos-agent whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ValOsHome\logs\gateway.log"
            Start-Process -FilePath $valosCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ValOsHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: valos-agent gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: valos-agent gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ValOsHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ValOsHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ValOsHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ValOsHome\valos-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   valos-agent              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   valos-agent setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   valos-agent config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   valos-agent config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   valos-agent gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   valos-agent update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/horatiubudai/valos-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
