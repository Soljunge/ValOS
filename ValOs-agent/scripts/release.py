...
VERSION_FILE = REPO_ROOT / "valos_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "teknium@valos-agent.com": "teknium1",
    ...
    "shannon@valos-agent.com": "shannonsands",
    ...
    "valos-agent@valos-agent.com": "horatiu.sol",
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/horatiubudai/valos-agent",
    ...
    lines.append(f"# ValOs v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for ValOs. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="ValOs Release Tool")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"ValOs v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"ValOs v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'ValOs v{new_version} ({calver_date})' "
