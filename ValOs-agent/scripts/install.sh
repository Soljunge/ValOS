...
# Configuration
REPO_URL_SSH="git@github.com:horatiubudai/valos-agent.git"
REPO_URL_HTTPS="https://github.com/horatiubudai/valos-agent.git"
VALOS_HOME="$HOME/.valos-agent"
INSTALL_DIR="${VALOS_INSTALL_DIR:-$VALOS_HOME/valos-agent}"
PYTHON_VERSION="3.11"
NODE_VERSION="22"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.valos-agent/valos-agent)"
...
print_banner() {
    ...
    echo "│             ⚕ ValOs Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by ValOs.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/horatiubudai/valos-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$VALOS_HOME/node/bin/node" ]; then
        export PATH="$VALOS_HOME/node/bin:$PATH"
        local found_ver=$("$VALOS_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (ValOs-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.valos-agent/node/..."
    ...
    # Place into ~/.valos-agent/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$VALOS_HOME/node"
    mkdir -p "$VALOS_HOME"
    mv "$extracted_dir" "$VALOS_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$VALOS_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$VALOS_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$VALOS_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$VALOS_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$VALOS_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.valos-agent/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "ValOs itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="valos-agent-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if ValOs behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up valos-agent command..."

    if [ "$USE_VENV" = true ]; then
        VALOS_BIN="$INSTALL_DIR/venv/bin/valos-agent"
    else
        VALOS_BIN="$(which valos-agent 2>/dev/null || echo "")"
        if [ -z "$VALOS_BIN" ]; then
            log_warn "valos-agent not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$VALOS_BIN" ]; then
        log_warn "valos-agent entry point not found at $VALOS_BIN"
    ...
    mkdir -p "$HOME/.local/bin"
    ln -sf "$VALOS_BIN" "$HOME/.local/bin/valos-agent"
    log_success "Symlinked valos-agent → ~/.local/bin/valos-agent"
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# ValOs — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
    ...
    # Export for current session so valos-agent works immediately
    export PATH="$HOME/.local/bin:$PATH"

    log_success "valos-agent command ready"
}

copy_config_templates() {
    log_info "Setting up configuration files..."

    # Create ~/.valos-agent directory structure (config at top level, code in subdir)
    mkdir -p "$VALOS_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.valos-agent/.env (top level, easy to find)
    if [ ! -f "$VALOS_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$VALOS_HOME/.env"
            log_success "Created ~/.valos-agent/.env from template"
        else
            touch "$VALOS_HOME/.env"
            log_success "Created ~/.valos-agent/.env"
        fi
    else
        log_info "~/.valos-agent/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.valos-agent/config.yaml (top level, easy to find)
    if [ ! -f "$VALOS_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$VALOS_HOME/config.yaml"
            log_success "Created ~/.valos-agent/config.yaml from template"
        fi
    else
        log_info "~/.valos-agent/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$VALOS_HOME/SOUL.md" ]; then
        cat > "$VALOS_HOME/SOUL.md" << 'SOUL_EOF'
# ValOs Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how ValOs communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.valos-agent/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.valos-agent/"

    # Seed bundled skills into ~/.valos-agent/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.valos-agent/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.valos-agent/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$VALOS_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$VALOS_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.valos-agent/skills/"
        fi
    fi
}
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ...
            *)
                ...
                log_info "This is standard Playwright setup — ValOs itself does not require root access."
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'valos-agent setup' after install."
        return 0
    fi
    ...
    # Run valos-agent setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m valos_cli.main setup < /dev/tty
    else
        python -m valos_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$VALOS_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for ValOs to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$VALOS_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'valos-agent whatsapp' to pair via QR code..."
            echo ""
            read -p "Pair WhatsApp now? [Y/n] " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
                VALOS_CMD="$HOME/.local/bin/valos-agent"
                [ ! -x "$VALOS_CMD" ] && VALOS_CMD="valos-agent"
                $VALOS_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'valos-agent whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'valos-agent gateway install' later."
        return 0
    fi
    ...
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        VALOS_CMD="$HOME/.local/bin/valos-agent"
        if [ ! -x "$VALOS_CMD" ]; then
            VALOS_CMD="valos-agent"
        fi

        if command -v systemctl &> /dev/null; then
            log_info "Installing systemd service..."
            if $VALOS_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $VALOS_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: valos-agent gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: valos-agent gateway"
            fi
        else
            log_info "systemd not available — starting gateway in background..."
            nohup $VALOS_CMD gateway > "$VALOS_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.valos-agent/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: valos-agent gateway"
        fi
    else
        log_info "Skipped. Start the gateway later with: valos-agent gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.valos-agent/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.valos-agent/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.valos-agent/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.valos-agent/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.valos-agent/valos-agent/"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}valos-agent${NC}              Start chatting"
    echo -e "   ${GREEN}valos-agent setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}valos-agent config${NC}       View/edit configuration"
    echo -e "   ${GREEN}valos-agent config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}valos-agent gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}valos-agent update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    echo -e "${YELLOW}⚡ Reload your shell to use 'valos-agent' command:${NC}"
    echo ""
    LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
