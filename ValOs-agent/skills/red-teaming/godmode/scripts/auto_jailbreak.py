...
Usage in execute_code:
    exec(open(os.path.expanduser(
        "~/.valos-agent/skills/red-teaming/godmode/scripts/auto_jailbreak.py"
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("VALOS_HOME", Path.home() / ".valos-agent")) / "skills" / "red-teaming" / "godmode"
...
VALOS_HOME = Path(os.getenv("VALOS_HOME", Path.home() / ".valos-agent"))
CONFIG_PATH = VALOS_HOME / "config.yaml"
PREFILL_PATH = VALOS_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # ValOs/ValOs models — already uncensored, just needs clean prompt
    "valos-agent": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "valos-agent" in model_lower or "valos" in model_lower:
        return "valos-agent"
...
def _get_current_model() -> tuple:
    """Read current model and provider from ValOs config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.valos-agent/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart ValOs for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart ValOs for changes to take effect.")
