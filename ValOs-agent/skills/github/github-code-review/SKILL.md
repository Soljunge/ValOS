...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.valos-agent/.env ] && grep -q "^GITHUB_TOKEN=" ~/.valos-agent/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.valos-agent/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source ~/.valos-agent/skills/github/github-auth/scripts/gh-env.sh
...
gh pr review $PR_NUMBER --approve --body "Reviewed by ValOs. Code looks clean — good test coverage, no security concerns."
