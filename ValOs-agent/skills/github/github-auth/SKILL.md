...
- Give it a name like "valos-agent"
- Select scopes:
...
- Give it a title like "valos-agent-<machine-name>"
...
elif [ -f ~/.valos-agent/.env ] && grep -q "^GITHUB_TOKEN=" ~/.valos-agent/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.valos-agent/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
