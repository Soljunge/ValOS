...
description: Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration via Python. Uses OAuth2 with automatic token refresh. No external binaries needed — runs entirely with Google's Python client libraries in the ValOs venv.
version: 1.0.0
author: ValOs
license: MIT
...
```bash
GSETUP="python ~/.valos-agent/skills/productivity/google-workspace/scripts/setup.py"
```
...
- Token is stored at `google_token.json` under the active profile's `VALOS_HOME` and auto-refreshes.
- Pending OAuth session state/verifier are stored temporarily at `google_oauth_pending.json` under the active profile's `VALOS_HOME` until exchange completes.
- ValOs now refuses to overwrite a full Google Workspace token with a narrower re-auth token missing Gmail scopes, so one profile's partial consent cannot silently break email actions later.
...
```bash
GAPI="python ~/.valos-agent/skills/productivity/google-workspace/scripts/google_api.py"
```
