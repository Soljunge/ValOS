#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for ValOs.
...
from valos_constants import display_valos_home, get_valos_home

VALOS_HOME = get_valos_home()
TOKEN_PATH = VALOS_HOME / "google_token.json"
CLIENT_SECRET_PATH = VALOS_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = VALOS_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same ValOs profile to refresh consent."
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to ValOs home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_valos_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for ValOs")
