#!/usr/bin/env python3
"""Google Workspace API CLI for ValOs.
...
def get_credentials():
    ...
    if missing_scopes:
        ...
        print(
            f"Re-run setup.py from the active ValOs profile ({display_valos_home()}) to restore full access.",
            file=sys.stderr,
        )
        sys.exit(1)
    return creds
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for ValOs")
