...
This skill intentionally does not vendor a separate CLI implementation into ValOs. Install and use upstream `x-cli` instead.
...
If you already keep your X credentials in `~/.valos-agent/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.valos-agent/.env ~/.config/x-cli/.env
```
...
x-cli tweet search "from:horatiu.sol" --max 3
...
- **Credential drift**: if you rotate tokens in `~/.valos-agent/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
