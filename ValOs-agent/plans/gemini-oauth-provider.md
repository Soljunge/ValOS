...
## Client ID
- Need to register a "Desktop app" OAuth client on a ValOs GCP project
...
## Token Lifecycle
- Store at `~/.valos-agent/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `valos_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `valos_cli/models.py` — Add Gemini model catalog
4. `valos_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `valos_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `valos_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- ValOs GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via VALOS_GEMINI_CLIENT_ID env var
...
- valos-agent Copilot OAuth: `valos_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
