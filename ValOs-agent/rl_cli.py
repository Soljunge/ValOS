...
if tinker_atropos_dir.exists():
    ...
    os.environ['VALOS_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to valos-agent directory if submodule not found
...
from valos_constants import get_valos_home, OPENROUTER_BASE_URL

DEFAULT_MODEL = "anthropic/claude-opus-4.5"
DEFAULT_BASE_URL = OPENROUTER_BASE_URL


def load_valos_config() -> dict:
    """
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.valos-agent/config.yaml if not provided)
    ...
    """
    # Load config from ~/.valos-agent/config.yaml
    config = load_valos_config()
    ...
    if check_server:
        ...
        if ok:
            ...
            if missing:
                ...
                print("   Add them to ~/.valos-agent/.env")
            else:
