"""
ValOs MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (ValOs-specific extra)

Usage:
    valos-agent mcp serve
    valos-agent mcp serve --verbose
...
logger = logging.getLogger("valos-agent.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using VALOS_HOME."""
    try:
        from valos_constants import get_valos_home
        return get_valos_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("VALOS_HOME", Path.home() / ".valos-agent")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from valos_constants import get_valos_home
        directory_file = get_valos_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("VALOS_HOME", Path.home() / ".valos-agent")
...
class EventBridge:
    ...
    This is the ValOs equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from valos_constants import get_valos_home
            db_file = get_valos_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("VALOS_HOME", Path.home() / ".valos-agent")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the ValOs MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'valos-agent[mcp]'"
        )

    mcp = FastMCP(
        "valos-agent",
        instructions=(
            "ValOs messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the ValOs MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
