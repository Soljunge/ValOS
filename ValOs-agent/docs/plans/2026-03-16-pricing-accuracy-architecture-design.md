...
ValOs should only show dollar costs when they are backed by an official source for the user's actual billing path.
...
Current ValOs behavior has four structural issues:
...
ValOs must stop keying pricing solely by `model`.
...
- manual `valos-agent pricing sync`

## Reconciliation Architecture

Live requests may produce only an estimate initially. ValOs should reconcile them later when a provider exposes actual billed cost.

Suggested flow:

1. Agent call completes.
2. ValOs stores canonical usage plus reconciliation ids.
3. ValOs computes an immediate estimate if a pricing source exists.
...
That path cannot satisfy the product requirement. ValOs should instead migrate to:
...
This is the minimum architecture that makes the statement "ValOs pricing is backed by official sources where possible, and otherwise clearly labeled" defensible.
